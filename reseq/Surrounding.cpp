...
void Surrounding::ChangeSurroundingBase(uintSurPos pos, Dna new_base) {
    ...
    // ~ = bit-wise not
    sur_.at(pos / kRange) =
        (sur_.at(pos / kRange) & ~(3 << bit_in_block)) + (static_cast<intType>(new_base) << bit_in_block);
}
...
void Surrounding::DeleteSurroundingBaseShiftingOnLeftSide(uintSurPos pos, Dna new_end_base) {
    ...
    sur_.at(del_block) =
        (sur_.at(del_block) >> (bit_in_block + 2) << (bit_in_block)) + sur_.at(del_block) % (1 << bit_in_block);
}
...
void Surrounding::InsertSurroundingBasesShiftingOnRightSide(uintSurPos pos, DnaString new_bases) {
    ...
    for (auto cur_block = kNumBlocks - shift_blocks; cur_block-- > block + 1;) {
        ...
        sur_.at(cur_block) += sur_.at(cur_block - 1) % (1 << 2 * shift_bases) *
                              (1 << 2 * (kRange - shift_bases)); // Add new bases from previous block
    }

    uintSurPos inv_pos_in_block = kRange - (pos % kRange);
    intType tmp_sur = sur_.at(block) %
                      (1 << 2 * inv_pos_in_block); // Store everything that needs to be shifted out of the start block
    ...
    if (0 < shift_blocks && kNumBlocks > block + shift_blocks) {
        ...
        // Shift everything that has to be shifted right from starting block and was not already shifted above (Happens
        // if the bases need to go into an empty block freed by the block shift)
...
void Surrounding::InsertSurroundingBasesShiftingOnLeftSide(uintSurPos pos, DnaString new_bases) {
    ...
    intType tmp_sur =
        sur_.at(block) % (1 << 2 * (kRange - pos_in_block)); // Store everything that needs to stay in the start block
    if (pos_in_block > shift_bases) {
        ...
        sur_.at(block) =
            0; // If everything was already shifted, we only need what's right of the insertion and that is in tmp_sur
    }
    ...
    if (0 < shift_blocks) {
        if (block >= shift_blocks) {
            ...
            // Shift everything that has to be shifted left from starting block and was not already shifted above
            // (Happens if the bases need to go into an empty block freed by the block shift)
            sur_.at(block - shift_blocks) = sur_.at(block) << 2 * (kRange - (pos_in_block - shift_bases));
        }
        sur_.at(block) = 0; // The not-shifted stuff to keep was moved to the proper shifted block, so now we can clean
                            // the start block surrounding
    }

    // Insert new bases in starting block
    uintSurPos bases_to_insert_into_this_block = min(bases_to_insert, pos_in_block);
    uintSurPos ins_pos_to = length(new_bases); // In case we do not insert all bases from new_bases, we need to keep
                                               // track of this separately to bases_to_insert
    ...
    for (uintSurBlockId block = 0; block < Surrounding::kNumBlocks; ++block) {
        ...
        for (Surrounding::intType sur = 0; sur < Surrounding::Size(); ++sur) {
            ...
            for (pos = 0; pos < Surrounding::kRange; ++pos) {
                bias_.at(block).at(sur) +=
                    separated.at(bases.at(Surrounding::kRange - 1 - pos) + pos * 4 + block * Surrounding::kRange * 4);
            }

            // Update the current bases we are at: If a position exceeds valid bases, set it back to A and increase next
            // position bases[0] is for pos=surrounding_range_-1
            pos = 0;
