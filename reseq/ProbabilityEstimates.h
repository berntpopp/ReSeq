...
template <size_t N>
inline void CombineDimIndices(std::array<std::vector<uintMatrixIndex>, N>& dim_indeces1,
                              const std::array<std::vector<uintMatrixIndex>, N>& dim_indeces2) {
...
template <uintMarginId N> class DataStorage {
  ...
  private:
    ...
    void CombineEndBinsToMinCount(std::array<std::vector<uintMatrixIndex>, N>& dim_indices,
                                  std::array<Vect<uintMatrixCount>, N>& dim_control);
    void CombineBins(std::array<std::vector<uintMatrixIndex>, N>& initial_dim_indices_reduced,
                     std::array<Vect<uintMatrixCount>, N>& dim_control);

    void FillDiffMatrix(std::vector<double>& diff_matrix, uintMarginId dimension) const;
    double MaxDiff(const std::vector<double>& diff_matrix, uintMatrixIndex combined_index,
                   std::vector<uintMatrixIndex>& same_combined_index,
                   const std::vector<uintMatrixIndex>& dim_indices_reduced,
                   const std::vector<uintMatrixIndex>& dim_indices_count) const;

    // Google test
    friend class reseq::ProbabilityEstimatesTest;

  public:
    DataStorage() { dim_size_.fill(0); }

    bool SetUp(const std::array<std::pair<const Vect<Vect<uintMatrixCount>>*, bool>, N*(N - 1) / 2>& margins,
               const Vect<SeqQualityStats<uintMatrixCount>>* alternative_margin,
               std::array<std::vector<uintMatrixIndex>, N>& dim_indices,
               std::array<std::vector<uintMatrixIndex>, N>& initial_dim_indices_reduced, std::mutex& print_mutex);

    template <uintMarginId A, uintMarginId B>
    inline double get(uintMatrixIndex index_a, uintMatrixIndex index_b) const {
        if (A > B) {
    ...
    template <uintMarginId A> inline uintMatrixIndex size() const { return dim_size_.at(A); }

    void Reduce(DataStorage<N>& reduced_data, const std::array<std::vector<uintMatrixIndex>, N>& dim_indices_reduced,
                const std::array<std::vector<uintMatrixIndex>, N>& dim_indices_count) const;
    void Reduce(DataStorage<N>& reduced_data, std::array<std::vector<uintMatrixIndex>, N>& dim_indices_reduced,
                std::array<std::vector<uintMatrixIndex>, N>& dim_indices_count, uintMatrixIndex max_size_dim) const;
    bool Expand(DataStorage<N>& reduced_data, std::array<std::vector<uintMatrixIndex>, N>& dim_indices_reduced,
                ...
                std::array<std::vector<uintMatrixIndex>, N>& expansion_indices,
                std::array<std::vector<uintMatrixIndex>, N>& expansion_count, uintMatrixIndex max_duplications) const;
};
...
template <uintMarginId N> class LogArrayCalc {
  ...
  public:
    ...
    void Normalize() {
            ...
            for (uintMatrixIndex i = dim_size_.at(dim_a); i--;) {
                ...
                for (uintMatrixIndex j = dim_size_.at(dim_b); j--;) {
                    if (0.0 != dim2_.at(n).at(i * dim_size_.at(dim_b) + j)) {
                        prod += std::log(
                            dim2_.at(n).at(i * dim_size_.at(dim_b) + j)); // Use the logarithm here so that we don't run
                                                                          // into a problem with overflow or underflow
                        ++count;
                    }
                }
                if (count) {
        ...
        // Now redistribute log_dim0 and log_dim1 equally to dim2_ again, so that exp(log_dim0+log_dim1)*dim2_ doesn't
        // change, but log_dim0=log_dim1=0 afterwards First store the factor to apply in log_dim1
    ...
    void Expand(const std::array<std::vector<uintMatrixIndex>, N>& dim_indices_reduced,
        ...
        for (auto n = N; n--;) {
            ...
            for (uintMatrixIndex i = dim_indices_reduced.at(n).size(); i--;) {
                multipliers.at(n).at(i) =
                    std::pow((dim_indices_new_count ? static_cast<double>((*dim_indices_new_count).at(n).at(i)) : 1.0) /
                                 dim_indices_count.at(n).at(dim_indices_reduced.at(n).at(i)),
                             1.0 / (N - 1));
            }
        }
        ...
        for (uintMarginId n = kNumMargins; n--;) {
            ...
            for (uintMatrixIndex i = dim_indices_reduced.at(dim_a).size(); i--;) {
                for (uintMatrixIndex j = dim_indices_reduced.at(dim_b).size(); j--;) {
                    dim2_.at(n).at(i * dim_indices_reduced.at(dim_b).size() + j) =
                        old_values.at(dim_indices_reduced.at(dim_a).at(i) * dim_indices_count.at(dim_b).size() +
                                      dim_indices_reduced.at(dim_b).at(j)) *
                        multipliers.at(dim_a).at(i) * multipliers.at(dim_b).at(j);
                }
            }
        }
    ...
    template <uintMarginId A, uintMarginId B>
    inline double& dim2(decltype(dim2_.at(0).size()) index1, decltype(dim2_.at(0).size()) index2) {
        if (A > B) {
    ...
    template <uintMarginId A, uintMarginId B>
    inline const double& dim2(decltype(dim2_.at(0).size()) index1, decltype(dim2_.at(0).size()) index2) const {
        if (A > B) {
            return dim2_.at(MapDim2To1<A, B, N>()).at(index1 * dim_size_.at(B) + index2);
        } else {
            return dim2_.at(MapDim2To1<B, A, N>()).at(index2 * dim_size_.at(A) + index1);
...
template <uintMarginId N> class LogArrayResult {
  ...
  public:
    LogArrayResult() { limits_.fill({0, 0}); }
    void GetResults(const LogArrayCalc<N>& calc, const std::array<std::vector<uintMatrixIndex>, N>& dim_indices) {
        if (dim_indices.at(0).size()) {
            // Order first parameter by increasing mean, so that when called from the back the values with the highest
            // probability come first
            ...
            for (uintMarginId n = kNumMargins; n--;) {
                ...
                for (auto i = dim_indices.at(dim_a).size(); i--;) {
                    for (auto j = dim_indices.at(0).size(); j--;) {
                        dim2_.at(n).at((dim_indices.at(dim_a).at(i) - limits_.at(n).first) * par0_indeces_.size() +
                                       par0_indeces_.at(j)) = calc.dim2_.at(n).at(i * dim_indices.at(0).size() + j);
                    }
                }
            }

            // Set par0_indeces to contain the link from result indeces to original indeces (ignoring the calculation
            // indeces we needed before)
            for (auto k = dim_order.size(); k--;) {
            ...
            for (uintMatrixIndex i = 1; i < limits_.at(n).second - limits_.at(n).first; ++i) {
                ...
                if (filled) {
                    ...
                    for (auto imp_ind = last_filled_index + 1; imp_ind < i; ++imp_ind) {
                        for (auto j = 0; j < par0_indeces_.size(); ++j) {
                            dim2_.at(n).at(imp_ind * par0_indeces_.size() + j) =
                                dim2_.at(n).at(last_filled_index * par0_indeces_.size() + j) *
                                    (imp_ind - last_filled_index) / (i - last_filled_index) +
                                dim2_.at(n).at(i * par0_indeces_.size() + j) * (i - imp_ind) /
                                    (i - last_filled_index); // Impute linearly
                        }
                    }
                    last_filled_index = i;
                }
    ...
    inline uintMatrixIndex Draw(std::vector<double>& prob, double& prob_sum, std::array<uintMatrixIndex, N - 1> index,
        ...
        if (par0_indeces_.size()) {
            ...
            random_number *= prob_sum; // Multiply the random number [0,1[ by the sum, which is the same as dividing all
                                       // likelihoods by the sum to make them probabilities
    ...
    inline const std::vector<uintMatrixIndex>& PossibleValues() const { return par0_indeces_; }
    ...
    inline void ModifyPar0(uintMatrixIndex par0_index, double multiplier) {
        ...
        if (corrected_index < par0_indeces_.size() && par0_indeces_.at(corrected_index) == par0_index) {
            // par0_index exists
            for (auto matrix_index = corrected_index; matrix_index < dim2_.at(0).size();
                 matrix_index += par0_indeces_.size()) {
...
template <uintMarginId G1, uintMarginId G2, uintMarginId L>
double SumLogTerms(const DataStorage<3>& data, const LogArrayCalc<3>& mat, uintMatrixIndex given_index1,
...
template <uintMarginId G1, uintMarginId G2, uintMarginId L1, uintMarginId L2>
double SumLogTerms(const DataStorage<4>& data, const LogArrayCalc<4>& mat, uintMatrixIndex given_index1,
                   uintMatrixIndex given_index2) {
    double tmp_sum;
    double sum = 0.0;
    for (uintMatrixIndex i = 0; i < data.size<L2>(); ++i) {
        ...
        for (uintMatrixIndex j = 0; j < data.size<L1>(); ++j) {
            tmp_sum += mat.template dim2<G1, L1>(given_index1, j) * mat.template dim2<G2, L1>(given_index2, j) *
                       mat.template dim2<L2, L1>(i, j);
        }
...
template <uintMarginId G1, uintMarginId G2, uintMarginId L1, uintMarginId L2, uintMarginId L3>
double SumLogTerms(const DataStorage<5>& data, const LogArrayCalc<5>& mat, uintMatrixIndex given_index1,
                   uintMatrixIndex given_index2) {
    ...
    for (uintMatrixIndex i = 0; i < data.size<L3>(); ++i) {
        ...
        for (uintMatrixIndex j = 0; j < data.size<L2>(); ++j) {
            ...
            for (uintMatrixIndex k = 0; k < data.size<L1>(); ++k) {
                tmp_sum2 += mat.template dim2<G1, L1>(given_index1, k) * mat.template dim2<G2, L1>(given_index2, k) *
                            mat.template dim2<L3, L1>(i, k) * mat.template dim2<L2, L1>(j, k);
            }
            tmp_sum += tmp_sum2 * mat.template dim2<G1, L2>(given_index1, j) *
                       mat.template dim2<G2, L2>(given_index2, j) * mat.template dim2<L3, L2>(i, j);
        }
...
template <uintMarginId N> class LogIPF {
  ...
  private:
    ...
    // Functions
    inline void UpdateEstimate(double& matrix_element, double factor) {
    ...
    template <uintMarginId U1, uintMarginId U2, uintMarginId... L>
    inline void IPFCheckPrecision(const DataStorage<N>& data) {
        double sum;
    ...
    template <uintMarginId U1, uintMarginId U2, uintMarginId... L>
    inline void IPFPotentialStep(const DataStorage<N>& data) {
        double sum;
        ...
        if (precision_ > precision_aim_) {
            for (uintMatrixIndex i = 0; i < data.template size<U1>(); ++i) {
                for (uintMatrixIndex j = 0; j < data.template size<U2>(); ++j) {
                    UpdateEstimate(estimates_.template dim2<U1, U2>(i, j),
                                   update_factors_.at(i * data.template size<U2>() + j));
                }
            }
        }
    ...
    template <uint16_t UPDATE, uintMarginId U1, uintMarginId U2, uintMarginId... L>
    inline void IPFUpdateOrCheck(const DataStorage<N>& data) {
        if (1 == UPDATE) {
        ...
        if (1 == UPDATE || (2 == UPDATE && precision_ > precision_aim_)) { // IPFStep or IPFPotentialStep with update
            ...
            if (precision_ > margin_precision_.at(S)) {
                ...
                if (steps_ >= last_update_.at(S) + update_dist_.at(S) * kNumMargins) {
                    // If precision did not get worse since the last update and we called it due to precision
                    // independent updates, update less often
                    ++update_dist_.at(S);
                }
            }
    ...
    template <uintMarginId S> inline void IPFStepList(const DataStorage<N>& data) { IPFStepCaller<S>(data); }

    template <uintMarginId S1, uintMarginId... S>
    inline typename std::enable_if<sizeof...(S) != 0, void>::type IPFStepList(const DataStorage<N>& data) {
        IPFStepList<S1>(data);
        IPFStepList<S...>(data);
    }
    ...
    inline uintMatrixIndex ReductionInfo(std::string& reduction_descriptor,
                                         const std::array<std::vector<uintMatrixIndex>, N>& dim_indices_count) {
        // Prepare
        ...
        for (uintMarginId n = 0; n < N; ++n) {
            step_reduction_factor *=
                static_cast<double>(dim_indices_reduced_.at(n).size()) / dim_indices_count.at(n).size();
            if (n) {
                reduction_descriptor_stream << ',';
            }
    ...
    inline void ReconstructDimIndicesCount(std::array<std::vector<uintMatrixIndex>, N>& dim_indices_count,
                                           const std::array<std::vector<uintMatrixIndex>, N>& dim_indices_reduced) {
        // Reconstruct dim_indices_count from dim_indices_reduced_
        for (auto n = N; n--;) {
            dim_indices_count.at(n).resize(
                *std::max_element(dim_indices_reduced.at(n).begin(), dim_indices_reduced.at(n).end()) + 1, 0);
  ...
  public:
    LogIPF()
        : steps_(0), needed_updates_(0), precision_(std::numeric_limits<double>::max()), last_margin_(0),
          normalize_(false) {
    ...
    void IterativeProportionalFitting(
        std::atomic<bool>& error_during_fitting, std::atomic<bool>& precision_improved, double precision_aim,
        uintNumFits max_iterations,
        const std::array<std::pair<const Vect<Vect<uintMatrixCount>>*, bool>, kNumMargins>& margins,
        const Vect<SeqQualityStats<uintMatrixCount>>* alternative_margin, const std::string& descriptor,
        ...
        if (precision_ > precision_aim) {
            ...
            while (true) { // Break conditions at the end
                ...
                if (data.Expand(reduced_data, dim_indices_reduced_, dim_indices_count, expansion_indices,
                                expansion_count, N)) {
...
template <>
template <uintMarginId S, uint16_t UPDATE>
inline void LogIPF<3>::IPFStepCallerTemp(const DataStorage<3>& data) {
    switch (S) {
    case 0:
...
template <>
template <uintMarginId S, uint16_t UPDATE>
inline void LogIPF<4>::IPFStepCallerTemp(const DataStorage<4>& data) {
    switch (S) {
    case 0:
...
template <>
template <uintMarginId S, uint16_t UPDATE>
inline void LogIPF<5>::IPFStepCallerTemp(const DataStorage<5>& data) {
    switch (S) {
    case 0:
...
class ProbabilityEstimates {
  private:
    ...
    // ipf calc
    std::array<std::vector<std::array<ProbabilityEstimatesSubClasses::LogIPF<5>, 4>>, 2>
        quality_; // quality_[template_segment][tile_id][ref_base] : quality, sequence quality, previous quality,
                  // position, error rate
    std::array<std::vector<ProbabilityEstimatesSubClasses::LogIPF<4>>, 2>
        sequence_quality_; // sequence_quality_[template_segment][tile_id]: sequence quality, gc, mean error rate,
                           // fragment length
    std::array<std::vector<std::array<std::array<ProbabilityEstimatesSubClasses::LogIPF<5>, 5>, 4>>, 2>
        base_call_; // base_call_[template_segment][tile_id][ref_base][domError] : called base, quality, position, error
                    // number, error rate
    std::array<std::array<std::array<ProbabilityEstimatesSubClasses::LogIPF<4>, 5>, 5>, 4>
        dom_error_; // dom_error_[ref_base][last_ref_base][dom_last5]: dominant error, distance, gc, start rate
    std::array<std::array<ProbabilityEstimatesSubClasses::LogIPF<4>, 5>, 4>
        error_rate_; // error_rate_[ref_base][dom_error]: error rate, distance, gc, start rate
    std::array<std::array<ProbabilityEstimatesSubClasses::LogIPF<4>, 6>, 2>
        indels_; // indels_[Insertion/DeletionBefore][PreviousRegularCall]: indel, indel pos/length, position, gc

    // ipf results
    std::array<std::vector<std::array<ProbabilityEstimatesSubClasses::LogArrayResult<5>, 4>>, 2>
        quality_result_; // quality_result_[template_segment][tile_id][ref_base] : quality, sequence quality, previous
                         // quality, position, error rate
    std::array<std::vector<ProbabilityEstimatesSubClasses::LogArrayResult<4>>, 2>
        sequence_quality_result_; // sequence_quality_result_[template_segment][tile_id]: sequence quality, gc, mean
                                  // error rate, fragment length
    std::array<std::vector<std::array<std::array<ProbabilityEstimatesSubClasses::LogArrayResult<5>, 5>, 4>>, 2>
        base_call_result_; // base_call_result_[template_segment][tile_id][ref_base][domError] : called base, quality,
                           // position, error number, error rate
    std::array<std::array<std::array<ProbabilityEstimatesSubClasses::LogArrayResult<4>, 5>, 5>, 4>
        dom_error_result_; // dom_error_result_[ref_base][last_ref_base][dom_last5]: dominant error, distance, gc, start
                           // rate
    std::array<std::array<ProbabilityEstimatesSubClasses::LogArrayResult<4>, 5>, 4>
        error_rate_result_; // error_rate_result_[ref_base][dom_error]: error rate, distance, gc, start rate
    std::array<std::array<ProbabilityEstimatesSubClasses::LogArrayResult<4>, 6>, 2>
        indels_result_; // indels_result_[Insertion/DeletionBefore][PreviousRegularCall]: indel, indel pos/length,
                        // position, gc

    void SetVariables(uintTileId num_tiles);

    inline const Vect<SeqQualityStats<uintMatrixCount>>*
    DefineMarginsQuality(const DataStats& stats,
                         std::array<std::pair<const Vect<Vect<uintMatrixCount>>*, bool>, 10>& margins,
        ...
        margins.at(0) = {&stats.Qualities().BaseQualityForSequenceQualityReference(template_segment, tile_id, ref_base),
                         true};
        margins.at(1) = {
            &stats.Qualities().BaseQualityForPrecedingQualityReference(template_segment, tile_id, ref_base), true};
        margins.at(3) = {&stats.Qualities().BaseQualityForErrorRateReference(template_segment, tile_id, ref_base),
                         true};
        margins.at(4) = {
            &stats.Qualities().PrecedingQualityForSequenceQualityReference(template_segment, tile_id, ref_base), false};
        margins.at(5) = {&stats.Qualities().SequenceQualityForPositionReference(template_segment, tile_id, ref_base),
                         true};
        margins.at(6) = {&stats.Qualities().SequenceQualityForErrorRateReference(template_segment, tile_id, ref_base),
                         true};
        margins.at(7) = {&stats.Qualities().PrecedingQualityForPositionReference(template_segment, tile_id, ref_base),
                         true};
        margins.at(8) = {&stats.Qualities().PrecedingQualityForErrorRateReference(template_segment, tile_id, ref_base),
                         true};
    ...
    inline const Vect<SeqQualityStats<uintMatrixCount>>*
    DefineMarginsSequenceQuality(const DataStats& stats,
                                 std::array<std::pair<const Vect<Vect<uintMatrixCount>>*, bool>, 6>& margins,
                                 uintTempSeq template_segment, uintTileId tile_id) {
        // Dimension order: sequence quality, gc, mean error rate, fragment length
        margins.at(1) = {
            &stats.Qualities().SequenceQualityMeanForMeanErrorRatePerTileReference(template_segment, tile_id), true};
        margins.at(2) = {
            &stats.Qualities().SequenceQualityMeanForFragmentLengthPerTileReference(template_segment, tile_id), true};
    ...
    inline const Vect<SeqQualityStats<uintMatrixCount>>* DefineMarginsBaseCall(
        const DataStats& stats, std::array<std::pair<const Vect<Vect<uintMatrixCount>>*, bool>, 10>& margins,
        uintTempSeq template_segment, uintTileId tile_id, uintBaseCall ref_base, uintBaseCall dom_error) {
        // Dimension order: called base, quality, position, error number, error rate
        margins.at(0) = {&stats.Errors().CalledBasesByBaseQuality(template_segment, tile_id, ref_base, dom_error),
                         false};
        ...
        margins.at(6) = {
            &stats.Qualities().BaseQualityForErrorRateReference(template_segment, tile_id, ref_base, dom_error), true};
    ...
    inline void DefineMarginsDominantError(const DataStats& stats,
                                           std::array<std::pair<const Vect<Vect<uintMatrixCount>>*, bool>, 6>& margins,
                                           uintTempSeq ref_base, uintBaseCall last_ref_base, uintBaseCall dom_last5) {
    ...
    inline void DefineMarginsErrorRate(const DataStats& stats,
                                       std::array<std::pair<const Vect<Vect<uintMatrixCount>>*, bool>, 6>& margins,
                                       uintBaseCall ref_base, uintBaseCall dom_error) {
    ...
    inline void DefineMarginsIndels(const DataStats& stats,
                                    std::array<std::pair<const Vect<Vect<uintMatrixCount>>*, bool>, 6>& margins,
                                    uintInDelType indel_type, uintBaseCall last_call) {
    ...
    template <size_t N>
    void UpdateMarginDefinitionToTotal(std::array<std::pair<const Vect<Vect<uintMatrixCount>>*, bool>, N>& margins,
                                       const std::array<Vect<Vect<uintMatrixCount>>, N>& margin_sums) {
        for (auto n = N; n--;) {
    ...
    template <size_t N>
    void AddMarginsFromDefinition(std::array<Vect<Vect<uintMatrixCount>>, N>& margin_sums,
                                  const std::array<std::pair<const Vect<Vect<uintMatrixCount>>*, bool>, N>& margins,
                                  const Vect<SeqQualityStats<uintMatrixCount>>* alternative_margin) {
        size_t N2(0);
        switch (N) {
        case 3:
    ...
    void IterativeProportionalFitting(const DataStats& stats, IPFDataSelector selected_data,
                                      uintTempSeq template_segment, uintTileId tile_id, uintBaseCall ref_base,
  ...
  public:
    ProbabilityEstimates() : current_param_(0), error_during_fitting_(false), precision_improved_(false) {}

    void PrepareResult();

    const ProbabilityEstimatesSubClasses::LogArrayResult<5>& Quality(uintTempSeq template_segment, uintTileId tile_id,
                                                                     uintBaseCall base) const {
        return quality_result_.at(template_segment).at(tile_id).at(base);
    }
    const ProbabilityEstimatesSubClasses::LogArrayResult<4>& SequenceQuality(uintTempSeq template_segment,
                                                                             uintTileId tile_id) const {
        return sequence_quality_result_.at(template_segment).at(tile_id);
    }
    const ProbabilityEstimatesSubClasses::LogArrayResult<5>&
    BaseCall(uintTempSeq template_segment, uintTileId tile_id, uintBaseCall ref_base, uintBaseCall dom_error) const {
        return base_call_result_.at(template_segment).at(tile_id).at(ref_base).at(dom_error);
    }
    const ProbabilityEstimatesSubClasses::LogArrayResult<4>& DominantError(uintBaseCall base, uintBaseCall prev_base,
                                                                           uintBaseCall dom_last5) const {
        return dom_error_result_.at(base).at(prev_base).at(dom_last5);
    }
    const ProbabilityEstimatesSubClasses::LogArrayResult<4>& ErrorRate(uintBaseCall base,
                                                                       uintBaseCall dom_last5) const {
        return error_rate_result_.at(base).at(dom_last5);
    }
    const ProbabilityEstimatesSubClasses::LogArrayResult<4>& InDels(uintInDelType indel_type,
                                                                    uintBaseCall last_call) const {
        ...
        for (auto template_segment = base_call_result_.size(); template_segment--;) {
            for (auto tile_id = base_call_result_.at(template_segment).size(); tile_id--;) {
                for (auto ref_base = base_call_result_.at(template_segment).at(tile_id).size(); ref_base--;) {
                    for (auto dom_error = base_call_result_.at(template_segment).at(tile_id).at(ref_base).size();
                         dom_error--;) {
                        base_call_result_.at(template_segment)
                            ...
                            .ModifyPar0(
                                ref_base,
                                1.0 /
                                    error_multiplier); // Didive the probability of the correct base by error_multiplier
                    }
                }
            }
        }
