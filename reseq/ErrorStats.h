...
class ErrorStats {
  ...
  private:
    ...
    std::array<std::vector<utilities::VectorAtomic<uintFragCount>>, kTemplateSegments> tmp_errors_per_read_;
    std::array<std::array<std::array<std::array<std::vector<utilities::VectorAtomic<uintNucCount>>, 6>, kNumBasesN>,
                          kNumBases>,
               kTemplateSegments>
    ...
    std::array<std::array<Vect<Vect<uintNucCount>>, 6>, 2>
        indel_by_indel_pos_; // indel_by_indel_pos_[Insertion/DeletionBefore][PreviousRegularCall][NumInDelsDirectlyBefore][Nothing/Deletion/Insertion(A/C/G/T)]
    std::array<std::array<Vect<Vect<uintNucCount>>, 6>, 2>
        indel_by_position_; // indel_by_position_[Insertion/DeletionBefore][PreviousRegularCall][Position][Nothing/Deletion/Insertion(A/C/G/T)]
    std::array<std::array<Vect<Vect<uintNucCount>>, 6>, 2>
        indel_by_gc_; // indel_by_gc_[Insertion/DeletionBefore][PreviousRegularCall][GC][Nothing/Deletion/Insertion(A/C/G/T)]
    std::array<std::array<Vect<Vect<uintNucCount>>, 6>, 2>
        indel_pos_by_position_; // indel_pos_by_position_[Insertion/DeletionBefore][PreviousRegularCall][Position][NumInDelsDirectlyBefore]
    std::array<std::array<Vect<Vect<uintNucCount>>, 6>, 2>
        indel_pos_by_gc_; // indel_pos_by_gc_[Insertion/DeletionBefore][PreviousRegularCall][GC][NumInDelsDirectlyBefore]
    std::array<std::array<Vect<Vect<uintNucCount>>, 6>, 2>
    ...
    std::array<Vect<uintFragCount>, kTemplateSegments>
        errors_per_read_; // errors_per_read_[first/second][#errors] = #reads
    std::array<std::array<std::array<std::array<Vect<uintNucCount>, 6>, kNumBasesN>, kNumBases>, kTemplateSegments>
    ...
    std::array<std::array<std::array<Vect<uintNucCount>, kNumBasesN>, kNumBases>, kTemplateSegments>
    ...
    std::array<Vect<uintNucCount>, 2>
        indel_error_by_length_; // indel_error_by_length_[Insertion/Deletion][LengthOfInDel] = #indels
    std::array<Vect<uintNucCount>, 2>
        indel_error_by_position_; // indel_error_by_position_[Insertion/Deletion][ReadPosition] = #indelBases
  ...
  public:
    // Getter functions
    inline const Vect<Vect<uintNucCount>>& CalledBasesByBaseQuality(uintTempSeq template_segment, uintTileId tile_id,
                                                                    seqan::Dna5 ref_base, seqan::Dna5 dom_error) const {
        return called_bases_by_base_quality_per_tile_.at(template_segment).at(ref_base).at(dom_error).at(tile_id);
    }

    inline const Vect<Vect<uintNucCount>>& CalledBasesByPosition(uintTempSeq template_segment, uintTileId tile_id,
                                                                 seqan::Dna5 ref_base, seqan::Dna5 dom_error) const {
        return called_bases_by_position_per_tile_.at(template_segment).at(ref_base).at(dom_error).at(tile_id);
    }

    inline const Vect<Vect<uintNucCount>>& CalledBasesByErrorNum(uintTempSeq template_segment, uintTileId tile_id,
                                                                 seqan::Dna5 ref_base, seqan::Dna5 dom_error) const {
        return called_bases_by_error_num_per_tile_.at(template_segment).at(ref_base).at(dom_error).at(tile_id);
    }

    inline const Vect<Vect<uintNucCount>>& CalledBasesByErrorRate(uintTempSeq template_segment, uintTileId tile_id,
                                                                  seqan::Dna5 ref_base, seqan::Dna5 dom_error) const {
        return called_bases_by_error_rate_per_tile_.at(template_segment).at(ref_base).at(dom_error).at(tile_id);
    }

    inline const Vect<Vect<uintNucCount>>& ErrorNumByBaseQuality(uintTempSeq template_segment, uintTileId tile_id,
                                                                 seqan::Dna5 ref_base, seqan::Dna5 dom_error) const {
        return error_num_by_quality_per_tile_.at(template_segment).at(ref_base).at(dom_error).at(tile_id);
    }

    inline const Vect<Vect<uintNucCount>>& ErrorNumByPosition(uintTempSeq template_segment, uintTileId tile_id,
                                                              seqan::Dna5 ref_base, seqan::Dna5 dom_error) const {
        return error_num_by_position_per_tile_.at(template_segment).at(ref_base).at(dom_error).at(tile_id);
    }

    inline const Vect<Vect<uintNucCount>>& ErrorNumByErrorRate(uintTempSeq template_segment, uintTileId tile_id,
                                                               seqan::Dna5 ref_base, seqan::Dna5 dom_error) const {
    ...
    inline const Vect<uintNucCount>& CalledBasesByBaseQualityPerPreviousCalledBase(uintTempSeq template_segment,
                                                                                   uintBaseCall ref_base,
                                                                                   uintBaseCall called_base,
                                                                                   uintBaseCall previous_base) const {
        return called_bases_by_base_quality_per_previous_called_base_.at(template_segment)
            .at(ref_base)
            .at(called_base)
            .at(previous_base);
    }

    inline uintReadLen MaxLenDeletion() const { return max_len_deletion_; }

    inline const Vect<uintNucCount>& CalledBasesByBaseQuality(uintTempSeq template_segment, uintBaseCall ref_base,
                                                              uintBaseCall called_base) const {
        return called_bases_by_base_quality_.at(template_segment).at(ref_base).at(called_base);
    }
    inline const Vect<uintNucCount>& CalledBasesByPosition(uintTempSeq template_segment, uintBaseCall ref_base,
                                                           uintBaseCall called_base) const {
    ...
    // Setter functions
    inline void AddBase(uintTempSeq template_segment, uintBaseCall ref_base, uintBaseCall dom_error, uintTileId tile_id,
                        uintBaseCall called_base, uintQual quality, uintReadLen read_pos, uintReadLen num_errors,
                        uintPercent error_rate) {
        // For estimation
        ++tmp_called_bases_by_base_quality_per_tile_.at(template_segment)
              .at(ref_base)
              .at(dom_error)
              .at(tile_id)
              .at(called_base)
              .at(quality);
        ++tmp_called_bases_by_position_per_tile_.at(template_segment)
              .at(ref_base)
              .at(dom_error)
              .at(tile_id)
              .at(called_base)
              .at(read_pos);
        ++tmp_called_bases_by_error_num_per_tile_.at(template_segment)
              .at(ref_base)
              .at(dom_error)
              .at(tile_id)
              .at(called_base)
              .at(num_errors);
        ++tmp_called_bases_by_error_rate_per_tile_.at(template_segment)
              .at(ref_base)
              .at(dom_error)
              .at(tile_id)
              .at(called_base)
              .at(error_rate);
        ++tmp_error_num_by_quality_per_tile_.at(template_segment)
              .at(ref_base)
              .at(dom_error)
              .at(tile_id)
              .at(num_errors)
              .at(quality);
        ++tmp_error_num_by_position_per_tile_.at(template_segment)
              .at(ref_base)
              .at(dom_error)
              .at(tile_id)
              .at(num_errors)
              .at(read_pos);
        ++tmp_error_num_by_error_rate_per_tile_.at(template_segment)
              .at(ref_base)
              .at(dom_error)
              .at(tile_id)
              .at(num_errors)
    ...
    inline void AddBasePlotting(uintTempSeq template_segment, uintBaseCall ref_base, uintBaseCall called_base,
                                uintQual quality, uintBaseCall last_called_base) {
        // For plotting
