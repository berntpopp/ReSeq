...
class FragmentDuplicationStats {
  ...
  private:
    ...
    std::vector<utilities::VectorAtomic<uintFragCount>>
        tmp_duplication_number_; // tmp_duplication_number_[#FragmentsWithSamePosition] = #FragmentSites
  ...
  public:
    ...
    // Main functions
    inline void PrepareTmpDuplicationVector() { tmp_duplication_number_.resize(kMaxDuplication + 2); }
