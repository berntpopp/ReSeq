...
void DataStatsTest::LoadStats(const string& stats_file, bool ignore_tiles, bool calculate_biases, const string adapter,
    ...
    ASSERT_TRUE(test_->ReadBam(stats_file.c_str(), (adapter_dir + adapter + ".fa").c_str(),
                               (adapter_dir + adapter + ".mat").c_str(), variants, 100000000, 1, calculate_biases));
...
void DataStatsTest::TestSequenceContentReference(uintTempSeq template_segment, uintTempSeq strand, uintReadLen at_pos,
                                                 uintNucCount cont_a, uintNucCount cont_c, uintNucCount cont_g,
                                                 uintNucCount cont_t, const char* context) {
    EXPECT_EQ(cont_a, test_->sequence_content_reference_.at(template_segment).at(strand).at(0)[at_pos])
        << "sequence_content_reference_[" << static_cast<uintTempSeqPrint>(template_segment) << "]["
        << static_cast<uintTempSeqPrint>(strand) << "] position " << at_pos << " wrong for " << context << '\n';
    EXPECT_EQ(cont_c, test_->sequence_content_reference_.at(template_segment).at(strand).at(1)[at_pos])
        << "sequence_content_reference_[" << static_cast<uintTempSeqPrint>(template_segment) << "]["
        << static_cast<uintTempSeqPrint>(strand) << "] position " << at_pos << " wrong for " << context << '\n';
    EXPECT_EQ(cont_g, test_->sequence_content_reference_.at(template_segment).at(strand).at(2)[at_pos])
        << "sequence_content_reference_[" << static_cast<uintTempSeqPrint>(template_segment) << "]["
        << static_cast<uintTempSeqPrint>(strand) << "] position " << at_pos << " wrong for " << context << '\n';
    EXPECT_EQ(cont_t, test_->sequence_content_reference_.at(template_segment).at(strand).at(3)[at_pos])
        << "sequence_content_reference_[" << static_cast<uintTempSeqPrint>(template_segment) << "]["
        << static_cast<uintTempSeqPrint>(strand) << "] position " << at_pos << " wrong for " << context << '\n';
}
...
void DataStatsTest::TestSrr490124Equality(const char* context, bool test_tile_information, bool bwa) {
    ...
    EXPECT_TRUE(test_->reference_->ReferenceIdFirstPart(0) == "NC_000913.3")
        << test_->reference_->ReferenceIdFirstPart(0) << " wrong for SRR490124-4pairs reference_ for " << context
        << '\n';
    ...
    for (int templ_seg = 2; templ_seg--;) {
        EXPECT_EQ(1, test_->read_lengths_.at(templ_seg).size())
            << "SRR490124-4pairs read_lengths_[" << templ_seg << "] wrong for " << context << '\n';
        EXPECT_EQ(18, test_->read_lengths_.at(templ_seg)[100])
            << "SRR490124-4pairs read_lengths_[" << templ_seg << "] wrong for " << context << '\n';
    }
    ...
    // awk '(0==NR%2){print gsub("G","",$0)+gsub("C","",$0)}'
    EXPECT_EQ(4, test_->gc_read_content_reference_.at(0).size())
        << "SRR490124-4pairs gc_read_content_reference_[0] wrong for " << context << '\n';
    EXPECT_EQ(1, test_->gc_read_content_reference_.at(0)[54])
        << "SRR490124-4pairs gc_read_content_reference_[0] wrong for " << context << '\n';
    EXPECT_EQ(8, test_->gc_read_content_reference_.at(0)[57])
        << "SRR490124-4pairs gc_read_content_reference_[0] wrong for " << context << '\n';
    EXPECT_EQ(7, test_->gc_read_content_reference_.at(1).size())
        << "SRR490124-4pairs gc_read_content_reference_[1] wrong for " << context << '\n';
    EXPECT_EQ(8, test_->gc_read_content_reference_.at(1)[46])
        << "SRR490124-4pairs gc_read_content_reference_[1] wrong for " << context << '\n';
    EXPECT_EQ(1, test_->gc_read_content_reference_.at(1)[52])
        << "SRR490124-4pairs gc_read_content_reference_[1] wrong for " << context << '\n';
    // samtools view -q 10 ecoli-SRR490124-4pairs.sam | awk '{print int($2%256/128), gsub("G","",$10)+gsub("C","",$10)}'
    // | sort -n
    EXPECT_EQ(7, test_->gc_read_content_mapped_.at(0).size())
        << "SRR490124-4pairs gc_read_content_mapped_[0] wrong for " << context << '\n';
    EXPECT_EQ(1, test_->gc_read_content_mapped_.at(0)[53])
        << "SRR490124-4pairs gc_read_content_mapped_[0] wrong for " << context << '\n';
    EXPECT_EQ(8, test_->gc_read_content_mapped_.at(0)[59])
        << "SRR490124-4pairs gc_read_content_mapped_[0] wrong for " << context << '\n';
    ...
    // '(0==NR%2){print substr($0,1,1)}' | sort | uniq -c
    ...
    // samtools view -q 10 ecoli-SRR490124-4pairs.sam | awk '(0==int($2%256/128)){print $4}' | sort -n | awk
    // '{system("samtools faidx ecoli-GCF_000005845.2_ASM584v2_genomic.fa NC_000913.3:" $1 "-" $1+99)}' | seqtk seq -r |
    // awk '(0==NR%2){print substr($0,1,1)}' | sort | uniq -c
    ...
    // samtools view -q 10 ecoli-SRR490124-4pairs.sam | awk '(1==int($2%256/128)){print $4}' | sort -n | awk
    // '{system("samtools faidx ecoli-GCF_000005845.2_ASM584v2_genomic.fa NC_000913.3:" $1 "-" $1+99)}' | seqtk seq |
    // awk '(0==NR%2){print substr($0,1,1)}' | sort | uniq -c
    ...
    for (int base = 4; base--;) {
        EXPECT_EQ(0, test_->sequence_content_reference_.at(0).at(0).at(base).size())
            << "SRR490124-4pairs sequence_content_reference_[0][0][" << base << "].size() not correct for " << context
            << '\n';
        EXPECT_EQ(0, test_->sequence_content_reference_.at(1).at(1).at(base).size())
            << "SRR490124-4pairs sequence_content_reference_[1][1][" << base << "].size() not correct for " << context
            << '\n';
    }
...
void DataStatsTest::TestDuplicates() {
    ...
    // ecoli-GCF_000005845.2_ASM584v2_genomic.fa NC_000913.3:" $4 "-" $4+length($10)-1)}' | awk
    // '(0==NR%2){for(i=1;i<=length($1);i+=1){print i-1, substr($1,i,1)}}' | awk '($1 < 10 && $1 ~ /^[4-7]/)' | sort
    // -k1,1n -k2,2 | uniq -c
    ...
    // samtools view -q 10 -f 83 ecoli-duplicates.bam | awk '(0 != substr($1,12,1)){system("samtools faidx
    // ecoli-GCF_000005845.2_ASM584v2_genomic.fa NC_000913.3:" $4 "-" $4+length($10)-1)}' | seqtk seq -r | awk
    // '(0==NR%2){print substr($0,5,1)}' | sort | uniq -c
...
void DataStatsTest::TestCrossDuplicates() {
    ...
    EXPECT_TRUE(test_->reference_->ReferenceIdFirstPart(0) == "NW_007931110.1")
        << test_->reference_->ReferenceIdFirstPart(0) << " wrong for reference_ in cross duplicates test\n";
    EXPECT_TRUE(test_->reference_->ReferenceIdFirstPart(1) == "NW_007931111.1")
...
void DataStatsTest::TestAdapters(const char* context, bool bwa) {
    ...
    if (bwa) { // Additional read-pair is mapping: SRR490124.7718805_1706:3:48:3040:12095_adapter_len20_low_quality
        // samtools view ecoli-SRR490124-adapter-bwa.bam -q 10 -f 81 -F 32 | awk '{print $6; system("samtools faidx
        // ecoli-GCF_000005845.2_ASM584v2_genomic.fa NC_000913.3:" $8 "-" $8-$9-1)}' | awk 'BEGIN{last=""}{if(">" ==
        // substr($0,1,1)){print ">" last; last=""}else{print last; last = $0}}END{print last}' | seqtk seq -r | awk
        // '(1==NR%2){cigar = substr($0,2,length($0)-1)}(0==NR%2){pos=1; print_pos=0; num=0;
        // for(i=1;i<=length(cigar);i+=1){e=substr(cigar,i,1);if(e ~ /^[0-9]/){num=num*10+e}else{if("M"==e ||
        // "S"==e){while(0<num && pos<=length($0)){print print_pos, substr($0,pos,1); ++print_pos; ++pos;
        // --num}};if("D"==e){pos+=num};if("I"==e){print_pos+=num};num=0}}}' | sort -k2,2 -k1,1n | uniq -c
        ...
        // samtools view ecoli-SRR490124-adapter.bam -q 10 -f 81 -F 32 | awk '{print $6; system("samtools faidx
        // ecoli-GCF_000005845.2_ASM584v2_genomic.fa NC_000913.3:" $8 "-" $4+length($10)-1)}' | awk
        // 'BEGIN{last=""}{if(">" == substr($0,1,1)){print ">" last; last=""}else{print last; last = $0}}END{print
        // last}' | seqtk seq -r | awk '(1==NR%2){cigar = substr($0,2,length($0)-1)}(0==NR%2){pos=1; print_pos=0; num=0;
        // for(i=1;i<=length(cigar);i+=1){e=substr(cigar,i,1);if(e ~ /^[0-9]/){num=num*10+e}else{if("M"==e){while(0<num
        // && pos<=length($0)){print print_pos, substr($0,pos,1); ++print_pos; ++pos;
        // --num}};if("D"==e){pos+=num};if("I"==e){print_pos+=num};num=0}}}' | sort -k2,2 -k1,1n | uniq -c
    ...
    if (bwa) { // Additional read-pair is mapping: SRR490124.7718805_1706:3:48:3040:12095_adapter_len20_low_quality
        // samtools view ecoli-SRR490124-adapter-bwa.bam -q 10 -f 161 -F 16 | awk '{print $6; system("samtools faidx
        // ecoli-GCF_000005845.2_ASM584v2_genomic.fa NC_000913.3:" $4 "-" $4+$9-1)}' | awk 'BEGIN{last=""}{if(">" ==
        // substr($0,1,1)){print ">" last; last=""}else{print last; last = $0}}END{print last}' | seqtk seq | awk
        // '(1==NR%2){cigar = substr($0,2,length($0)-1)}(0==NR%2){pos=1; print_pos=0; num=0;
        // for(i=1;i<=length(cigar);i+=1){e=substr(cigar,i,1);if(e ~ /^[0-9]/){num=num*10+e}else{if("M"==e ||
        // "S"==e){while(0<num && pos<=length($0)){print print_pos, substr($0,pos,1); ++print_pos; ++pos;
        // --num}};if("D"==e){pos+=num};if("I"==e){print_pos+=num};num=0}}}' | sort -k2,2 -k1,1n | uniq -c
        ...
        // samtools view ecoli-SRR490124-adapter.bam -q 10 -f 161 -F 16 | awk '{print $6; system("samtools faidx
        // ecoli-GCF_000005845.2_ASM584v2_genomic.fa NC_000913.3:" $4 "-" $8+length($10)-1)}' | awk
        // 'BEGIN{last=""}{if(">" == substr($0,1,1)){print ">" last; last=""}else{print last; last = $0}}END{print
        // last}' | seqtk seq | awk '(1==NR%2){cigar = substr($0,2,length($0)-1)}(0==NR%2){pos=1; print_pos=0; num=0;
        // for(i=1;i<=length(cigar);i+=1){e=substr(cigar,i,1);if(e ~ /^[0-9]/){num=num*10+e}else{if("M"==e){while(0<num
        // && pos<=length($0)){print print_pos, substr($0,pos,1); ++print_pos; ++pos;
        // --num}};if("D"==e){pos+=num};if("I"==e){print_pos+=num};num=0}}}' | sort -k2,2 -k1,1n | uniq -c
...
TEST_F(DataStatsTest, Ecoli) {
    ...
    // zcat SRR490124_1.fastq.gz | head -n8 | awk '{if(1==NR%4 || 3==NR%4){print $1 "c", $2, $3}else{print $0}}' >>
    // ecoli-SRR490124-4pairs-R1.fq zcat SRR490124_1.fastq.gz | head -n8 | awk '{if(1==NR%4 || 3==NR%4){print $1 "c2",
    // $2, $3}else{print $0}}' >> ecoli-SRR490124-4pairs-R1.fq
    // ...
    // zcat SRR490124_1.fastq.gz | head -n8 | awk '{if(1==NR%4 || 3==NR%4){print $1 "c7", $2, $3}else{print $0}}' >>
    // ecoli-SRR490124-4pairs-R1.fq bowtie2 -X 2000 -x ecoli-GCF_000005845.2_ASM584v2_genomic.fa -1
    // <(reseq-prepare-names.py ecoli-SRR490124-4pairs-R1.fq ecoli-SRR490124-4pairs-R2.fq) -2 <(reseq-prepare-names.py
    // ecoli-SRR490124-4pairs-R2.fq ecoli-SRR490124-4pairs-R1.fq) | samtools sort -m 10G -@ 4 -T _tmp -o
    // ecoli-SRR490124-4pairs.sam -
    ...
    // ecoli-SRR490124-4pairs-R2.fq) <(reseq-prepare-names.py ecoli-SRR490124-4pairs-R2.fq ecoli-SRR490124-4pairs-R1.fq)
    // | samtools sort -m 10G -@ 4 -T _tmp -o ecoli-SRR490124-4pairs-bwa.bam -
    ...
    DeleteTestObject();
    CreateTestObject(&species_reference_);
    // bowtie2 -X 2000 -x ecoli-GCF_000005845.2_ASM584v2_genomic.fa -1 ecoli-duplicates-R1.fq -2 ecoli-duplicates-R2.fq
    // | samtools sort -m 10G -@ 4 -T _tmp -o ecoli-duplicates.bam -
    LoadStats(test_dir + "ecoli-duplicates.bam", true);
    TestDuplicates();

    DeleteTestObject();
    CreateTestObject(&species_reference_);
    LoadStats(test_dir + "ecoli-SRR490124-4pairs.bam", false, false, "TruSeq_single",
              test_dir + "ecoli-SRR490124-4pairs.vcf");
    TestVariants();
}
...
TEST_F(DataStatsTest, Drosophila) {
    ...
    // bowtie2 -X 2000 -x drosophila-GCF_000001215.4_cut.fna -1 drosophila-crossDuplicates-R1.fq -2
    // drosophila-crossDuplicates-R2.fq | samtools sort -m 10G -@ 4 -T _tmp -o drosophila-crossDuplicates.bam -
    ...
    // samtools faidx drosophila-GCF_000001215.4_cut.fna NW_007931113.1 | seqtk seq | awk '(0==NR%2){print length($0)}'
    // | awk '{for(pos=1;pos<=$0-150;++pos){system("samtools faidx drosophila-GCF_000001215.4_cut.fna NW_007931113.1:"
    // pos "-" pos+49 " NW_007931113.1:" pos+100 "-" pos+149)}}' | awk '(1==NR%2){print $0}(0==NR%2){system("seqtk seq
    // drosophila-GCF_000001215.4_cut.fna | grep -i -o " $0 " | wc -l")}' | awk
    // '(1==NR%4){head1=$0}(2==NR%4){num1=$0}(3==NR%4){head2=$0}(0==NR%4 && 1==num1 && 1==$0){print head1, head2, num1,
    // $0}' bowtie2 -X 2000 -x drosophila-GCF_000001215.4_cut.fna -1 drosophila-coverageTest-R1.fq -2
    // drosophila-coverageTest-R2.fq | samtools sort -m 10G -@ 4 -T _tmp -o drosophila-coverageTest.bam -
...
TEST_F(DataStatsTest, Adapter) {
    ...
    // samtools view -q 10  ../mtest/ecoli-SRR490124-4x.bam | awk '(int($2%32/16) && $8>$4 && $8<$4+200){len=0;
    // for(i=1;i<=length($6);i+=1){e=substr($6,i,1);if(e ~ /^[0-9]/){num=num*10+e}else{if("M"==e || "D"==e){len+=num};
    // num=0}}; print $4-$8+len, $1}' | sort -n | head zcat ../../../ecoli/data/SRR490124_1.fastq.gz | awk
    // '{if(1==NR%4){if($1=="@SRR490124.10672007" || $1=="@SRR490124.2703087" || $1=="@SRR490124.7718805" ||
    // $1=="@SRR490124.7353367" || $1=="@SRR490124.3382600" || $1=="@SRR490124.8368120" || $1=="@SRR490124.10415918" ||
    // $1=="@SRR490124.110795" || $1=="@SRR490124.202918"){print $0;pri=1}else{pri=0}}else{if(pri){print $0}}}' >
    // ecoli-SRR490124-adapter-R1.fq Manually added additional information in id and inserted _ to keep it after mapping
    // bowtie2 -X 2000 -x ecoli-GCF_000005845.2_ASM584v2_genomic.fa -1 ecoli-SRR490124-adapter-R1.fq -2
    // ecoli-SRR490124-adapter-R2.fq | samtools sort -m 10G -@ 4 -T _tmp -o ecoli-SRR490124-adapter.bam -
    ...
    DeleteTestObject();
    CreateTestObject(&species_reference_);
    // bwa mem ecoli-GCF_000005845.2_ASM584v2_genomic.fa ecoli-SRR490124-adapter-R1.fq ecoli-SRR490124-adapter-R2.fq |
    // samtools sort -m 10G -@ 4 -T _tmp -o ecoli-SRR490124-adapter-bwa.bam -
...
TEST_F(DataStatsTest, NexteraAdapter) {
    ...
    // zcat Nextera-Repeat-600pM-2x151-DI/Nextera_L1/Ecoli1_L001_S5_L001_R1_001.fastq.gz | awk '(2==NR%4){print NR, $0}'
    // | grep --color=always 'CCGAGCCCACGAGAC' | head -n20 zcat
    // Nextera-Repeat-600pM-2x151-DI/Nextera_L1/Ecoli1_L001_S5_L001_R1_001.fastq.gz | awk '(NR>=25)' | head -n4 >
    // ecoli-S1L001-adapters-R1.fq zcat Nextera-Repeat-600pM-2x151-DI/Nextera_L1/Ecoli1_L001_S5_L001_R1_001.fastq.gz |
    // awk '(NR>=33)' | head -n4 >> ecoli-S1L001-adapters-R1.fq zcat
    // Nextera-Repeat-600pM-2x151-DI/Nextera_L1/Ecoli1_L001_S5_L001_R1_001.fastq.gz | awk '(NR>=129)' | head -n4 >>
    // ecoli-S1L001-adapters-R1.fq zcat Nextera-Repeat-600pM-2x151-DI/Nextera_L1/Ecoli1_L001_S5_L001_R2_001.fastq.gz |
    // awk '(NR>=25)' | head -n4 > ecoli-S1L001-adapters-R2.fq zcat
    // Nextera-Repeat-600pM-2x151-DI/Nextera_L1/Ecoli1_L001_S5_L001_R2_001.fastq.gz | awk '(NR>=33)' | head -n4 >>
    // ecoli-S1L001-adapters-R2.fq zcat Nextera-Repeat-600pM-2x151-DI/Nextera_L1/Ecoli1_L001_S5_L001_R2_001.fastq.gz |
    // awk '(NR>=129)' | head -n4 >> ecoli-S1L001-adapters-R2.fq bowtie2 -X 2000 -x
    // ecoli-GCF_000005845.2_ASM584v2_genomic.fa -1 ecoli-S1L001-adapters-R1.fq -2 ecoli-S1L001-adapters-R2.fq |
    // samtools sort -m 10G -@ 4 -T _tmp -o ecoli-S1L001-adapters.bam -
