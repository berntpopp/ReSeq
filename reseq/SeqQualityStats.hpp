...
template <typename T> class SeqQualityStats {
  private:
    ...
    template <class Archive> void serialize(Archive& ar, const unsigned int UNUSED(version)) { ar & qualities_; }
  ...
  public:
    ...
    SeqQualityStats()
        : mean_(0), minimum_(0), first_quartile_(0), median_(0), third_quartile_(0), maximum_(0), probability_mean_(0) {
    }
    SeqQualityStats(decltype(qualities_.capacity()) reserved) : SeqQualityStats() { qualities_.reserve(reserved); }
    ...
    const std::pair<typename std::vector<T>::size_type, typename std::vector<T>>& stdQualities() const {
        return qualities_.std();
    }
    ...
    void Calculate(bool all = true) {
        ...
        if (0 == num_values) {
            ...
            third_quartile_ = 0;
            maximum_ = 0;
        } else if (1 == num_values) { // Quartiles not defined as median-excluded list is empty: Set them to the only
                                      // existing element
            mean_ = sum;
            minimum_ = sum;
            first_quartile_ = sum;
...
template <typename T>
bool operator==(decltype(SeqQualityStats<T>().size()) val,
                const SeqQualityStats<T>& stats) { // Compares the size_t value to the quality vector
