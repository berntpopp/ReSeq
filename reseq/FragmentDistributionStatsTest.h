...
class FragmentDistributionStatsTest : public BasicTestClassWithReference {
  ...
  protected:
    ...
    static void TestOutskirtContent(const FragmentDistributionStats& test, uintTempSeq template_segment,
                                    uintSeqLen at_pos, uintNucCount cont_a, uintNucCount cont_c, uintNucCount cont_g,
                                    uintNucCount cont_t, const char* context);

    virtual void TearDown();

    void CreateCoverageDataHelper(uintPercent gc_perc, uintSeqLen start_pos, uintSeqLen fragment_length,
                                  std::mt19937_64& rgen);
    void CreateCoverageData(uintSeqLen fragment_length);

    template <size_t N>
    void CheckDrawnCounts(double bias, std::array<double, N> thresholds, const Surrounding& start_sur,
