...
class TileStats {
  private:
    // Definitions
    const uint16_t kReadIdBufferSize =
        200; // Size of the buffer used for parsing the read id; Has to be long enough to contain everything until the
             // first colon in read id; Minimum 70 so some generated error messages fit
    ...
    uint16_t tile_colon_number_; // The count at which colon beginning from the left the tile number is starting (and
                                 // then goes to the next colon)
    bool tile_accessible_;       // Whether the read id can be parsed to extract the tile

    // Helper functions
    inline void ReadUntil(std::stringstream& id_stream, char* buffer, uint16_t buffer_size, char until,
                          const std::string& field) const;
    inline void CheckDelimiter(char delimiter, char expected_delimiter, const std::string& occurence) const;
    inline void CheckDelimiter(std::stringstream& id_stream, char expected_delimiter,
                               const std::string& occurence) const;
    inline uint32_t ReadInt(std::stringstream& id_stream, const std::string& field,
                            bool read_must_continue = true) const;

    uintTile GetKnownTile(const seqan::CharString& read_id, uint16_t tile_colon_number, bool print_warnings,
                          std::stringstream* error_message = NULL) const;
  ...
  public:
    ...
    EnterTile(const seqan::CharString& read_id,
              std::stringstream* error_message = NULL); // Reads the tile from the read id and returns its tile id. In
