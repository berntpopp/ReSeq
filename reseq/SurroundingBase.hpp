...
template <uintSurBlockId N, uintSurPos R, int16_t S, typename I> class SurroundingBase {
  ...
  public:
    ...
    static uintSeqLen StartPos(uintSeqLen pos, uintSeqLen seq_len) { return pos + seq_len - kStartPos; }
    static uintSeqLen EndPos(uintSeqLen pos, uintSeqLen seq_len) { return StartPos(pos, seq_len) + Length(); }

    static uintSeqLen StartPosReverse(uintSeqLen pos, uintSeqLen seq_len) { return pos + seq_len + kStartPos; }
  ...
  protected:
    ...
    template <typename T> void Set(const T& sequence, uintSeqLen pos) {
        for (auto block = 0; block < kNumBlocks; ++block) {
            // Add sequence length to avoid negative positions if block reaches over lower sequence ends so it wraps
            // around and start at the end The higher sequence end is covered by the called function
    ...
    void UpdateWithN(const seqan::Dna5String& sequence, uintSeqLen new_pos, bool forward) {
        ...
        for (auto block = kNumBlocks; block--;) {
            if (forward) {
                ...
                new_base = utilities::Complement::Dna5(
                    utilities::at(sequence, BlockStartPosReverse(new_pos, length(sequence), block) % length(sequence)));
            }
            ...
            if (utilities::IsN(new_base)) {
                ...
                if (-1 == sur_.at(block)) {
                    ...
                    if (forward) {
                        ...
                        sur_.at(block) =
                            GetBlock(utilities::ConstDna5StringReverseComplement(sequence),
                                     BlockStartPos(length(sequence) - new_pos - 1, length(sequence), block));
                    }
                } else {
                    ++(sur_.at(block));
                }
  ...
  public:
    SurroundingBase() { sur_.fill(-1); }

    SurroundingBase(const SurroundingBase& origin) { sur_ = origin.sur_; }
    ...
    void Forward(const seqan::Dna5String& sequence, uintSeqLen pos) { Set(sequence, pos); }
    ...
    void ReverseWithN(const seqan::Dna5String& sequence, uintSeqLen pos) {
        for (auto block = 0; block < kNumBlocks; ++block) {
            if (!BlockHasN(sur_.at(block), sequence, BlockLastPosReverse(pos, length(sequence), block))) {
                sur_.at(block) = GetBlock(utilities::ConstDna5StringReverseComplement(sequence),
                                          BlockStartPos(length(sequence) - pos - 1, length(sequence), block));
            }
        }
    }
    ...
    inline void UpdateReverse(const seqan::Dna5String& sequence, uintSeqLen new_pos) {
        UpdateOnLeftSide(utilities::Complement::Dna5(
            utilities::at(sequence, (StartPosReverse(new_pos, length(sequence))) % length(sequence))));
    }

    inline void RollBackReverse(const seqan::Dna5String& sequence, uintSeqLen new_pos) {
        UpdateOnRightSide(utilities::Complement::Dna5(
            utilities::at(sequence, (EndPosReverse(new_pos, length(sequence)) + 1) % length(sequence))));
    }
