...
class Simulator {
  private:
    ...
    class VariantBiasVarModifiers {
      public:
        ...
        // GC modification
        std::vector<intSeqShift>
            gc_mod_; // Change in gc for that allele compared to the reference for the given fragment
        ...
        VariantBiasVarModifiers(intVariantId first_variant_id, uintAlleleId num_alleles)
            : last_gc_(0), last_gc_end_(0), first_variant_id_(first_variant_id), start_variant_pos_(0) {
        ...
        std::pair<intVariantId, uintSeqLen> StartVariant() const { return {first_variant_id_, start_variant_pos_}; }
        std::pair<intVariantId, uintSeqLen> EndVariant(const std::vector<Reference::Variant>& variants,
                                                       uintSeqLen cur_end_position, uintAlleleId allele) const {
            if (unhandled_bases_in_variant_.at(allele)) {
                return {unhandled_variant_id_.at(allele),
                        length(variants.at(unhandled_variant_id_.at(allele)).var_seq_) -
                            unhandled_bases_in_variant_.at(allele)};
            } else if (start_variant_pos_ &&
                       variants.at(first_variant_id_).position_ == cur_end_position - end_pos_shift_.at(allele) - 1) {
    ...
    class SysErrorVariant {
      public:
        ...
        SysErrorVariant(uintSeqLen position, const std::vector<std::pair<seqan::Dna5, uintPercent>>& var_errors,
                        const std::array<uintAlleleBitArray, reseq::Reference::Variant::kMaxAlleles / 64>& allele)
            : position_(position), var_errors_(var_errors), allele_(allele) {}
    ...
    class GeneralRandomDistributions {
      ...
      public:
        GeneralRandomDistributions(const DataStats& stats)
            : stats_(stats), tile_(stats.Tiles().Abundance().begin(), stats.Tiles().Abundance().end()),
            ...
            for (uintTempSeq template_segment = 2; template_segment--;) {
                ...
                read_length_.at(template_segment) = std::discrete_distribution<uintReadLen>(
                    stats.ReadLengths(template_segment).begin(), stats.ReadLengths(template_segment).end());
            }
        }
        ...
        uintSeqLen Adapter(uintTempSeq template_segment, std::mt19937_64& rgen) {
            return adapter_.at(template_segment)(rgen);
        }
        uintReadLen PolyATailLength(std::mt19937_64& rgen) {
            return polya_tail_length_(rgen) + stats_.Adapters().PolyATailLength().from();
        }
        ...
        uintReadLen ReadLength(uintTempSeq template_segment, uintSeqLen fragment_length, std::mt19937_64& rgen) {
            ...
            while (counter <= random_value &&
                   (read_len-- > stats_.ReadLengthsByFragmentLength(template_segment).at(fragment_length).from())) {
    ...
    const uintFragCount kBatchSize =
        100000; // 100,000 reads are written in each batch, balance between speed and memory usage
    const uintSeqLen kBlockSize = 1000; // Continuous bases that are handled by a single thread; too high: work load
                                        // isn't shared properly, memory allocation of big arrays slows program down;
                                        // too low: overhead like random generator seeding slows program down
    ...
    std::vector<std::vector<std::array<double, 2>>>
        non_zero_thresholds_; // non_zero_thresholds_[CoverageGroup][FragmentLength][oneAllele/allAlleles] = Threshold
                              // that random number from fragment generation has to reach or fragment count will be zero
    ...
    static uintFragCount CoverageToNumberPairs(double coverage, uintRefLenCalc total_ref_size,
    ...
    bool FlushWriteValues(uintTempSeq template_segment, seqan::StringSet<seqan::CharString>* old_output_ids,
                          ...
                          seqan::StringSet<seqan::CharString>* old_output_quals);
    bool Flush();
    bool WriteSingleReads(uintFragCount cur_block, seqan::StringSet<seqan::CharString>& old_output_ids,
                          seqan::StringSet<seqan::Dna5String>& old_output_seqs,
                          seqan::StringSet<seqan::CharString>& old_output_quals);
    bool Output(const SimPair& sim_reads);

    void ReadSystematicErrors(std::vector<std::pair<seqan::Dna5, uintPercent>>& sys_errors, uintSeqLen start_pos,
                              uintSeqLen end_pos) {
        uintPercent error_rate;
        for (auto pos = start_pos; pos < end_pos; ++pos) {
            error_rate = utilities::at(sys_err_perc_, pos) - 33;
            if (86 < error_rate) {
                error_rate += error_rate - 86; // Decompress percentages again (odd percentages over 86 are removed, as
                                               // fastq can store only up to 94 quality values)
            }
            sys_errors.emplace_back(utilities::at(sys_dom_err_, pos), error_rate);
        }
    }

    inline uintPercent DrawSystematicError(std::vector<std::pair<seqan::Dna5, uintPercent>>& sys_errors,
                                           seqan::Dna5 ref_base, seqan::Dna5 last_base, seqan::Dna5 dom_base,
                                           uintPercent gc_percent, uintSeqLen start_dist_error_region,
                                           uintPercent start_rate, const ProbabilityEstimates& estimates) {
        double prob_sum;

        seqan::Dna5 dom_error =
            estimates.DominantError(ref_base, last_base, dom_base)
                .Draw(tmp_probabilities_, prob_sum, {start_dist_error_region, gc_percent, start_rate},
                      rdist_zero_to_one_(block_seed_gen_));
        if (0.0 == prob_sum) {
            dom_error = 4;
        }

        uintPercent error_rate =
            estimates.ErrorRate(ref_base, dom_error)
                .Draw(tmp_probabilities_, prob_sum, {start_dist_error_region, gc_percent, start_rate},
                      rdist_zero_to_one_(block_seed_gen_));
        if (0.0 == prob_sum) {
            error_rate = 0;
    ...
    template <typename T>
    void SetSystematicErrors(std::vector<std::pair<seqan::Dna5, uintPercent>>& sys_errors, const T& sequence,
                             uintSeqLen start_pos, uintSeqLen end_pos, const DataStats& stats,
        ...
        for (auto pos = start_pos; pos < end_pos; ++pos) {
            ...
            error_rate = DrawSystematicError(
                sys_errors, ref_base, sys_last_base_, sys_dom_base_.Get(),
                utilities::SafePercent(sys_gc_, sys_gc_bases_),
                utilities::TransformDistanceToStartOfErrorRegion(distance_to_start_of_error_region_), start_error_rate_,
                estimates);
    ...
    void GetSysErrorFromBlock(seqan::Dna5& dom_error, uintPercent& error_rate, const SimBlock*& block,
                              uintSeqLen& block_pos, intVariantId& cur_var, uintSeqLen& var_pos, uintAlleleId allele);
    bool FillReadPart(SimRead& sim_read, uintTempSeq template_segment, uintTileId tile_id,
                      const seqan::DnaString& org_sequence, uintReadLen org_pos, char base_cigar_element,
                      const SimBlock* block, uintSeqLen block_pos, uintAdapterId adapter_id, ReadFillParameter& par,
                      std::pair<intVariantId, uintSeqLen> first_variant, uintAlleleId allele, const DataStats& stats,
                      const ProbabilityEstimates& estimates, GeneralRandomDistributions& rdist, std::mt19937_64& rgen);
    ...
    void CreateReadId(seqan::CharString& id, uintRefSeqBin block_number, uintFragCount read_number,
                      uintSeqLen start_pos, uintSeqLen end_pos, uintAlleleId allele, uintTile tile,
                      uintRefSeqId ref_seq_id, const Reference& ref, const utilities::CigarString& cigar,
                      uintReadLen num_errors);
    bool CreateReads(const Reference& ref, const DataStats& stats, const ProbabilityEstimates& estimates,
                     GeneralRandomDistributions& rdist, SimPair& sim_reads, std::mt19937_64& rgen, uintFragCount counts,
                     bool strand, uintAlleleId allele, uintRefSeqId ref_seq_id, uintSeqLen fragment_length,
                     uintFragCount& read_number, const SimBlock* start_block, uintSeqLen start_position_forward,
                     uintSeqLen end_position_forward, std::pair<intVariantId, uintSeqLen> start_variant = {0, 0},
                     std::pair<intVariantId, uintSeqLen> end_variant = {0, 0});
    ...
    void SetSystematicErrorVariantsReverse(uintSeqLen& start_dist_error_region, uintPercent& start_rate,
    ...
    inline bool AlleleSkipped(const VariantBiasVarModifiers& bias_mod, uintAlleleId allele,
        ...
        if (bias_mod.first_variant_id_ < variants.size() &&
            variants.at(bias_mod.first_variant_id_).position_ == cur_start_position) {
    ...
    void GetPossibleAlleles(std::vector<uintAlleleId>& possible_alleles, const Reference& ref,
    ...
    inline bool ProbabilityAboveThreshold(double probability_chosen, uintRefSeqId ref_seq,
                                          uintSeqLen fragment_length) const {
        return probability_chosen >= non_zero_thresholds_.at(coverage_groups_.at(ref_seq)).at(fragment_length).at(1);
    }
    void SelectAllele(std::vector<uintAlleleId>& chosen_allele_ids, std::vector<bool>& reverse_selection,
                      uintAlleleId possible_strands, double random_value) const;
    void DrawNAlleles(std::vector<uintAlleleId>& chosen_allele_ids, std::vector<bool>& reverse_selection,
                      uintAlleleId non_zero_strands, uintAlleleId possible_strands, GeneralRandomDistributions& rdist,
                      std::mt19937_64& rgen) const;
    void ReverseSelection(std::vector<uintAlleleId>& chosen_allele_ids, std::vector<bool>& reverse_selection,
    ...
    void ChooseAlleles(std::vector<uintAlleleId>& chosen_allele_ids, std::vector<bool>& reverse_selection,
                       uintAlleleId non_zero_strands, uintAlleleId possible_strands, GeneralRandomDistributions& rdist,
                       std::mt19937_64& rgen) const;
    inline bool VariantInsideCurrentFragment(intVariantId cur_var_id, uintSeqLen cur_end_position,
                                             intSeqShift end_pos_shift, uintRefSeqId ref_seq_id,
                                             const Reference& ref) const;
    void HandleGCModAndEndPosShiftForNewVariants(VariantBiasVarModifiers& bias_mod, uintAlleleId allele,
                                                 uintRefSeqId ref_seq_id, const Reference& ref,
                                                 uintSeqLen cur_end_position) const;
    void HandleSurroundingVariantsBeforeCenter(Surrounding& mod_surrounding, uintSeqLen center_position,
                                               intSeqShift initial_pos_shift, intVariantId center_var,
                                               uintAlleleId allele, uintRefSeqId ref_seq_id, const Reference& ref,
                                               bool reverse) const;
    void HandleSurroundingVariantsAfterCenter(Surrounding& mod_surrounding, uintSeqLen center_position,
                                              intSeqShift initial_pos_shift, intVariantId center_var,
                                              uintAlleleId allele, uintRefSeqId ref_seq_id, const Reference& ref,
                                              bool reverse) const;
    void VariantModStartSurrounding(VariantBiasVarModifiers& bias_mod, uintAlleleId allele, uintRefSeqId ref_seq_id,
                                    const Reference& ref, uintSeqLen cur_start_position,
                                    const Surrounding& surrounding_start) const;
    void PrepareBiasModForCurrentStartPos(VariantBiasVarModifiers& bias_mod, uintRefSeqId ref_seq_id,
                                          const Reference& ref, uintSeqLen cur_start_position,
                                          uintSeqLen first_fragment_length, const Surrounding& surrounding_start) const;
    void VariantModEndSurrounding(VariantBiasVarModifiers& bias_mod, uintAlleleId allele, uintRefSeqId ref_seq_id,
                                  const Reference& ref, uintSeqLen last_position) const;
    void UpdateBiasModForCurrentFragmentLength(VariantBiasVarModifiers& bias_mod, uintRefSeqId ref_seq_id,
                                               const Reference& ref, uintSeqLen cur_start_position,
                                               uintSeqLen cur_end_position, uintSeqLen last_end_position,
                                               uintAlleleId allele) const;
    inline void PrepareEndSurroundingsForCurrentFragmentLength(VariantBiasVarModifiers& bias_mod,
                                                               uintRefSeqId ref_seq_id, const Reference& ref,
                                                               uintSeqLen cur_end_position, uintAlleleId allele) const;
    void PrepareBiasModForCurrentFragmentLength(VariantBiasVarModifiers& bias_mod, uintRefSeqId ref_seq_id,
                                                const Reference& ref, uintSeqLen cur_start_position,
                                                uintSeqLen fragment_length, uintAlleleId allele) const;
    uintPercent GetGCPercent(VariantBiasVarModifiers& bias_mod, uintRefSeqId ref_seq_id, const Reference& ref,
                             uintSeqLen cur_end_position, uintSeqLen fragment_length, uintAlleleId allele);
    void CheckForInsertedBasesToStartFrom(VariantBiasVarModifiers& bias_mod, uintRefSeqId ref_seq_id,
                                          uintSeqLen cur_start_position, const Reference& ref) const;
    void GetOrgSeq(SimPair& sim_reads, bool strand, uintAlleleId allele, uintSeqLen fragment_length,
    ...
    void CTConversion(seqan::DnaString& read, const Reference& ref, uintRefSeqId seq_id, uintSeqLen start_pos,
                      uintAlleleId allele, intVariantId cur_methylation_start, GeneralRandomDistributions& rdist,
                      std::mt19937_64& rgen, bool reversed) const;
    void CTConversion(seqan::DnaString& read, const Reference& ref, uintRefSeqId seq_id, uintSeqLen start_pos,
                      uintAlleleId allele, intVariantId cur_methylation_start, GeneralRandomDistributions& rdist,
                      std::mt19937_64& rgen, bool reversed, const std::vector<Reference::Variant>& variants,
                      std::pair<intVariantId, uintSeqLen> first_variant) const;
    void CTConversion(SimPair& sim_reads, bool strand, uintAlleleId allele, uintSeqLen cur_start_position,
                      ...
                      GeneralRandomDistributions& rdist, std::mt19937_64& rgen) const;
    bool SimulateFromGivenBlock(const SimBlock& block, const SimUnit& unit, const Reference& ref,
                                const DataStats& stats, const ProbabilityEstimates& estimates,
                                GeneralRandomDistributions& rdist, std::mt19937_64& rgen);
    bool SimulateAdapterOnlyPairs(const Reference& ref, const DataStats& stats, const ProbabilityEstimates& estimates,
                                  GeneralRandomDistributions& rdist, std::mt19937_64& rgen);
    static void SimulationThread(Simulator& self, Reference& ref, const DataStats& stats,
                                 const ProbabilityEstimates& estimates);

    bool ApplyErrorsAndQualityToFastaInput(seqan::StringSet<seqan::CharString>& input_ids,
                                           seqan::StringSet<seqan::DnaString>& input_seqs,
                                           seqan::StringSet<seqan::CharString>& output_ids,
                                           seqan::StringSet<seqan::Dna5String>& output_seqs,
                                           seqan::StringSet<seqan::CharString>& output_quals,
                                           GeneralRandomDistributions& rdist, std::mt19937_64& rgen,
                                           const DataStats& stats, const ProbabilityEstimates& estimates);
    static void ErrorModelOnlyThread(Simulator& self, seqan::SeqFileIn& org_seq_reader, const DataStats& stats,
                                     const ProbabilityEstimates& estimates);

    bool WriteOutSystematicErrorProfile(const std::string& id,
                                        std::vector<std::pair<seqan::Dna5, uintPercent>> sys_errors,
                                        seqan::Dna5String dom_err, seqan::CharString err_perc);
  ...
  public:
    ...
    bool CreateSystematicErrorProfile(const char* destination_file, const Reference& ref, const DataStats& stats,
    ...
    bool SimulateErrorModelOnly(const std::string& destination_file, const std::string& org_seq_file, DataStats& stats,
                                const ProbabilityEstimates& estimates, uintNumThreads num_threads, uintSeed seed);
};
} // namespace reseq
#endif // SIMULATOR_H
