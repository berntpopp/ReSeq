    ...
    for (auto template_segment = 2; template_segment--;) {
        for (auto ref_base = 4; ref_base--;) {
            for (auto dom_error = 5; dom_error--;) {
                ...
                SetDimensions(tmp_error_num_by_position_per_tile_.at(template_segment).at(ref_base).at(dom_error),
                              num_tiles, size_pos, size_pos);
                SetDimensions(tmp_error_num_by_error_rate_per_tile_.at(template_segment).at(ref_base).at(dom_error),
...
void ErrorStats::Finalize() {
        ...
        for (auto ref_base = 4; ref_base--;) {
            for (auto dom_error = 5; dom_error--;) {
                called_bases_by_base_quality_per_tile_.at(template_segment)
                    ...
                    .Acquire(
                        tmp_called_bases_by_base_quality_per_tile_.at(template_segment).at(ref_base).at(dom_error));
                called_bases_by_position_per_tile_.at(template_segment)
                    .at(ref_base)
                    .at(dom_error)
                    .Acquire(tmp_called_bases_by_position_per_tile_.at(template_segment).at(ref_base).at(dom_error));
                called_bases_by_error_num_per_tile_.at(template_segment)
                    .at(ref_base)
                    .at(dom_error)
                    .Acquire(tmp_called_bases_by_error_num_per_tile_.at(template_segment).at(ref_base).at(dom_error));
                called_bases_by_error_rate_per_tile_.at(template_segment)
                    .at(ref_base)
                    .at(dom_error)
                    .Acquire(tmp_called_bases_by_error_rate_per_tile_.at(template_segment).at(ref_base).at(dom_error));

                error_num_by_quality_per_tile_.at(template_segment)
                    .at(ref_base)
                    .at(dom_error)
                    .Acquire(tmp_error_num_by_quality_per_tile_.at(template_segment).at(ref_base).at(dom_error));
                error_num_by_position_per_tile_.at(template_segment)
                    .at(ref_base)
                    .at(dom_error)
                    .Acquire(tmp_error_num_by_position_per_tile_.at(template_segment).at(ref_base).at(dom_error));
                error_num_by_error_rate_per_tile_.at(template_segment)
                    .at(ref_base)
                    .at(dom_error)
            ...
            for (auto prev_base = 5; prev_base--;) {
                for (auto prev_called_base = 6; prev_called_base--;) {
                    called_bases_by_base_quality_per_previous_called_base_.at(template_segment)
                        ...
                        .at(prev_base)
                        .at(prev_called_base)
                        .Acquire(tmp_called_bases_by_base_quality_per_previous_called_base_.at(template_segment)
                                     .at(ref_base)
                                     .at(prev_base)
                                     .at(prev_called_base));
                }
    ...
    for (auto indel_type = 2; indel_type--;) {
        for (auto last_call = 6; last_call--;) {
            indel_by_indel_pos_.at(indel_type)
                .at(last_call)
                .Acquire(tmp_indel_by_indel_pos_.at(indel_type).at(last_call));
            indel_by_position_.at(indel_type)
                .at(last_call)
                .Acquire(tmp_indel_by_position_.at(indel_type).at(last_call));
            indel_by_gc_.at(indel_type).at(last_call).Acquire(tmp_indel_by_gc_.at(indel_type).at(last_call));
            indel_pos_by_position_.at(indel_type)
                .at(last_call)
                .Acquire(tmp_indel_pos_by_position_.at(indel_type).at(last_call));
...
void ErrorStats::PreparePlotting() {
    for (uintTempSeq template_segment = 2; template_segment--;) {
        for (auto ref_base = called_bases_by_base_quality_.at(template_segment).size(); ref_base--;) {
            for (auto called_base = called_bases_by_base_quality_.at(template_segment).at(ref_base).size();
                ...
                // Fill vectors
                for (auto dom_error = called_bases_by_base_quality_per_tile_.at(template_segment).at(ref_base).size();
                     dom_error--;) {
                    for (auto tile_id = called_bases_by_base_quality_per_tile_.at(template_segment)
                                            .at(ref_base)
                                            .at(dom_error)
                                            .to();
                         tile_id-- > called_bases_by_base_quality_per_tile_.at(template_segment)
                                         .at(ref_base)
                                         .at(dom_error)
                                         .from();) {
                        called_bases_by_base_quality_.at(template_segment).at(ref_base).at(called_base) +=
                            getConst(called_bases_by_base_quality_per_tile_)
                                .at(template_segment)
                                .at(ref_base)
                                .at(dom_error)
                                .at(tile_id)[called_base]; // called_base might not exist in all tiles for all
                                                           // dom_errors (make const to not change object)
                        called_bases_by_position_.at(template_segment).at(ref_base).at(called_base) +=
                            getConst(called_bases_by_position_per_tile_)
                                .at(template_segment)
                                .at(ref_base)
                                .at(dom_error)
                                .at(tile_id)[called_base]; // called_base might not exist in all tiles for all
    ...
    for (uintInDelType prev_indel = 2; prev_indel--;) {
        ...
        for (uintBaseCall prev_call = 6; prev_call--;) {
            for (auto indel_pos = indel_by_indel_pos_.at(prev_indel).at(prev_call).to();
                 indel_pos-- > indel_by_indel_pos_.at(prev_indel)
                                   .at(prev_call)
                                   .from();) { // indel_pos=0 does not exist for prev_indel=1(deletion) as no indel is
                                               // treated as prev_indel=0(insertion)
                // Deletions
                indel_error_by_length_.at(1)[indel_pos + 1] +=
                    getConst(indel_by_indel_pos_)
                        .at(prev_indel)
                        .at(prev_call)
                        .at(indel_pos)[1]; // There might be no deletions (make const to not change object)

                // Insertions
                for (uintBaseCall ins_nuc = max(static_cast<size_t>(2),
                                                indel_by_indel_pos_.at(prev_indel).at(prev_call).at(indel_pos).from());
                     ins_nuc < indel_by_indel_pos_.at(prev_indel).at(prev_call).at(indel_pos).to(); ++ins_nuc) {
                    indel_error_by_length_.at(0)[indel_pos + 1] +=
                        indel_by_indel_pos_.at(prev_indel).at(prev_call).at(indel_pos).at(ins_nuc);
                }
            }

            for (auto read_pos = indel_by_position_.at(prev_indel).at(prev_call).to();
                 read_pos-- > indel_by_position_.at(prev_indel).at(prev_call).from();) {
                // Deletions
                indel_error_by_position_.at(1)[read_pos] +=
                    getConst(indel_by_position_)
                        .at(prev_indel)
                        .at(prev_call)
                        .at(read_pos)[1]; // There might be no deletions (make const to not change object)
                // Insertions
                for (uintBaseCall ins_nuc = max(static_cast<size_t>(2),
                                                indel_by_position_.at(prev_indel).at(prev_call).at(read_pos).from());
                     ins_nuc < indel_by_position_.at(prev_indel).at(prev_call).at(read_pos).to(); ++ins_nuc) {
                    indel_error_by_position_.at(0)[read_pos] +=
                        indel_by_position_.at(prev_indel).at(prev_call).at(read_pos).at(ins_nuc);
                }
            }

            for (auto gc = indel_by_gc_.at(prev_indel).at(prev_call).to();
                 gc-- > indel_by_gc_.at(prev_indel).at(prev_call).from();) {
                // Deletions
                indel_error_by_gc_.at(1)[gc] +=
                    getConst(indel_by_gc_)
                        .at(prev_indel)
                        .at(prev_call)
                        .at(gc)[1]; // There might be no deletions (make const to not change object)
                // Insertions
                for (uintBaseCall ins_nuc =
                         max(static_cast<size_t>(2), indel_by_gc_.at(prev_indel).at(prev_call).at(gc).from());
                     ins_nuc < indel_by_gc_.at(prev_indel).at(prev_call).at(gc).to(); ++ins_nuc) {
