...
bool DataStats::RecordEqual::operator()(CoverageStats::FullRecord* const& lhs,
...
inline bool DataStats::PotentiallyValidGeneral(const seqan::BamAlignmentRecord& record) const {
    return !hasFlagNextUnmapped(record) && !hasFlagUnmapped(record) && record.rNextId == record.rID &&
           !reference_->ReferenceSequenceExcluded(record.rID);
}
inline bool DataStats::PotentiallyValidFirst(const seqan::BamAlignmentRecord& record_first) const {
    return PotentiallyValidGeneral(record_first) &&
           record_first.beginPos + maximum_insert_length_ >= record_first.pNext;
}
inline bool DataStats::PotentiallyValidSecond(const seqan::BamAlignmentRecord& record_second) const {
    return PotentiallyValidGeneral(record_second) &&
           record_second.pNext + maximum_insert_length_ >= record_second.beginPos;
}
...
bool DataStats::IsSecondRead(CoverageStats::FullRecord* record, CoverageStats::FullRecord*& record_first,
    ...
    if (insert_info.second) {
        ...
        if (PotentiallyValidFirst(record->record_)) {
            ...
            coverage_.AddFragment(record->record_.rID,
...
bool DataStats::CheckForAdapters(const seqan::BamAlignmentRecord& record_first,
                                 const seqan::BamAlignmentRecord& record_second) {
    uintReadLen adapter_position_first, adapter_position_second;
    bool adapter_detected =
        adapters_.Detect(adapter_position_first, adapter_position_second, record_first, record_second, *reference_);
...
bool DataStats::EvalReferenceStatistics(CoverageStats::FullRecord* record, uintTempSeq template_segment,
            ...
            if (record->from_ref_pos_ <= ref_pos && ref_pos < record->to_ref_pos_) {
                ...
                if (!coverage_block->coverage_.at(coverage_pos).valid_) {
                    ...
                    for (indel_pos = 0; indel_pos < cigar_element.count; ++indel_pos) {
                        ...
                        errors_.AddInDel(indel_type, last_base,
    ...
    ++tmp_gc_read_content_mapped_.at(template_segment)
    ...
    if (hasFlagUnmapped(record.first->record_) || hasFlagNextUnmapped(record.first->record_)) {
        ...
        if (!QualitySufficient(record.first->record_) || !QualitySufficient(record.second->record_)) {
            if (reference_->ReferenceSequenceExcluded(record.first->record_.rID) ||
                reference_->ReferenceSequenceExcluded(record.second->record_.rID)) {
                ...
                if (reference_->FragmentExcluded(thread.last_exclusion_region_id_, thread.last_exclusion_ref_seq_,
                                                 record.first->record_.rID, start_pos_first, end_pos_first) ||
                    reference_->FragmentExcluded(thread.last_exclusion_region_id_, thread.last_exclusion_ref_seq_,
                                                 record.second->record_.rID, start_pos_second, end_pos_second)) {
                    reads_in_excluded_regions_ += 2;
                } else {
                    if (CheckForAdapters(record.first->record_,
                                         record.second->record_)) { // We don't trust the mapping, so look for adapters
            ...
            if (record.first->record_.rID == record.second->record_.rID) {
                if (reference_->ReferenceSequenceExcluded(record.first->record_.rID)) {
                    ...
                    if (InProperDirection(record.first->record_, end_pos_second, start_pos_first, start_pos_second) ||
                        adapter_detected) {
                        if ((!adapter_detected && reference_->FragmentExcluded(
                                                      thread.last_exclusion_region_id_, thread.last_exclusion_ref_seq_,
                                                      record.first->record_.rID, start_pos_first, end_pos_second)) ||
                            (adapter_detected && reference_->FragmentExcluded(
                                                     thread.last_exclusion_region_id_, thread.last_exclusion_ref_seq_,
                            ...
                            if (adapter_detected) {
                                ...
                                adapters_.Detect(adapter_position_first, adapter_position_second, record.first->record_,
                                                 record.second->record_, *reference_,
                                                 true); // Call it just to insert the adapter stats, position is better
                                                        // known from mapping
                            }
                            ...
                            if (adapter_detected) {
                                ...
                                fragment_distribution_.AddGCContent(
                                    reference_->GCContent(record.second->record_.rID, start_pos_second, end_pos_first));
                                fragment_distribution_.FillInOutskirtContent(
                                    *reference_, record.second->record_, start_pos_second,
                                ...
                                fragment_distribution_.AddFragmentSite(
                                    record.first->record_.rID, insert_length, record.first->from_ref_pos_,
                                    template_segment, *reference_,
                                ...
                                fragment_distribution_.AddGCContent(
                                    reference_->GCContent(record.first->record_.rID, start_pos_first, end_pos_second));
                                fragment_distribution_.FillInOutskirtContent(*reference_, record.first->record_,
                                                                             start_pos_first, end_pos_second);
                                ...
                                fragment_distribution_.AddFragmentSite(
                                    record.first->record_.rID, insert_length, record.first->from_ref_pos_,
                                    (template_segment + 1) % 2, *reference_,
                            ...
                            fragment_distribution_.AddInsertLengths(
                            ...
                            ++tmp_read_lengths_by_fragment_length_.at(0)
                                  .at(insert_length)
                                  .at(length(record.first->record_.seq));
                            ++tmp_read_lengths_by_fragment_length_.at(1)
                                  .at(insert_length)
...
bool DataStats::SignsOfPairsWithNamesNotIdentical() {
    ...
    if (first_read_records_.size()) {
        printErr
            ...
            << reference_->ReferenceIdFirstPart((*first_read_records_.begin())->record_.rID) << ":"
...
void DataStats::PrepareReadIn(uintQual size_mapping_quality, uintReadLen size_indel, uintSeqLen max_ref_seq_bin_size,
    ...
    for (int template_segment = 2; template_segment--;) {
        for (auto len = read_lengths_.at(template_segment).from(); len < read_lengths_.at(template_segment).to();
             ++len) {
    ...
    coverage_.Prepare(Divide(num_bases, reference_->TotalSize()), Divide(num_bases, num_reads),
                      maximum_read_length_on_reference_);
    ...
    fragment_distribution_.Prepare(*reference_, maximum_insert_length_, max_ref_seq_bin_size, reads_per_frag_len_bin_,
                                   lowq_reads_per_frag_len_bin_, num_threads);
...
void DataStats::FinishReadIn() {
    ...
    for (auto template_segment = 2; template_segment--;) {
        ...
        for (auto frag_len = non_mapped_read_lengths_by_fragment_length_.at(template_segment).from();
             frag_len < non_mapped_read_lengths_by_fragment_length_.at(template_segment).to(); ++frag_len) {
            for (auto read_len = non_mapped_read_lengths_by_fragment_length_.at(template_segment).at(frag_len).from();
                 read_len < non_mapped_read_lengths_by_fragment_length_.at(template_segment).at(frag_len).to();
                 ++read_len) {
                tmp_read_lengths_by_fragment_length_.at(template_segment).at(frag_len).at(read_len) +=
                    non_mapped_read_lengths_by_fragment_length_.at(template_segment).at(frag_len).at(read_len);
            }
        }
...
bool DataStats::Calculate(uintNumThreads num_threads) {
    ...
    corrected_coverage_ = fragment_distribution_.CorrectedCoverage(
        *reference_, static_cast<double>(num_bases) / num_reads,
        coverage_.Coverage()); // Must be called after FinalizeBiasCalculation, where the coverage is corrected
...
bool DataStats::OrderOfBamFileCorrect(const seqan::BamAlignmentRecord& record,
...
bool DataStats::PreRun(BamFileIn& bam, const char* bam_file, BamHeader& header, uintQual& size_mapping_quality,
    ...
    do {
        try {
            ++total_number_reads_; // Counter total_number_reads_ for read in, later in Calculate correct it with the
                                   // number of accepted reads from read_length_
                ...
                if (OrderOfBamFileCorrect(record, last_record_pos)) {
                    if (!hasFlagSecondary(record) && !hasFlagSupplementary(record)) { // Ignore supplementary reads
                        if (hasFlagFirst(record)) {
                            tiles_.EnterTile(record.qName); // Names of records in a pair are identical, so tile must
                        ...
                        if (PotentiallyValid(record)) {
                            ...
                            while (record.rID > cur_ref_seq) {
                                ref_seq_start_bin +=
                                    reference_->SequenceLength(cur_ref_seq) / maximum_insert_length_ + 1;
                                ++cur_ref_seq;
                            }
                        ...
                        if (!hasFlagUnmapped(record) && record.mapQ < minimum_mapping_quality_) {
                            ...
                            ++lowq_reads_per_frag_len_bin_.at(ref_seq_start_bin +
                                                              record.beginPos / maximum_insert_length_);
                        }
    ...
    if (minimum_quality_ < 64) {
        if (minimum_quality_ < 33) {
            printErr << "Minimum quality is " << minimum_quality_
    ...
    printInfo << "Finished PreRun\nTotal number of reads: " << total_number_reads_ << '\n'
              << "Read length: " << min(read_lengths_.at(0).from(), read_lengths_.at(1).from()) << " - "
              << (max(read_lengths_.at(0).to(), read_lengths_.at(1).to()) - 1) << '\n'
              << "Read length on reference: " << minimum_read_length_on_reference_ << " - "
              << maximum_read_length_on_reference_ << '\n'
              << "Quality: " << static_cast<uintQualPrint>(minimum_quality_) << " - "
              << static_cast<uintQualPrint>(maximum_quality_) << " ( "
              << static_cast<uintQualPrint>(phred_quality_offset_) << "-based encoding )\n"
        ...
        while (thread_data.rec_store_.size() < kBatchSize && !atEnd(bam) && reading_success_) {
            ...
            if (hasFlagSecondary(record->record_) || hasFlagSupplementary(record->record_)) {
                ...
                if (PotentiallyValid(record->record_)) {
                    // Use record->record_.beginPos-maximum_read_length_on_reference_ as start, because of potentially
                    // soft-clipped bases at the beginning
                    if (!coverage_.EnsureSpace(record->record_.rID,
                                               (record->record_.beginPos > maximum_read_length_on_reference_
                                                    ? record->record_.beginPos - maximum_read_length_on_reference_
                                                    : 0),
                ...
                // Add low q sites
                if (!QualitySufficient(record->record_) && !hasFlagUnmapped(record->record_) &&
                    !reference_->ReferenceSequenceExcluded(record->record_.rID) && // low quality and not excluded
                    ...
                    if (!reference_->FragmentExcluded(thread_data.last_exclusion_region_id_,
                                                      thread_data.last_exclusion_ref_seq_, record->record_.rID,
                                                      start_pos, end_pos)) {
                        if (hasFlagRC(record->record_)) {
                            fragment_distribution_.AddLowQSiteEnd(record->record_.rID, end_pos, *reference_,
                                                                  thread_data.fragment_distribution_);
                        } else {
                            fragment_distribution_.AddLowQSiteStart(record->record_.rID, start_pos, *reference_,
                                                                    thread_data.fragment_distribution_);
                        }
                    }
                }
            ...
            if (total_number_reads_ > kBatchSize && !(read_records_ % (total_number_reads_ / 20))) {
                ...
                printInfo << "Read " << static_cast<uintPercentPrint>(Percent(read_records_, total_number_reads_))
...
void DataStats::ReadThread(DataStats& self, BamFileIn& bam) {
    ...
    while (not_done && self.reading_success_) {
        ...
        if (self.ReadRecords(bam, not_done, thread_data)) {
            ...
            for (auto& rec : thread_data.rec_store_) {
                ...
                if (self.EvalRecord(rec, thread_data)) {
                    if (self.PotentiallyValidFirst(rec.first->record_)) {
                        ...
                        // soft-clipped bases at the beginning
                        self.coverage_.RemoveFragment(
                            rec.first->record_.rID,
                            (rec.first->record_.beginPos > self.maximum_read_length_on_reference_
                                 ? rec.first->record_.beginPos - self.maximum_read_length_on_reference_
                                 : 0),
            ...
            if (cov_block) { // To make sure we don't have a whole batch of non-valid reads as coverage_.CleanUp already
                ...
                still_needed_reference_sequence = self.coverage_.CleanUp(
                    still_needed_position, *self.reference_, self.qualities_, self.errors_, self.phred_quality_offset_,
                    cov_block, processed_fragments, thread_data.coverage_);
            }
    ...
    if (self.reading_success_) {
        if (0 == --self.running_threads_) {
            ...
            if (!self.coverage_.Finalize(*self.reference_, self.qualities_, self.errors_, self.phred_quality_offset_,
                                         self.print_mutex_, thread_data.coverage_)) {
                self.reading_success_ = false;
            }
        } else {
...
DataStats::DataStats(Reference* ref, uintSeqLen maximum_insert_length, uintQual minimum_mapping_quality)
    : reference_(ref), maximum_insert_length_(maximum_insert_length),
      ...
      reading_success_(true), read_records_(0), minimum_quality_(255), maximum_quality_(0),
      minimum_read_length_on_reference_(numeric_limits<uintReadLen>::max()), maximum_read_length_on_reference_(0),
      total_number_reads_(0), reads_in_unmapped_pairs_without_adapters_(0), reads_in_unmapped_pairs_with_adapters_(0),
      reads_with_low_quality_with_adapters_(0), reads_with_low_quality_without_adapters_(0),
      reads_on_too_short_fragments_(0), reads_in_excluded_regions_(0), reads_used_(0) {
    for (uintTempSeq template_segment = 2; template_segment--;) {
...
inline void DataStats::GetReadPosOnReference(uintSeqLen& start_pos, uintSeqLen& end_pos,
...
bool DataStats::ReadBam(const char* bam_file, const char* adapter_file, const char* adapter_matrix,
            ...
            if (success) {
                ...
                if (length(contigNames(bam_context)) == reference_->NumberSequences()) {
                    ...
                    if (reference_ordering_identical) {
                        ...
                        reference_->ObtainExclusionRegions(
                            reference_->NumberSequences(),
                            maximum_insert_length_); // At the end we need all of them together anyways, so it makes no
                                                     // sense to read them in one after another

                        if ((0 == strlen(adapter_file) && 0 == strlen(adapter_matrix)) ||
                            adapters_.LoadAdapters(adapter_file, adapter_matrix)) {
    ...
    uintFragCount reads_in_wrong_place =
        total_number_reads_ - reads_in_unmapped_pairs_without_adapters_ - reads_in_unmapped_pairs_with_adapters_ -
        reads_with_low_quality_with_adapters_ - reads_with_low_quality_without_adapters_ -
        reads_on_too_short_fragments_ - reads_in_excluded_regions_ - reads_used_;
    printInfo << "Of the " << total_number_reads_ << " reads in the file" << std::endl;
    printInfo << reads_used_ << " (" << static_cast<uintPercentPrint>(Percent(reads_used_, total_number_reads_))
              << "\%) could be used for all statistics" << std::endl;
    printInfo << reads_with_low_quality_with_adapters_ << " ("
              << static_cast<uintPercentPrint>(Percent(reads_with_low_quality_with_adapters_, total_number_reads_))
              << "\%) had too low mapping qualities with adapters detected" << std::endl;
    printInfo << reads_with_low_quality_without_adapters_ << " ("
              << static_cast<uintPercentPrint>(Percent(reads_with_low_quality_without_adapters_, total_number_reads_))
              << "\%) had too low mapping qualities without adapters detected" << std::endl;
    printInfo << reads_in_excluded_regions_ << " ("
              << static_cast<uintPercentPrint>(Percent(reads_in_excluded_regions_, total_number_reads_))
              << "\%) mapped to excluded regions of the reference" << std::endl;
    printInfo << reads_in_wrong_place << " ("
              << static_cast<uintPercentPrint>(Percent(reads_in_wrong_place, total_number_reads_))
              << "\%) were mapping too far apart from their partner or in wrong direction" << std::endl;
    printInfo << reads_on_too_short_fragments_ << " ("
              << static_cast<uintPercentPrint>(Percent(reads_on_too_short_fragments_, total_number_reads_))
              << "\%) are on reference sequences that are too short" << std::endl;
    printInfo << reads_in_unmapped_pairs_with_adapters_ << " ("
              << static_cast<uintPercentPrint>(Percent(reads_in_unmapped_pairs_with_adapters_, total_number_reads_))
              << "\%) were in an unmapped pair with adapters detected" << std::endl;
    printInfo << reads_in_unmapped_pairs_without_adapters_ << " ("
              << static_cast<uintPercentPrint>(Percent(reads_in_unmapped_pairs_without_adapters_, total_number_reads_))
