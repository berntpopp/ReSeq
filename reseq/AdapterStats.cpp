...
reseq::uintReadLen AdapterStats::CountErrors(uintReadLen& last_error_pos, const BamAlignmentRecord& record,
    ...
    for (const auto& cigar_element : record.cigar) {
        ...
        case 'S': // Treat soft-clipping as match, so that bwa and bowtie2 behave the same
            for (auto i = cigar_element.count; i--;) {
                if (ref_pos >= length(ref_seq) || at(ref_seq, ref_pos++) != at(record.seq, read_pos++)) {
                    if (!hasFlagRC(record) ||
                        !num_errors) { // In case of a normal read always overwrite errors to get the last; in case of a
                                       // reversed read stop after the first
        ...
        case 'N':
        case 'D':
            // Deletions can be attributed to the base before or after the deletion, choose the one resulting in the
            // lowest last_error_pos in the end (so shortest adapter required length)
            if (hasFlagRC(record)) {
                if (!num_errors) {
            ...
            break;
        case 'I':
            // Insertions can be attributed to the first or last base of the insertion, choose the one resulting in the
            // lowest last_error_pos in the end (so shortest adapter required length)
            if (hasFlagRC(record)) {
                if (!num_errors) {
                    last_error_pos = read_pos;
...
bool AdapterStats::AdaptersAmbigous(const seqan::DnaString& adaper1, const seqan::DnaString& adaper2,
                                    uintReadLen max_length) {
...
bool AdapterStats::LoadAdapters(const char* adapter_file, const char* adapter_matrix) {
    ...
    for (; nline--;) {
        ...
        for (auto nchar = matrix.at(nline).size(); nchar--;) {
            if ('0' != matrix.at(nline).at(nchar) && '1' != matrix.at(nline).at(nchar)) {
                printErr << "Not allowed character '" << matrix.at(nline).at(nchar) << "' in matrix at line " << nline
                         << ", position " << nchar << std::endl;
                error = true;
            }
        }
    }
    ...
    combinations_.clear();
    combinations_.resize(adapter_list.at(0).size(),
                         vector<bool>(adapter_list.at(1).size(), true)); // Set all combinations by default to valid
    for (auto adapter1 = adapter_list.at(0).size(); adapter1--;) {
        for (auto adapter2 = adapter_list.at(1).size(); adapter2--;) {
            if ('0' == matrix.at(adapter_list.at(0).at(adapter1).second % length(ids))
                           .at(adapter_list.at(1).at(adapter2).second % length(ids))) {
    ...
    if (0 == combinations_.size()) {
        if (hasFlagUnmapped(record) || hasFlagNextUnmapped(record)) {
            ...
            if (record.beginPos <= record.pNext && record.beginPos + length(record.seq) > record.pNext) {
                ...
                auto adapter_start = adapter_kmers_.at(hasFlagLast(record))
                                         .CountSequenceReverse(record.seq, clipped + record.pNext - record.beginPos);
                if (adapter_start >= 0) {
                    ++adapter_start_kmers_.at(hasFlagLast(record)).at(adapter_start);
                }
            }
            ...
            if (record.beginPos >= record.pNext && record.beginPos < record.pNext + length(record.seq)) {
                ...
                auto adapter_start =
                    adapter_kmers_.at(hasFlagLast(record))
                        .CountSequenceForward(
                            record.seq, length(record.seq) -
                                            max(clipped, static_cast<uintReadLen>(record.beginPos - record.pNext)));
                if (adapter_start >= 0) {
                    ++adapter_start_kmers_.at(hasFlagLast(record)).at(adapter_start);
                }
            }
...
bool AdapterStats::PredictAdapters() {
    if (0 == combinations_.size()) {
        ...
        for (uintTempSeq template_segment = kTemplateSegments; template_segment--;) {
            ...
            use_kmer.at(adapter_kmers_.at(template_segment).counts_.size() - 1) =
                false; // Ignore all T's as possible adapter
            ...
            while (!found_adapter && tries--) {
                ...
                for (Kmer<kKmerLength>::intType kmer = max_kmer + 1;
                     kmer < adapter_start_kmers_.at(template_segment).size(); ++kmer) {
                    if (use_kmer.at(kmer) && adapter_start_kmers_.at(template_segment).at(kmer) >
                                                 adapter_start_kmers_.at(template_segment).at(max_kmer)) {
                        max_kmer = kmer;
                    }
                }
                ...
                if (kAdapterSearchInfoFile) {
                    ...
                    for (Kmer<kKmerLength>::intType kmer = 0; kmer < adapter_start_kmers_.at(template_segment).size();
                         ++kmer) {
                ...
                while (true) {
                    ...
                    if (kAdapterSearchInfoFile) {
                        myfile << static_cast<uintTempSeqPrint>(template_segment) << ", " << tries << ", Left"
                               << ++adapter_ext_pos << ", A";
                    ...
                    if (adapter_kmers_.at(template_segment).counts_.at(kmer + 3 * base) >
                        adapter_kmers_.at(template_segment).counts_.at(kmer + 2 * base)) {
                        max_extension = 3;
                        second_highest = 2;
                    } else {
                        max_extension = 2;
                    ...
                    for (uintBaseCall ext = 2; ext--;) {
                        if (adapter_kmers_.at(template_segment).counts_.at(kmer + ext * base) >
                            adapter_kmers_.at(template_segment).counts_.at(kmer + max_extension * base)) {
                            second_highest = max_extension;
                            max_extension = ext;
                        } else if (adapter_kmers_.at(template_segment).counts_.at(kmer + ext * base) >
                                   adapter_kmers_.at(template_segment).counts_.at(kmer + second_highest * base)) {
                            second_highest = ext;
                        }
                    }
                    kmer += base * max_extension;

                    if (use_kmer.at(kmer) && adapter_kmers_.at(template_segment).counts_.at(kmer) > 100 &&
                        adapter_kmers_.at(template_segment).counts_.at(kmer) >
                            5 * adapter_kmers_.at(template_segment)
                                    .counts_.at(kmer - max_extension * base + second_highest * base)) {
                ...
                while (true) {
                    ...
                    if (kAdapterSearchInfoFile) {
                        myfile << static_cast<uintTempSeqPrint>(template_segment) << ", " << tries << ", Right"
                               << ++adapter_ext_pos << ", ";
                    ...
                    if (adapter_kmers_.at(template_segment).counts_.at(kmer + 3) >
                        adapter_kmers_.at(template_segment).counts_.at(kmer + 2)) {
                        max_extension = 3;
                        second_highest = 2;
                    } else {
                        max_extension = 2;
                    ...
                    for (uintBaseCall ext = 2; ext--;) {
                        if (adapter_kmers_.at(template_segment).counts_.at(kmer + ext) >
                            adapter_kmers_.at(template_segment).counts_.at(kmer + max_extension)) {
                            second_highest = max_extension;
                            max_extension = ext;
                        } else if (adapter_kmers_.at(template_segment).counts_.at(kmer + ext) >
                                   adapter_kmers_.at(template_segment).counts_.at(kmer + second_highest)) {
                            second_highest = ext;
                        }
                    }
                    ...
                    if (use_kmer.at(kmer) && adapter_kmers_.at(template_segment).counts_.at(kmer) > 100 &&
                        adapter_kmers_.at(template_segment).counts_.at(kmer) >
                            5 * adapter_kmers_.at(template_segment).counts_.at(kmer - max_extension + second_highest)) {
                ...
                if (30 > length(adapter)) {
                    printInfo << "Detected non-extendible sequence '" << adapter << "' as adapter for read "
                              << static_cast<uintTempSeqPrint>(template_segment + 1) << std::endl;
                } else if (120 < length(adapter)) {
                    printErr << "Detected very long sequence '" << adapter << "' as adapter for read "
                             << static_cast<uintTempSeqPrint>(template_segment + 1)
                             << ", which is likely part of the genome." << std::endl;
                    if (kAdapterSearchInfoFile) {
                        myfile.close();
                    }
                    return false;
                } else {
                    printInfo << "Detected adapter " << static_cast<uintTempSeqPrint>(template_segment + 1) << ": "
                              << adapter << std::endl;
                    found_adapter = true;
                }
            }
            ...
            if (!found_adapter) {
                printErr << "Only non-extendible sequences were found as adapter for read "
                         << static_cast<uintTempSeqPrint>(template_segment + 1) << std::endl;
                if (kAdapterSearchInfoFile) {
                    myfile.close();
                }
...
void AdapterStats::PrepareAdapters(uintReadLen size_read_length, uintQual phred_quality_offset) {
    ...
    skewer::cMatrix::InitParameters(skewer::TRIM_PE, 0.1, 0.03, phred_quality_offset,
                                    false); // 0.1 and 0.03 are the default values from the skewer software

    for (auto& adapter : seqs_.at(0)) {
        skewer::cMatrix::AddAdapter(skewer::cMatrix::firstAdapters, toCString(static_cast<CharString>(adapter)),
                                    length(adapter), skewer::TRIM_TAIL);
    }
    for (auto& adapter : seqs_.at(1)) {
        skewer::cMatrix::AddAdapter(skewer::cMatrix::secondAdapters, toCString(static_cast<CharString>(adapter)),
                                    length(adapter), skewer::TRIM_TAIL);
    }
    skewer::cMatrix::CalculateIndices(combinations_, seqs_.at(0).size(), seqs_.at(1).size());
}

bool AdapterStats::Detect(uintReadLen& adapter_position_first, uintReadLen& adapter_position_second,
                          const BamAlignmentRecord& record_first, const BamAlignmentRecord& record_second,
                          const Reference& reference, bool properly_mapped) {
    ...
    if (hasFlagUnmapped(record_first) || hasFlagUnmapped(record_second)) {
        ...
        if ((hasFlagUnmapped(record_first) || CountErrors(last_error_pos, record_first, reference)) &&
            (hasFlagUnmapped(record_second) || CountErrors(last_error_pos, record_second, reference))) {
        ...
        if (CountErrors(last_error_pos1, record_first, reference) &&
            CountErrors(last_error_pos2, record_second, reference)) {
            i_min_overlap =
                max(last_error_pos1, last_error_pos2) + 1; // A perfect matching piece cannot be an adapter, so adapter
                                                           // must be at least reach last error in both reads
        } else {
            search_adapters = false;
        }
    }
    ...
    if (search_adapters) {
        // Fill read1, read2 with first/second measured in sequencer(by fastq file) not by position in bam file and
        // convert to measurement/fastq direction (not reference/bam direction)
        ...
        auto index1 = skewer::cMatrix::findAdapter(toCString(read1), length(read1),
                                                   reinterpret_cast<unsigned char*>(toCString(*qual1)), length(*qual1),
                                                   i_min_overlap);
        if (index1.bc--) { // Adapter has been detected for first read (bc is now index of adapter)
            auto index2 = skewer::cMatrix::findAdapter2(toCString(read2), length(read2),
                                                        reinterpret_cast<unsigned char*>(toCString(*qual2)),
                                                        length(*qual2), i_min_overlap);
            if (index2.bc-- &&
                combinations_.at(index1.bc).at(index2.bc)) { // Adapter has been detected also for second read and it is
                                                             // a valid pair with the first
                if (2 > max(index1.pos, index2.pos) -
                            min(index1.pos,
                                index2.pos)) { // Allow a single one-base indel in the insert between the adapters
                    if (properly_mapped || VerifyInsert(read1, read2, index1.pos, index2.pos)) {
                        ...
                        for (uintReadLen pos = index1.pos + length(seqs_.at(0).at(index1.bc)); pos < length(read1);
                             ++pos) {
                            if (poly_a_tail) {
                                if ('A' == at(read1, pos)) {
                                    ++poly_a_tail_length;
                        ...
                        for (uintReadLen pos = index2.pos + length(seqs_.at(1).at(index2.bc)); pos < length(read2);
                             ++pos) {
                            if (poly_a_tail) {
                                if ('A' == at(read2, pos)) {
                                    ++poly_a_tail_length;
                        ...
                        // If the read starts with an adapter note down if and how much it has been cut of at the
                        // beginning (position below 0) and set position to 0 for the further stats
                        if (1 > index1.pos) {
                        ...
                        // Fill in stats
                        ++tmp_counts_.at(index1.bc)
                              .at(index2.bc)
                              .at(length(read1) - index1.pos)
                              .at(length(read2) - index2.pos);
...
void AdapterStats::SumCounts() {
    ...
    // Sum adapters starting from length where adapters are unambiguous (sorted by content: so simply first position
    // that differs from adapter before and after)
    uintFragCount sum;
    ...
    for (auto a1 = counts_.size(); a1--;) {
        ...
        if (a1) { // Not last in loop
            while (first_diff_after_a1 < min(length(seqs_.at(0).at(a1)), length(seqs_.at(0).at(a1 - 1))) &&
                   at(seqs_.at(0).at(a1), first_diff_after_a1) == at(seqs_.at(0).at(a1 - 1), first_diff_after_a1)) {
                ++first_diff_after_a1;
            }
        }

        first_diff_before_a2 = 0;
        for (auto a2 = counts_.at(0).size();
             a2--;) { // All vectors i are the same length so we can simply take the length of 0
            first_diff_after_a2 = 0;
            if (a2) { // Not last in loop
                while (first_diff_after_a2 < min(length(seqs_.at(1).at(a2)), length(seqs_.at(1).at(a2 - 1))) &&
                       at(seqs_.at(1).at(a2), first_diff_after_a2) == at(seqs_.at(1).at(a2 - 1), first_diff_after_a2)) {
                    ++first_diff_after_a2;
                }
            }

            sum = 0;
            for (auto pos1 = max(max(first_diff_before_a1, first_diff_after_a1),
                                 static_cast<uintReadLen>(counts_.at(a1).at(a2).from()));
                 pos1 < counts_.at(a1).at(a2).to(); ++pos1) {
                for (auto pos2 = max(max(first_diff_before_a2, first_diff_after_a2),
                                     static_cast<uintReadLen>(counts_.at(a1).at(a2).at(pos1).from()));
                     pos2 < counts_.at(a1).at(a2).at(pos1).to(); ++pos2) {
...
void AdapterStats::PrepareSimulation() {
    for (uintTempSeq seg = kTemplateSegments; seg--;) {
        ...
        uintFragCount threshold = ceil(*max_element(count_sum_.at(seg).begin(), count_sum_.at(seg).end()) *
                                       kMinFractionOfMaximumForSimulation);
