...
class Reference {
  public:
    ...
    class Variant {
      public:
        ...
        template <typename T>
        Variant(uintSeqLen position, const T& var_seq, const std::array<uintAlleleBitArray, kMaxAlleles / 64>& allele)
            : position_(position), var_seq_(var_seq), allele_(allele) {}
  ...
  private:
    const uintSeqLen kMaxNInFragmentSite =
        50; // The maximum number of n that is allowed in a fragment site to be counted for the bias calculation as it
            // is highly likely that the fragment length and GC from those sites are wrong and highly unlikely that real
            // fragments are found in this regions
    const uintSeqLen kMinNToSplitContigs = 10; // At positions with minimum kMinNToSplitContigs N's scaffolds will be
                                               // broken into contigs, which means kMinDistToContigEnds applies
    const uintSeqLen kMinNToReplaceNWithRepeat =
        100; // If at least kMinNToReplaceNWithRepeat consecutive N's are in a reference sequence the function ReplaceN
             // fills in a short repeated pattern for these N instead of random bases
    ...
    inline seqan::CharString& RefId(uintRefSeqId n) { return utilities::at(reference_ids_, n); }

    inline seqan::Dna5String& RefSeq(uintRefSeqId n) { return utilities::at(reference_sequences_, n); }
    ...
    std::vector<std::vector<std::pair<uintSeqLen, uintSeqLen>>>
        excluded_regions_; // excluded_regions_[RefSeqId][PositionSortedExclusionRegionID] = {Start,End}
    std::vector<uintSeqLen> sum_of_excluded_bases_;

    // Variants variables
    uintAlleleId num_alleles_;
    seqan::VcfFileIn vcf_file_;
    seqan::VcfRecord cur_vcf_record_; // Already read but not yet handled vcf record (mostly the first of the next
                                      // reference sequence, that is not yet prepared)
    std::atomic<uintRefSeqId> read_variation_for_num_sequences_;
    std::atomic<uintRefSeqId> cleared_variation_for_num_sequences_;
    std::vector<std::vector<Reference::Variant>>
        variants_; // variants_[RefSeqId][PositionSortedVariantID] = {Position,VarSeq,[Yes/No per allele]}
    std::vector<std::vector<uintSeqLen>>
        variant_positions_; // variant_positions_[RefSeqId][PositionSortedVariantID] = VariantPosition
    ...
    std::vector<std::vector<std::vector<double>>>
        unmethylation_; // unmethylation_[RefSeqId][Allele][PositionSortedMethylationID] = 1 - Methylation
    std::vector<std::vector<std::pair<uintSeqLen, uintSeqLen>>>
        unmethylated_regions_; // unmethylated_regions_[RefSeqId][PositionSortedMethylationID] =
                               // {UnMethylatedRegionStart, UnMethylatedRegionEnd}
    ...
    inline void PushBackExclusionRegion(std::pair<uintSeqLen, uintSeqLen> region, uintRefSeqId ref_seq,
    ...
    template <typename T>
    void InsertVariant(uintRefSeqId ref_seq_id, uintSeqLen position, const T& var_seq,
                       const std::array<uintAlleleBitArray, Variant::kMaxAlleles / 64>& allele) {
        if (0 == variants_.at(ref_seq_id).size()) {
            ...
            auto insert_it = variants_.at(ref_seq_id).end(); // Have variant at same position sorted by length:
                                                             // deletion/substitution/insertion(by length)
            ...
            while (0 < var && variants_.at(ref_seq_id).at(--var).position_ == position) {
                if (variants_.at(ref_seq_id).at(var).var_seq_ == var_seq) {
                    ...
                    for (auto block = variants_.at(ref_seq_id).at(var).allele_.size(); block--;) {
                        variants_.at(ref_seq_id).at(var).allele_.at(block) =
                            variants_.at(ref_seq_id).at(var).allele_.at(block) | allele.at(block);
                    }
                    return;
    ...
    inline void UpdateGC(uintSeqLen& gc, uintSeqLen& n_count, const seqan::Dna5String& ref_seq, uintSeqLen old_pos,
                         uintSeqLen new_pos) const {
    ...
    void AddFragmentSite(std::vector<FragmentSite>& sites, uintSeqLen fragment_length, uintSeqLen gc,
                         uintSeqLen n_count, const Surrounding& start_sur, const Surrounding& end_sur) const;
  ...
  public:
    ...
    inline uintRefSeqId NumberSequences() const { return length(reference_ids_); }

    inline const seqan::CharString& ReferenceId(uintRefSeqId n) const { return utilities::at(reference_ids_, n); }
    ...
    void ReferenceSequence(seqan::DnaString& insert_string, uintRefSeqId seq_id, uintSeqLen start_pos,
                           uintSeqLen frag_length, bool reversed = false) const;
#ifndef SWIG // This part is not needed for the python plotting and swig can't handle the nested classes
    void ReferenceSequence(seqan::DnaString& insert_string, uintRefSeqId seq_id, uintSeqLen start_pos,
                           uintSeqLen frag_length, bool reversed, const std::vector<Variant>& variants,
                           std::pair<intVariantId, uintSeqLen> first_variant, uintAlleleId allele) const;
#endif // SWIG

    inline const seqan::Dna5String& operator[](uintRefSeqId n) const { return ReferenceSequence(n); }

    uint32_t SequenceLength(uintRefSeqId n) const { return length(ReferenceSequence(n)); }
    ...
    inline bool N(uintRefSeqId seq_id, uintSeqLen pos) const { return utilities::IsN(ReferenceSequence(seq_id), pos); }

    inline uintSeqLen GCContentAbsolut(uintSeqLen& n_count, uintRefSeqId seq_id, uintSeqLen start_pos,
                                       uintSeqLen end_pos) const {
        uintSeqLen gc = 0;
        n_count = 0;
    ...
    inline void UpdateGC(uintSeqLen& gc, uintSeqLen& n_count, uintRefSeqId seq_id, uintSeqLen old_pos,
                         uintSeqLen new_pos) const {
        UpdateGC(gc, n_count, ReferenceSequence(seq_id), old_pos, new_pos);
    }

    inline unsigned char GCContent(uintRefSeqId seq_id, uintSeqLen start_pos,
                                   uintSeqLen end_pos) const { // end_pos like basically everywhere else is the position
                                                               // after the last element that still should be counted
        uintSeqLen n_count(0);
...
#ifndef SWIG // This part is not needed for the python plotting and swig can't handle the decltype in Vect and
             // SeqQualityStats
    static inline double Bias(double normalization, double ref_seq, double fragment_length, double gc, double start_sur,
                              double end_sur) {
    ...
    double SumBias(double& max_bias, uintRefSeqId ref_seq_id, uintSeqLen fragment_length, double general_bias,
                   const Vect<double>& gc_bias, const SurroundingBias& sur_bias) const;
    double SumBias(uintRefSeqId ref_seq_id, uintSeqLen fragment_length, double general_bias,
                   const Vect<double>& gc_bias, const SurroundingBias& sur_bias) const;
    void GetFragmentSites(std::vector<FragmentSite>& sites, uintRefSeqId ref_seq_id, uintSeqLen fragment_length,
    ...
    inline bool ObtainedExclusionRegionsForSequence(uintRefSeqId ref_seq_id) const {
        return obtained_exclusion_regions_for_num_sequences_ > ref_seq_id;
    }
    inline bool ExclusionRegionsCompletelyObtained() const {
        return obtained_exclusion_regions_for_num_sequences_ >= NumberSequences();
    }
    bool FragmentExcluded(uintSeqLen& last_region_id, uintRefSeqId& last_ref_seq, uintRefSeqId ref_seq_id,
                          uintSeqLen fragment_start, uintSeqLen fragment_end) const;
    bool ReferenceSequenceExcluded(uintRefSeqId ref_seq_id) const {
        return 1 == excluded_regions_.at(ref_seq_id).size();
    }
    ...
    uintSeqLen SumExcludedBases(uintRefSeqId ref_seq_id) const { return sum_of_excluded_bases_.at(ref_seq_id); }
    ...
    uintSeqLen NumExcludedRegions(uintRefSeqId ref_seq_id) const { return excluded_regions_.at(ref_seq_id).size(); }
    uintSeqLen StartExclusion(uintRefSeqId ref_seq_id) const { return excluded_regions_.at(ref_seq_id).front().second; }
    uintSeqLen EndExclusion(uintRefSeqId ref_seq_id) const { return excluded_regions_.at(ref_seq_id).back().first; }
    ...
    void CorrectPositionAddingExcludedRegions(uintSeqLen& start_pos, uintSeqLen& next_exclusion_region,
                                              uintRefSeqId ref_seq) const {
        while (start_pos >= excluded_regions_.at(ref_seq)
                                .at(next_exclusion_region)
                                .first) { // We don't reach the end of the list because we don't test for the last bin
                                          // (start of last bin is tested on second to last bin)
            start_pos += excluded_regions_.at(ref_seq).at(next_exclusion_region).second -
                         excluded_regions_.at(ref_seq).at(next_exclusion_region).first;
            ++next_exclusion_region;
        }
    }
    void CorrectPositionRemovingExcludedRegions(uintSeqLen& correction, uintSeqLen& cur_exclusion_region,
                                                uintRefSeqId ref_seq_id, uintSeqLen position) const {
        while (cur_exclusion_region + 1 < NumExcludedRegions(ref_seq_id) &&
               position > excluded_regions_.at(ref_seq_id).at(cur_exclusion_region + 1).first) {
            ++cur_exclusion_region;
            correction += excluded_regions_.at(ref_seq_id).at(cur_exclusion_region).second -
                          excluded_regions_.at(ref_seq_id).at(cur_exclusion_region).first;
        }
    ...
    inline bool VariantsLoadedForSequence(uintRefSeqId ref_seq_id) const {
        return read_variation_for_num_sequences_ > ref_seq_id;
    }
    inline bool VariantPositionsLoadedForSequence(uintRefSeqId ref_seq_id) const {
        return read_variation_for_num_sequences_ >= ref_seq_id;
    }
    inline bool VariantsCompletelyLoaded() const { return read_variation_for_num_sequences_ >= variants_.size(); }
    inline bool VariantPositionsCompletelyLoaded() const {
        return read_variation_for_num_sequences_ >= variant_positions_.size();
    }
    inline const std::vector<Reference::Variant>& Variants(uintRefSeqId ref_seq_id) const {
        return variants_.at(ref_seq_id);
    }
    inline const std::vector<uintRefSeqId>& VariantPositions(uintRefSeqId ref_seq_id) const {
    ...
    inline bool MethylationLoadedForSequence(uintRefSeqId ref_seq_id) const {
        return read_methylation_for_num_sequences_ > ref_seq_id;
    }
    inline bool MethylationCompletelyLoaded() const {
        return read_methylation_for_num_sequences_ >= unmethylation_.size();
    }
...
struct FragmentSite {
    ...
    FragmentSite(uintPercent gc, const Surrounding& start_sur, const Surrounding& end_sur)
        : gc_(gc), start_surrounding_(start_sur), end_surrounding_(end_sur), count_forward_(0), count_reverse_(0),
          bias_(1.0) {}

    FragmentSite()
        : // Placeholder
          gc_(0), count_forward_(0), count_reverse_(0), bias_(0.0) {}

    static bool LowerBias(const FragmentSite& lhs, const FragmentSite& rhs) { return lhs.bias_ < rhs.bias_; }
};
} // namespace reseq
