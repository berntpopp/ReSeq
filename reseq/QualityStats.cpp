...
void QualityStats::SplitPairedSequenceQuality() {
    ...
    for (auto tile_id = sequence_quality_mean_paired_per_tile_.size(); tile_id--;) {
        for (auto sq_first = sequence_quality_mean_paired_per_tile_.at(tile_id).to();
             sq_first-- > sequence_quality_mean_paired_per_tile_.at(tile_id).from();) {
            for (auto sq_second = sequence_quality_mean_paired_per_tile_.at(tile_id).at(sq_first).to();
                 sq_second-- > sequence_quality_mean_paired_per_tile_.at(tile_id).at(sq_first).from();) {
                sequence_quality_mean_per_tile_.at(0)[tile_id][sq_first] +=
                    sequence_quality_mean_paired_per_tile_.at(tile_id).at(sq_first).at(sq_second);
                sequence_quality_mean_per_tile_.at(1)[tile_id][sq_second] +=
                    sequence_quality_mean_paired_per_tile_.at(tile_id).at(sq_first).at(sq_second);

                // Sum up tiles
                sequence_quality_mean_paired_[sq_first][sq_second] +=
                    sequence_quality_mean_paired_per_tile_.at(tile_id).at(sq_first).at(sq_second);
            }
        }
    }
...
void QualityStats::SumTiles() {
    ...
    for (uintTempSeq template_segment = kTemplateSegments; template_segment--;) {
        ...
        for (auto ref_base = base_quality_stats_per_tile_per_error_reference_.at(template_segment).size();
             ref_base--;) {
            for (auto dom_error =
                     base_quality_stats_per_tile_per_error_reference_.at(template_segment).at(ref_base).size();
                 dom_error--;) {
                for (auto tile_id = base_quality_stats_per_tile_per_error_reference_.at(template_segment)
                                        .at(ref_base)
                                        .at(dom_error)
                                        .from();
                     tile_id < base_quality_stats_per_tile_per_error_reference_.at(template_segment)
                                   .at(ref_base)
                                   .at(dom_error)
                                   .to();
                     ++tile_id) {
                    base_quality_stats_reference_.at(template_segment) +=
        ...
        for (auto tile_id = sequence_quality_mean_for_gc_per_tile_reference_.at(template_segment).size(); tile_id--;) {
            sequence_quality_tile_sum +=
                sequence_quality_mean_for_gc_per_tile_reference_.at(template_segment).at(tile_id);
        }
        average_sequence_quality_for_gc_.at(template_segment).Clear();
        for (uintPercent gc_percentage = sequence_quality_tile_sum.to();
             gc_percentage-- > sequence_quality_tile_sum.from();) {
            sequence_quality_tile_sum.at(gc_percentage).Calculate(false);
            average_sequence_quality_for_gc_.at(template_segment)[gc_percentage] =
                sequence_quality_tile_sum.at(gc_percentage).mean_;
        }
        ...
        for (auto tile_id = sequence_quality_mean_per_tile_.at(template_segment).size(); tile_id--;) {
            sequence_quality_mean_.at(template_segment) +=
                sequence_quality_mean_per_tile_.at(template_segment).at(tile_id);
        }
        ...
        for (auto called_base = kNumBasesN; called_base--;) {
            ...
            for (auto tile_id = base_quality_stats_per_tile_.at(template_segment).at(called_base).size(); tile_id--;) {
                base_quality_stats_.at(template_segment) +=
                    base_quality_stats_per_tile_.at(template_segment).at(called_base).at(tile_id);

                base_quality_for_sequence_.at(template_segment) +=
                    base_quality_for_sequence_per_tile_.at(template_segment).at(called_base).at(tile_id);
                base_quality_for_preceding_quality_.at(template_segment) +=
                    base_quality_for_preceding_quality_per_tile_.at(template_segment).at(called_base).at(tile_id);
            }
            for (auto tile_id = sequence_quality_for_base_per_tile_.at(template_segment).at(called_base).size();
                 tile_id--;) { // Always has all nucleotides (including N), therefore it might have tiles that the
                               // previous one does not have for N
                sequence_quality_tile_sum +=
                    sequence_quality_for_base_per_tile_.at(template_segment).at(called_base).at(tile_id);
            }

            average_sequence_quality_for_base_.at(template_segment).at(called_base).Clear();
            for (uintPercent nuc_percentage = sequence_quality_tile_sum.to();
                 nuc_percentage-- > sequence_quality_tile_sum.from();) {
                sequence_quality_tile_sum.at(nuc_percentage).Calculate(false);
                average_sequence_quality_for_base_.at(template_segment).at(called_base)[nuc_percentage] =
                    sequence_quality_tile_sum.at(nuc_percentage).mean_;
            }
...
void QualityStats::CalculateQualityStats() {
    ...
    for (uintTempSeq template_segment = kTemplateSegments; template_segment--;) {
        ...
        for (auto pos = base_quality_stats_.at(template_segment).size(); pos--;) {
            ...
            for (auto tile_id = base_quality_stats_per_tile_.at(template_segment).at(0).size(); tile_id--;) {
                ...
                for (auto base = base_quality_stats_per_tile_.at(template_segment).size(); base--;) {
                    base_quality_stats_tile_sum +=
                        getConst(base_quality_stats_per_tile_)
                            .at(template_segment)
                            .at(base)[tile_id][pos]; // Tile and position might not exist for all bases, especially for
                                                     // the N, but make const to not modify Vect
                }
        ...
        for (auto pos = base_quality_stats_per_strand_.at(template_segment).size(); pos--;) {
            ...
            stats =
                &base_quality_stats_per_strand_.at(template_segment).at(pos); // template_segment really is strand here

            base_quality_mean_per_strand_.at(template_segment)[pos] =
                stats->DoubleMean(); // template_segment really is strand here
        }

        // Fill mean_sequence_quality_mean_by_fragment_length_
        uintSeqLen min_frag_length(
            sequence_quality_mean_for_fragment_length_per_tile_reference_.at(template_segment).at(0).from()),
            max_frag_length(0);
        for (uintTileId tile_id =
                 sequence_quality_mean_for_fragment_length_per_tile_reference_.at(template_segment).to();
             tile_id--;) {
            SetToMin(
                min_frag_length,
                sequence_quality_mean_for_fragment_length_per_tile_reference_.at(template_segment).at(tile_id).from());
            SetToMax(
                max_frag_length,
                sequence_quality_mean_for_fragment_length_per_tile_reference_.at(template_segment).at(tile_id).to());
        }
        ...
        for (uintSeqLen frag_length = max_frag_length; frag_length-- > min_frag_length;) {
            from = frag_length;
            for (const auto& sq_mean_at_tile :
                 sequence_quality_mean_for_fragment_length_per_tile_reference_.at(template_segment)) {
                for (uintQual qual = sq_mean_at_tile[frag_length].from(); qual < sq_mean_at_tile[frag_length].to();
                     ++qual) {
                    counts += sq_mean_at_tile[frag_length].at(
                        qual); // Fragment length might not exist for this tile, as we go from min to max of all tiles,
                               // make Vect const, so we don't change it
        ...
        if (counts) { // Add last fragment bin to the previous bin if it did not reach 100 counts
            for (uintSeqLen len = last_to; len-- > from;) {
                mean_sequence_quality_mean_by_fragment_length_.at(template_segment)[len] =
                    Divide(sum + last_sum, counts + last_counts);
            }
        }
    }
}

void QualityStats::Prepare(uintTileId num_tiles, uintQual size_qual, uintReadLen size_pos,
                           uintSeqLen maximum_fragment_length) {
    // Precalculate values
    ...
    for (auto template_segment = kTemplateSegments; template_segment--;) {
        for (auto ref_base = kNumBases; ref_base--;) {
            for (auto dom_error = kNumBasesN; dom_error--;) {
                ...
                SetDimensions(tmp_error_rate_for_position_per_tile_per_error_reference_.at(template_segment)
                                  .at(ref_base)
                                  .at(dom_error),
                              num_tiles, size_pos, 101);
                SetDimensions(tmp_base_quality_for_error_rate_per_tile_per_error_reference_.at(template_segment)
                                  .at(ref_base)
                                  .at(dom_error),
                              num_tiles, 101, size_qual);
            }
            SetDimensions(tmp_base_quality_for_preceding_quality_per_tile_reference_.at(template_segment).at(ref_base),
                          num_tiles, size_qual, size_qual);
            SetDimensions(tmp_preceding_quality_for_error_rate_per_tile_reference_.at(template_segment).at(ref_base),
                          num_tiles, 101, size_qual);
            SetDimensions(tmp_preceding_quality_for_position_per_tile_reference_.at(template_segment).at(ref_base),
                          num_tiles, size_pos, size_qual);
            SetDimensions(tmp_base_quality_for_sequence_quality_per_tile_reference_.at(template_segment).at(ref_base),
                          num_tiles, size_qual, size_qual);
            SetDimensions(
                tmp_preceding_quality_for_sequence_quality_per_tile_reference_.at(template_segment).at(ref_base),
                num_tiles, size_qual, size_qual);
            SetDimensions(tmp_sequence_quality_for_error_rate_per_tile_reference_.at(template_segment).at(ref_base),
                          num_tiles, 101, size_qual);
            SetDimensions(tmp_sequence_quality_for_position_per_tile_reference_.at(template_segment).at(ref_base),
                          num_tiles, size_pos, size_qual);
        }
        ...
        for (auto base = kNumBasesN; base--;) {
            ...
            SetDimensions(tmp_base_quality_for_preceding_quality_per_tile_.at(template_segment).at(base), num_tiles,
                          size_qual, size_qual);
            SetDimensions(tmp_base_quality_stats_per_tile_.at(template_segment).at(base), num_tiles, size_pos,
                          size_qual);
            SetDimensions(tmp_preceding_quality_for_sequence_per_tile_.at(template_segment).at(base), num_tiles,
                          size_qual, size_qual);
            SetDimensions(tmp_preceding_quality_for_position_per_tile_.at(template_segment).at(base), num_tiles,
                          size_pos, size_qual);
            SetDimensions(tmp_sequence_quality_for_position_per_tile_.at(template_segment).at(base), num_tiles,
                          size_pos, size_qual);

            SetDimensions(tmp_sequence_quality_for_base_per_tile_.at(template_segment).at(base), num_tiles, 101,
                          size_qual);
    ...
    if (kWriteOut4dMatrixCsvs) {
        SetDimensions(tmp_csv_seg0_a_base_quality_preceding_quality_sequence_quality_position_, size_qual, size_qual,
                      size_qual, size_pos);
        SetDimensions(tmp_csv_seg1_c_base_quality_preceding_quality_error_rate_position_, size_qual, size_qual, 101,
                      size_pos);
    }
}
...
void QualityStats::Finalize(uintFragCount total_number_reads) {
    ...
    for (auto template_segment = kTemplateSegments; template_segment--;) {
        for (auto ref_base = kNumBases; ref_base--;) {
            for (auto dom_error = kNumBasesN; dom_error--;) {
                base_quality_stats_per_tile_per_error_reference_.at(template_segment)
                    .at(ref_base)
                    .at(dom_error)
                    .Acquire(tmp_base_quality_stats_per_tile_per_error_reference_.at(template_segment)
                                 .at(ref_base)
                                 .at(dom_error));
                error_rate_for_position_per_tile_per_error_reference_.at(template_segment)
                    .at(ref_base)
                    .at(dom_error)
                    .Acquire(tmp_error_rate_for_position_per_tile_per_error_reference_.at(template_segment)
                                 .at(ref_base)
                                 .at(dom_error));
                base_quality_for_error_rate_per_tile_per_error_reference_.at(template_segment)
                    .at(ref_base)
                    .at(dom_error)
                    .Acquire(tmp_base_quality_for_error_rate_per_tile_per_error_reference_.at(template_segment)
                                 .at(ref_base)
                                 .at(dom_error));
            }
            base_quality_for_preceding_quality_per_tile_reference_.at(template_segment)
                .at(ref_base)
                .Acquire(tmp_base_quality_for_preceding_quality_per_tile_reference_.at(template_segment).at(ref_base));
            preceding_quality_for_error_rate_per_tile_reference_.at(template_segment)
                .at(ref_base)
                .Acquire(tmp_preceding_quality_for_error_rate_per_tile_reference_.at(template_segment).at(ref_base));
            preceding_quality_for_position_per_tile_reference_.at(template_segment)
                .at(ref_base)
                .Acquire(tmp_preceding_quality_for_position_per_tile_reference_.at(template_segment).at(ref_base));
            base_quality_for_sequence_quality_per_tile_reference_.at(template_segment)
                .at(ref_base)
                .Acquire(tmp_base_quality_for_sequence_quality_per_tile_reference_.at(template_segment).at(ref_base));
            preceding_quality_for_sequence_quality_per_tile_reference_.at(template_segment)
                .at(ref_base)
                .Acquire(
                    tmp_preceding_quality_for_sequence_quality_per_tile_reference_.at(template_segment).at(ref_base));
            sequence_quality_for_error_rate_per_tile_reference_.at(template_segment)
                .at(ref_base)
                .Acquire(tmp_sequence_quality_for_error_rate_per_tile_reference_.at(template_segment).at(ref_base));
            sequence_quality_for_position_per_tile_reference_.at(template_segment)
                .at(ref_base)
                .Acquire(tmp_sequence_quality_for_position_per_tile_reference_.at(template_segment).at(ref_base));
        }

        sequence_quality_mean_for_gc_per_tile_reference_.at(template_segment)
            .Acquire(tmp_sequence_quality_mean_for_gc_per_tile_reference_.at(template_segment));
        sequence_quality_mean_for_mean_error_rate_per_tile_reference_.at(template_segment)
            .Acquire(tmp_sequence_quality_mean_for_mean_error_rate_per_tile_reference_.at(template_segment));
        sequence_quality_mean_for_fragment_length_per_tile_reference_.at(template_segment)
            .Acquire(tmp_sequence_quality_mean_for_fragment_length_per_tile_reference_.at(template_segment));
        mean_error_rate_for_gc_per_tile_reference_.at(template_segment)
            .Acquire(tmp_mean_error_rate_for_gc_per_tile_reference_.at(template_segment));
        mean_error_rate_for_fragment_length_per_tile_reference_.at(template_segment)
            .Acquire(tmp_mean_error_rate_for_fragment_length_per_tile_reference_.at(template_segment));
        ...
        for (auto base = kNumBasesN; base--;) {
            ...
            base_quality_for_preceding_quality_per_tile_.at(template_segment)
                .at(base)
                .Acquire(tmp_base_quality_for_preceding_quality_per_tile_.at(template_segment).at(base));
            base_quality_stats_per_tile_.at(template_segment)
                .at(base)
                .Acquire(tmp_base_quality_stats_per_tile_.at(template_segment).at(base));
            preceding_quality_for_sequence_per_tile_.at(template_segment)
                .at(base)
                .Acquire(tmp_preceding_quality_for_sequence_per_tile_.at(template_segment).at(base));
            preceding_quality_for_position_per_tile_.at(template_segment)
                .at(base)
                .Acquire(tmp_preceding_quality_for_position_per_tile_.at(template_segment).at(base));
            sequence_quality_for_position_per_tile_.at(template_segment)
                .at(base)
                .Acquire(tmp_sequence_quality_for_position_per_tile_.at(template_segment).at(base));

            sequence_quality_for_base_per_tile_.at(template_segment)
                .at(base)
                .Acquire(tmp_sequence_quality_for_base_per_tile_.at(template_segment).at(base));
            nucleotide_quality_.at(template_segment)
                .at(base)
                .Acquire(tmp_nucleotide_quality_.at(template_segment).at(base));
        }

        base_quality_stats_per_strand_.at(template_segment)
            .Acquire(tmp_base_quality_stats_per_strand_.at(template_segment));

        sequence_quality_mean_for_gc_per_tile_.at(template_segment)
            .Acquire(tmp_sequence_quality_mean_for_gc_per_tile_.at(template_segment));
        sequence_quality_probability_mean_.at(template_segment)
            .Acquire(tmp_sequence_quality_probability_mean_.at(template_segment));
        sequence_quality_minimum_.at(template_segment).Acquire(tmp_sequence_quality_minimum_.at(template_segment));
        sequence_quality_first_quartile_.at(template_segment)
            .Acquire(tmp_sequence_quality_first_quartile_.at(template_segment));
        sequence_quality_median_.at(template_segment).Acquire(tmp_sequence_quality_median_.at(template_segment));
        sequence_quality_third_quartile_.at(template_segment)
            .Acquire(tmp_sequence_quality_third_quartile_.at(template_segment));
    ...
    for (uintTempSeq template_segment = kTemplateSegments; template_segment--;) {
        for (auto qual = sequence_quality_content_.at(template_segment).from();
            ...
            if (sequence_quality_content_.at(template_segment).at(qual).size()) {
                sequence_quality_content_.at(template_segment).at(qual)[0] +=
                    total_number_reads - SumVect(sequence_quality_content_.at(template_segment).at(qual));
            }
        }
    }
    ...
    if (kWriteOut4dMatrixCsvs) {
        ...
        for (uintQualPrint qual = 0;
             qual < tmp_csv_seg0_a_base_quality_preceding_quality_sequence_quality_position_.size(); ++qual) {
            for (uintQualPrint pq = 0;
                 pq < tmp_csv_seg0_a_base_quality_preceding_quality_sequence_quality_position_.at(qual).size(); ++pq) {
                for (uintQualPrint sq = 0;
                     sq <
                     tmp_csv_seg0_a_base_quality_preceding_quality_sequence_quality_position_.at(qual).at(pq).size();
                     ++sq) {
                    for (uintReadLen pos = 0;
                         pos < tmp_csv_seg0_a_base_quality_preceding_quality_sequence_quality_position_.at(qual)
                                   .at(pq)
                                   .at(sq)
                                   .size();
                         ++pos) {
                        if (0 < tmp_csv_seg0_a_base_quality_preceding_quality_sequence_quality_position_.at(qual)
                                    .at(pq)
                                    .at(sq)
                                    .at(pos)) {
                            myfile << qual << ", " << pq << ", " << sq << ", " << pos << ", "
                                   << tmp_csv_seg0_a_base_quality_preceding_quality_sequence_quality_position_.at(qual)
                                          .at(pq)
                                          .at(sq)
                                          .at(pos)
                                   << '\n';
        ...
        for (uintQualPrint qual = 0; qual < tmp_csv_seg1_c_base_quality_preceding_quality_error_rate_position_.size();
             ++qual) {
            for (uintQualPrint pq = 0;
                 pq < tmp_csv_seg1_c_base_quality_preceding_quality_error_rate_position_.at(qual).size(); ++pq) {
                for (uintPercentPrint er = 0;
                     er < tmp_csv_seg1_c_base_quality_preceding_quality_error_rate_position_.at(qual).at(pq).size();
                     ++er) {
                    for (uintReadLen pos = 0;
                         pos < tmp_csv_seg1_c_base_quality_preceding_quality_error_rate_position_.at(qual)
                                   .at(pq)
                                   .at(er)
                                   .size();
                         ++pos) {
                        if (0 < tmp_csv_seg1_c_base_quality_preceding_quality_error_rate_position_.at(qual)
                                    .at(pq)
                                    .at(er)
                                    .at(pos)) {
                            myfile << qual << ", " << pq << ", " << er << ", " << pos << ", "
                                   << tmp_csv_seg1_c_base_quality_preceding_quality_error_rate_position_.at(qual)
                                          .at(pq)
                                          .at(er)
                                          .at(pos)
                                   << '\n';
        ...
        for (auto ref_base = kNumBases; ref_base--;) {
            for (auto dom_error = kNumBasesN; dom_error--;) {
                ...
                ShrinkVect(error_rate_for_position_per_tile_per_error_reference_.at(template_segment)
                               .at(ref_base)
                               .at(dom_error));
                ShrinkVect(base_quality_for_error_rate_per_tile_per_error_reference_.at(template_segment)
                               .at(ref_base)
                               .at(dom_error));
            }
    ...
    for (uintTempSeq template_segment = kTemplateSegments; template_segment--;) {
        for (auto ref_base = base_quality_stats_per_tile_per_error_reference_.at(template_segment).size();
            ...
            for (auto dom_error =
                     base_quality_stats_per_tile_per_error_reference_.at(template_segment).at(ref_base).size();
                 dom_error--;) {
                base_quality_stats_per_tile_reference_.at(template_segment).at(ref_base) +=
                    base_quality_stats_per_tile_per_error_reference_.at(template_segment).at(ref_base).at(dom_error);
                error_rate_for_position_per_tile_reference_.at(template_segment).at(ref_base) +=
                    error_rate_for_position_per_tile_per_error_reference_.at(template_segment)
                        .at(ref_base)
                        .at(dom_error);
                base_quality_for_error_rate_per_tile_reference_.at(template_segment).at(ref_base) +=
                    base_quality_for_error_rate_per_tile_per_error_reference_.at(template_segment)
                        .at(ref_base)
                        .at(dom_error);
            }
