...
double Simulator::CoveragePropLostFromAdapters(const DataStats& stats) {
    ...
    for (uintTempSeq seg = kTemplateSegments; seg--;) {
        for (uintSeqLen frag_len = stats.ReadLengthsByFragmentLength(seg).from();
            ...
            for (uintReadLen read_len = stats.ReadLengthsByFragmentLength(seg).at(frag_len).from();
                 read_len < stats.ReadLengthsByFragmentLength(seg).at(frag_len).to(); ++read_len) {
                total_bases += stats.ReadLengthsByFragmentLength(seg).at(frag_len).at(read_len) * read_len;
                if (frag_len < read_len) {
                    adapter_bases += (stats.ReadLengthsByFragmentLength(seg).at(frag_len).at(read_len) -
                                      stats.NonMappedReadLengthsByFragmentLength(seg)[frag_len][read_len]) *
                                     (read_len - frag_len);
    ...
    return static_cast<double>(adapter_bases) / total_bases; // Compensate the part that is made up of adapters and
                                                             // therefore does not produce reference coverage
}

reseq::uintFragCount Simulator::CoverageToNumberPairs(double coverage, uintRefLenCalc total_ref_size,
                                                      double average_read_length, double adapter_part) {
    return round(coverage * total_ref_size / average_read_length / 2 / (1 - adapter_part));
}

double Simulator::NumberPairsToCoverage(uintFragCount total_pairs, uintRefLenCalc total_ref_size,
                                        double average_read_length, double adapter_part) {
...
bool Simulator::FlushWriteValues(uintTempSeq template_segment, StringSet<CharString>* old_output_ids,
...
void Simulator::GetSysErrorFromBlock(Dna5& dom_error, uintPercent& error_rate, const SimBlock*& block,
                                     uintSeqLen& block_pos, intVariantId& cur_var, uintSeqLen& var_pos,
                                     uintAlleleId allele) {
...
bool Simulator::FillReadPart(SimRead& sim_read, uintTempSeq template_segment, uintTileId tile_id,
                             const DnaString& org_sequence, uintReadLen org_pos, char base_cigar_element,
                             const SimBlock* block, uintSeqLen block_pos, uintAdapterId adapter_id,
                             ReadFillParameter& par, pair<intVariantId, uintSeqLen> first_variant, uintAlleleId allele,
    ...
    while (par.read_pos_ < par.read_length_ && org_pos < length(org_sequence)) {
        ...
        indel = static_cast<ErrorStats::InDelDef>(
        ...
        if (ErrorStats::InDelDef::kNoInDel == indel) {
            ...
            // Roll quality
            par.qual_ = estimates.Quality(template_segment, tile_id, at(org_sequence, org_pos))
                            .Draw(prob_quality, prob_sum, {par.seq_qual_, par.qual_, par.read_pos_, par.error_rate_},
                                  rdist.ZeroToOne(rgen));
            if (0.0 == prob_sum) {
                if (par.read_pos_) {
                    par.qual_ =
                        at(sim_read.qual_, par.read_pos_ - 1) - stats.PhredQualityOffset(); // Take last quality again
                } else {
            ...
            // Roll base call
            par.base_call_ =
                estimates.BaseCall(template_segment, tile_id, at(org_sequence, org_pos), dom_error)
                    .Draw(prob_base_call, prob_sum, {par.qual_, par.read_pos_, par.num_errors_, par.error_rate_},
                          rdist.ZeroToOne(rgen));
            if (0.0 == prob_sum) {
            ...
            // Insertion
            // Roll quality
            at(sim_read.qual_, par.read_pos_) =
                stats.PhredQualityOffset() + estimates.Quality(template_segment, tile_id, at(org_sequence, org_pos))
                                                 .Draw(prob_quality, prob_sum,
                                                       {par.seq_qual_, par.qual_, par.read_pos_, par.error_rate_},
                                                       rdist.ZeroToOne(rgen));
            if (0.0 == prob_sum) {
...
bool Simulator::FillRead(SimRead& sim_read, uintReadLen& num_errors, uintTempSeq template_segment, uintTileId tile_id,
                         uintSeqLen fragment_length, const SimBlock* start_block, uintSeqLen block_start_pos,
                         pair<intVariantId, uintSeqLen> first_variant, uintAlleleId allele, const DataStats& stats,
                         const ProbabilityEstimates& estimates, GeneralRandomDistributions& rdist, mt19937_64& rgen) {
    ...
    uintReadLen seq_length(min(par.read_length_, static_cast<uintReadLen>(length(sim_read.org_seq_)))),
        read_pos(seq_length);
    ...
    if (seq_length) {
        ...
        par.gc_seq_ = Percent(
            par.gc_seq_, static_cast<uintReadLen>(length(stats.Adapters().Sequence(template_segment, adapter_id))));
        mean_error_rate =
            Divide(mean_error_rate,
                   static_cast<uintReadLenCalc>(length(stats.Adapters().Sequence(template_segment, adapter_id))));
    }

    // Roll sequence quality
    vector<double> prob_seq_qual;
    double prob_sum;
    par.seq_qual_ = estimates.SequenceQuality(template_segment, tile_id)
                        .Draw(prob_seq_qual, prob_sum,
                              {par.gc_seq_, mean_error_rate, fragment_length / QualityStats::kSqFragmentLengthBinSize},
                              rdist.ZeroToOne(rgen));
    if (0.0 == prob_sum) {
        par.seq_qual_ = estimates.SequenceQuality(template_segment, tile_id)
                            .MostLikely(); // Take the one with the highest general probability, as parameter specific
                                           // probabilities do not exist
    }

    // Reference part of the sequence
    if (!FillReadPart(sim_read, template_segment, tile_id, sim_read.org_seq_, 0, 'M', start_block, block_start_pos, 0,
                      par, first_variant, allele, stats, estimates, rdist, rgen)) {
        return false;
    }
    ...
    if (par.read_pos_ < par.read_length_) {
        ...
        if (0 == par.read_pos_) {
            ...
            discrete_distribution<uint64_t> rdist_cut(stats.Adapters().StartCut(template_segment, adapter_id).begin(),
                                                      stats.Adapters().StartCut(template_segment, adapter_id).end());
        ...
        if (par.read_pos_ < par.read_length_) {
            ...
            for (uintReadLen pos_tail = 0; pos_tail < tail_length && par.read_pos_ < par.read_length_; ++pos_tail) {
                ...
                par.qual_ =
                    estimates.Quality(template_segment, tile_id, 0)
                        .Draw(prob_quality, prob_sum, {par.seq_qual_, par.qual_, par.read_pos_, par.error_rate_},
                              rdist.ZeroToOne(rgen));
                if (0.0 == prob_sum) {
                    par.qual_ =
...
void Simulator::CreateReadId(CharString& id, uintRefSeqBin block_number, uintFragCount read_number,
                             uintSeqLen start_pos, uintSeqLen end_pos, uintAlleleId allele, uintTile tile,
                             uintRefSeqId ref_seq_id, const Reference& ref, const CigarString& cigar,
                             uintReadLen num_errors) {
    // start_pos should start at 1 (not at 0 as the array)[0 means adapter only], end_pos should be the last included
    // position (not the first not included position like usual)
...
bool Simulator::CreateReads(const Reference& ref, const DataStats& stats, const ProbabilityEstimates& estimates,
                            GeneralRandomDistributions& rdist, SimPair& sim_reads, mt19937_64& rgen,
                            uintFragCount counts, bool strand, uintAlleleId allele, uintRefSeqId ref_seq_id,
                            uintSeqLen fragment_length, uintFragCount& read_number, const SimBlock* start_block,
                            uintSeqLen start_position_forward, uintSeqLen end_position_forward,
                            pair<intVariantId, uintSeqLen> start_variant, pair<intVariantId, uintSeqLen> end_variant) {
    ...
    if (fragment_length) {
        ...
        if (ref.VariantsLoaded()) {
            ...
            variant.at(!strand) = {
                end_block->first_variant_id_ - end_variant.first,
                (end_variant.second
                     ? length(ref.Variants(ref_seq_id).at(end_variant.first).var_seq_) - end_variant.second
                     : 0)};
        }
    } else {
        // Adapter only pair
    ...
    for (auto n = counts; n--;) {
        ...
        for (uintTempSeq template_segment = kTemplateSegments; template_segment--;) {
            ...
            if (!FillRead(sim_reads.at(template_segment), num_errors, template_segment, tile_id, fragment_length,
                          ...
                          variant.at(template_segment), allele, stats, estimates, rdist, rgen)) {
                return false;
            }

            CreateReadId(sim_reads.at(template_segment).id_, block_id, read_number, print_start_position,
                         print_end_position, allele, stats.Tiles().Tiles().at(tile_id), ref_seq_id, ref,
                         sim_reads.at(template_segment).cigar_, num_errors);
        }
...
bool Simulator::LoadSysErrorRecord(uintRefSeqId ref_id, const Reference& ref) {
    try {
        ...
        printErr << "Could not read systematic error profile for reference sequence '" << ref.ReferenceId(ref_id)
                 << "': " << e.what() << std::endl;
        simulation_error_ = true;
        return false;
    }
    ...
    if (length(sys_dom_err_) != ref.SequenceLength(ref_id)) {
        printErr << "Systematic error profile '" << sys_id_ << "' (length " << length(sys_dom_err_)
                 << ") does not match reference sequence '" << ref.ReferenceId(ref_id) << "' (length "
                 << ref.SequenceLength(ref_id) << "). Wrong file or order incorrect?" << std::endl;
        simulation_error_ = true;
        return false;
    }
...
void Simulator::SetSystematicErrorVariantsReverse(uintSeqLen& start_dist_error_region, uintPercent& start_rate,
    ...
    if (ref.VariantsLoaded()) {
        ...
        for (; 0 <= var_id && ref.Variants(ref_seq_id).at(var_id).position_ >= end_pos; --var_id) {
            ...
            // Get last_base
            if (sys_last_var_pos_per_allele_.at(chosen_allele) < ref.SequenceLength(ref_seq_id) &&
                sys_last_var_pos_per_allele_.at(chosen_allele) == var.position_ + 1) {
            ...
            // Get dom_base
            if (sys_last_var_pos_per_allele_.at(chosen_allele) < ref.SequenceLength(ref_seq_id) &&
                sys_last_var_pos_per_allele_.at(chosen_allele) <= var.position_ + DominantBase::kLastX) {
                ...
                for (auto pos = sys_last_var_pos_per_allele_.at(chosen_allele) - 1; pos > var.position_; --pos) {
                    sys_dom_base_per_allele_.at(chosen_allele)
                        .Update(Complement::Dna(at(ref.ReferenceSequence(ref_seq_id), pos)));
                }
            } else {
                ...
                if (rev_pos) {
                    ...
                    sys_dom_base_per_allele_.at(chosen_allele)
                        .Set(ConstDna5StringReverseComplement(ref.ReferenceSequence(ref_seq_id)), rev_pos - 1);
                }
            }
            ...
            if (block.first_variant_id_ == var_id) {
                ...
                for (auto pos = block.start_pos_ + block.sys_errors_.size() -
                                ref.Variants(ref_seq_id).at(var_id + 1).position_ - 1;
                     pos < block_pos; ++pos) {
                    stats.Coverage().UpdateDistances(start_dist_error_region, start_rate,
                                                     block.sys_errors_.at(pos).second);
                }
            ...
            if (ref.Variants(ref_seq_id).size() > var_id + 1 && var_id != block.first_variant_id_ &&
                ref.Variants(ref_seq_id).at(var_id + 1).position_ <
                    var.position_ + sys_gc_range_ / 3) { // Updating is ~3 times as much work as recounting
                for (auto pos = ref.SequenceLength(ref_seq_id) - ref.Variants(ref_seq_id).at(var_id + 1).position_ - 1;
                     pos < rev_pos; ++pos) {
            ...
            if (0 < length(var.var_seq_)) {
                ...
                for (uintSeqLen vpos = length(var.var_seq_); vpos--;) {
                    ...
                    DrawSystematicError(tmp_sys_errors, base, last_base,
                                        sys_dom_base_per_allele_.at(chosen_allele).Get(), SafePercent(gc, gc_bases),
                                        TransformDistanceToStartOfErrorRegion(start_dist_error_region), start_rate,
                                        estimates);
                    last_base = base;
                }
            }
            ...
            auto ref_allele =
                ref.NumAlleles(); // For this allele we already used the reference, so in case we have another allele
                                  // identical to the reference up to the current variance we can copy the values
            if (sys_last_var_pos_per_allele_.at(chosen_allele) >= ref.SequenceLength(ref_seq_id) ||
                sys_last_var_pos_per_allele_.at(chosen_allele) + length(var.var_seq_) >
                    var.position_ + DominantBase::kLastX) {
                ref_allele = chosen_allele;
            }
            for (uintAlleleId allele = 0; allele < ref.NumAlleles(); ++allele) {
                if (var.InAllele(allele)) {
                    if (allele !=
                        chosen_allele) { // Dom base for chosen allele was already updated during the drawing process
                        // Update dom base
                        if (sys_last_var_pos_per_allele_.at(allele) < ref.SequenceLength(ref_seq_id) &&
                            sys_last_var_pos_per_allele_.at(allele) + length(var.var_seq_) <=
                                var.position_ + DominantBase::kLastX) {
                            for (auto pos = sys_last_var_pos_per_allele_.at(allele) - 1; pos > var.position_; --pos) {
                                sys_dom_base_per_allele_.at(allele).Update(
                                    Complement::Dna(at(ref.ReferenceSequence(ref_seq_id), pos)));
                            }
                            ...
                            if (ref_allele < ref.NumAlleles()) {
                                ...
                                if (rev_pos) {
                                    // If we have bases before the variant add them
                                    sys_dom_base_per_allele_.at(allele).Set(
                                        ConstDna5StringReverseComplement(ref.ReferenceSequence(ref_seq_id)),
                                        rev_pos - 1);
                                }
        ...
        if (sys_from_file_) {
            if (block.first_variant_id_ == var_id) {
                ...
                for (auto pos = block.start_pos_ + block.sys_errors_.size() -
                                ref.Variants(ref_seq_id).at(var_id + 1).position_ - 1;
                     pos < block.sys_errors_.size(); ++pos) {
                    stats.Coverage().UpdateDistances(start_dist_error_region, start_rate,
                                                     block.sys_errors_.at(pos).second);
                }
            }
...
bool Simulator::CreateUnit(uintRefSeqId ref_id, uintRefSeqBin first_block_id, Reference& ref, const DataStats& stats,
    ...
    ResetSystematicErrorCounters(
        ref); // Reset them again for the creation of the forward strand that will follow block by block
...
void Simulator::SetSystematicErrorVariantsForward(uintSeqLen& start_dist_error_region, uintPercent& start_rate,
                                                  SimBlock& block, uintRefSeqId ref_seq_id, uintSeqLen end_pos,
                                                  const Reference& ref, const DataStats& stats,
                                                  const ProbabilityEstimates& estimates) {
    if (ref.VariantsLoaded()) {
        ...
        auto var_id = block.first_variant_id_;
        for (; var_id < ref.Variants(ref_seq_id).size() && ref.Variants(ref_seq_id).at(var_id).position_ < end_pos;
             ++var_id) {
            auto& var(ref.Variants(ref_seq_id).at(var_id));
            chosen_allele = var.FirstAllele();
            tmp_sys_errors.clear();
            ...
            // Get last_base
            if (sys_last_var_pos_per_allele_.at(chosen_allele) < ref.SequenceLength(ref_seq_id) &&
                sys_last_var_pos_per_allele_.at(chosen_allele) + 1 == var.position_) {
                last_base = sys_last_base_per_allele_.at(chosen_allele);
            } else {
                if (var.position_) {
            ...
            // Get dom_base
            if (sys_last_var_pos_per_allele_.at(chosen_allele) < ref.SequenceLength(ref_seq_id) &&
                sys_last_var_pos_per_allele_.at(chosen_allele) + DominantBase::kLastX >= var.position_) {
                for (auto pos = sys_last_var_pos_per_allele_.at(chosen_allele) + 1; pos < var.position_; ++pos) {
                ...
                if (var.position_) {
                    // If we have bases before the variant add them
                    sys_dom_base_per_allele_.at(chosen_allele)
                        .Set(ref.ReferenceSequence(ref_seq_id), var.position_ - 1);
                }
            }
            ...
            if (block.first_variant_id_ == var_id) {
                ...
                for (auto pos = ref.Variants(ref_seq_id).at(var_id - 1).position_ - block.start_pos_;
                     pos < var.position_ - block.start_pos_; ++pos) {
                    stats.Coverage().UpdateDistances(start_dist_error_region, start_rate,
                                                     block.sys_errors_.at(pos).second);
                }
            }
            ...
            // Get GC (Variants' GC is not taken into account)
            if (var_id && var_id != block.first_variant_id_ &&
                ref.Variants(ref_seq_id).at(var_id - 1).position_ + sys_gc_range_ / 3 >
                    var.position_) { // Updating is ~3 times as much work as recounting
            ...
            auto ref_allele =
                ref.NumAlleles(); // For this allele we already used the reference, so in case we have another allele
                                  // identical to the reference up to the current variance we can copy the values
            if (sys_last_var_pos_per_allele_.at(chosen_allele) >= ref.SequenceLength(ref_seq_id) ||
                sys_last_var_pos_per_allele_.at(chosen_allele) + DominantBase::kLastX <
                    var.position_ + length(var.var_seq_)) {
                ref_allele = chosen_allele;
            }
            for (uintAlleleId allele = 0; allele < ref.NumAlleles(); ++allele) {
                if (var.InAllele(allele)) {
                    if (allele !=
                        ...
                        // Update dom base
                        if (sys_last_var_pos_per_allele_.at(allele) < ref.SequenceLength(ref_seq_id) &&
                            sys_last_var_pos_per_allele_.at(allele) + DominantBase::kLastX >=
                                var.position_ + length(var.var_seq_)) {
                            for (auto pos = sys_last_var_pos_per_allele_.at(allele) + 1; pos < var.position_; ++pos) {
                            ...
                            if (ref_allele < ref.NumAlleles()) {
                                ...
                                sys_dom_base_per_allele_.at(allele).Clear();
                                if (var.position_) {
                                    sys_dom_base_per_allele_.at(allele).Set(ref.ReferenceSequence(ref_seq_id),
                                                                            var.position_ - 1);
                                }
...
bool Simulator::CreateBlock(Reference& ref, const DataStats& stats, const ProbabilityEstimates& estimates) {
    ...
    if (!simulation_error_) {
        ...
        if (sys_from_file_) {
            ...
            SetSystematicErrorVariantsForward(distance_to_start_of_error_region_, start_error_rate_, *block,
                                              unit->ref_seq_id_, end_pos, ref, stats, estimates);
        } else {
            auto tmp_distance_to_start_of_error_region = distance_to_start_of_error_region_;
            auto tmp_start_error_rate = start_error_rate_;
            SetSystematicErrors(block->sys_errors_, ref.ReferenceSequence(unit->ref_seq_id_), block->start_pos_,
                                end_pos, stats, estimates);
            SetSystematicErrorVariantsForward(tmp_distance_to_start_of_error_region, tmp_start_error_rate, *block,
                                              unit->ref_seq_id_, end_pos, ref, stats, estimates);
        }
...
void Simulator::GetPossibleAlleles(vector<uintAlleleId>& possible_alleles, const Reference& ref,
                                   const VariantBiasVarModifiers& bias_mod, uintSeqLen cur_start_position,
                                   uintRefSeqId ref_seq) const {
    if (ref.VariantsLoaded()) {
...
void Simulator::SelectAllele(vector<uintAlleleId>& chosen_allele_ids, vector<bool>& reverse_selection,
                             uintAlleleId possible_strands, double random_value) const {
...
void Simulator::DrawNAlleles(vector<uintAlleleId>& chosen_allele_ids, vector<bool>& reverse_selection,
                             uintAlleleId non_zero_strands, uintAlleleId possible_strands,
                             GeneralRandomDistributions& rdist, mt19937_64& rgen) const {
...
void Simulator::ReverseSelection(vector<uintAlleleId>& chosen_allele_ids, vector<bool>& reverse_selection,
                                 uintAlleleId possible_strands) const {
    reverse_selection.clear();
    reverse_selection.resize(possible_strands, true);
...
void Simulator::ChooseAlleles(vector<uintAlleleId>& chosen_allele_ids, vector<bool>& reverse_selection,
                              uintAlleleId non_zero_strands, uintAlleleId possible_strands,
                              GeneralRandomDistributions& rdist, mt19937_64& rgen) const {
    if (non_zero_strands <= possible_strands / 2) {
        ...
        DrawNAlleles(chosen_allele_ids, reverse_selection, possible_strands - non_zero_strands, possible_strands, rdist,
                     rgen);
        ReverseSelection(chosen_allele_ids, reverse_selection, possible_strands);
    }
}

inline bool Simulator::VariantInsideCurrentFragment(intVariantId cur_var_id, uintSeqLen cur_end_position,
                                                    intSeqShift end_pos_shift, uintRefSeqId ref_seq_id,
                                                    const Reference& ref) const {
    // variant with that id exists && ( is relevant to end surrounding || is relevant to start surrounding) // if insert
    // length is very short start surrounding can be longer than end surrounding
    return ref.Variants(ref_seq_id).size() > cur_var_id &&
           (ref.Variants(ref_seq_id).at(cur_var_id).position_ < cur_end_position + end_pos_shift);
}

void Simulator::HandleGCModAndEndPosShiftForNewVariants(VariantBiasVarModifiers& bias_mod, uintAlleleId allele,
                                                        uintRefSeqId ref_seq_id, const Reference& ref,
                                                        uintSeqLen cur_end_position) const {
    while (VariantInsideCurrentFragment(bias_mod.unhandled_variant_id_.at(allele), cur_end_position,
                                        bias_mod.end_pos_shift_.at(allele), ref_seq_id, ref) &&
           0 == bias_mod.unhandled_bases_in_variant_.at(allele)) {
        ...
        if (!var.InAllele(allele)) {
            ...
            // ---GC modification---
            // Add GC from variant
            for (uintSeqLen pos = 0; pos < length(var.var_seq_) &&
                                     var.position_ + pos < cur_end_position + bias_mod.end_pos_shift_.at(allele);
                 ++pos) {
            ...
            if (0 == length(var.var_seq_)) {
                ...
                if (var.position_ + length(var.var_seq_) <= cur_end_position + bias_mod.end_pos_shift_.at(allele)) {
                    ...
                    bias_mod.unhandled_bases_in_variant_.at(allele) =
                        var.position_ + length(var.var_seq_) - (cur_end_position + bias_mod.end_pos_shift_.at(allele));
                    bias_mod.end_pos_shift_.at(allele) -=
                        length(var.var_seq_) - bias_mod.unhandled_bases_in_variant_.at(allele) - 1;
...
void Simulator::HandleSurroundingVariantsBeforeCenter(Surrounding& mod_surrounding, uintSeqLen center_position,
                                                      intSeqShift initial_pos_shift, intVariantId center_var,
                                                      uintAlleleId allele, uintRefSeqId ref_seq_id,
                                                      const Reference& ref, bool reverse) const {
    ...
    while (0 <= --cur_var) { // + Break statements at beginning
        if (reverse) {
            sur_pos =
                static_cast<intSeqShift>(center_position) - ref.Variants(ref_seq_id).at(cur_var).position_ + pos_shift;
        } else {
            sur_pos =
                static_cast<intSeqShift>(ref.Variants(ref_seq_id).at(cur_var).position_) - center_position + pos_shift;
        }
        ...
        if (ref.Variants(ref_seq_id).at(cur_var).InAllele(allele)) {
            if (0 == length(ref.Variants(ref_seq_id).at(cur_var).var_seq_)) {
                // Deletion
                if (reverse) {
                    auto new_base_pos = static_cast<intSeqShift>(center_position) + pos_shift -
                                        static_cast<intSeqShift>(Surrounding::Length());
                    if (0 > new_base_pos) {
                        mod_surrounding.DeleteSurroundingBaseShiftingOnRightSide(
                            sur_pos, Complement::Dna(at(ref.ReferenceSequence(ref_seq_id),
                                                        ref.SequenceLength(ref_seq_id) + new_base_pos)));
                    } else {
                        mod_surrounding.DeleteSurroundingBaseShiftingOnRightSide(
                            sur_pos, Complement::Dna(at(ref.ReferenceSequence(ref_seq_id), new_base_pos)));
                    }
                    --pos_shift;
                } else {
                    if (++pos_shift > center_position) {
                        mod_surrounding.DeleteSurroundingBaseShiftingOnLeftSide(
                            sur_pos, at(ref.ReferenceSequence(ref_seq_id),
                                        ref.SequenceLength(ref_seq_id) + center_position - pos_shift));
                    } else {
                        mod_surrounding.DeleteSurroundingBaseShiftingOnLeftSide(
                            sur_pos, at(ref.ReferenceSequence(ref_seq_id), center_position - pos_shift));
                    }
                }
            } else {
                // Base modification (Insertions may also include a base modification and we skip checking first if we
                // exchange it with the same)
                if (reverse) {
                    mod_surrounding.ChangeSurroundingBase(
                        sur_pos, Complement::Dna(at(ref.Variants(ref_seq_id).at(cur_var).var_seq_, 0)));
                } else {
                    mod_surrounding.ChangeSurroundingBase(sur_pos,
                                                          at(ref.Variants(ref_seq_id).at(cur_var).var_seq_, 0));
                }

                if (1 < length(ref.Variants(ref_seq_id).at(cur_var).var_seq_)) {
                    // Insertion (First base is the already handled)
                    if (reverse) {
                        mod_surrounding.InsertSurroundingBasesShiftingOnRightSide(
                            sur_pos, ReverseComplementorDna(suffix(ref.Variants(ref_seq_id).at(cur_var).var_seq_, 1)));
                        pos_shift += length(ref.Variants(ref_seq_id).at(cur_var).var_seq_) - 1;
                    } else {
                        mod_surrounding.InsertSurroundingBasesShiftingOnLeftSide(
                            sur_pos, suffix(ref.Variants(ref_seq_id).at(cur_var).var_seq_, 1));
                        pos_shift -= length(ref.Variants(ref_seq_id).at(cur_var).var_seq_) - 1;
                    }
                }
            }
...
void Simulator::HandleSurroundingVariantsAfterCenter(Surrounding& mod_surrounding, uintSeqLen center_position,
                                                     intSeqShift initial_pos_shift, intVariantId center_var,
                                                     uintAlleleId allele, uintRefSeqId ref_seq_id, const Reference& ref,
                                                     bool reverse) const {
    auto pos_shift = initial_pos_shift;
    intSeqShift sur_pos;
    for (auto cur_var = center_var; cur_var < ref.Variants(ref_seq_id).size();
         ++cur_var) { // + Break statements at beginning
        if (reverse) {
            sur_pos =
                static_cast<intSeqShift>(center_position) - ref.Variants(ref_seq_id).at(cur_var).position_ + pos_shift;
        } else {
        ...
        if (ref.Variants(ref_seq_id).at(cur_var).InAllele(allele)) {
            if (0 == length(ref.Variants(ref_seq_id).at(cur_var).var_seq_)) {
                ...
                if (reverse) {
                    ...
                    mod_surrounding.DeleteSurroundingBaseShiftingOnLeftSide(
                        sur_pos, Complement::Dna(at(ref.ReferenceSequence(ref_seq_id),
                                                    (center_position + pos_shift) % ref.SequenceLength(ref_seq_id))));
                } else {
                    mod_surrounding.DeleteSurroundingBaseShiftingOnRightSide(
                        sur_pos,
                        at(ref.ReferenceSequence(ref_seq_id),
                           (center_position - pos_shift + Surrounding::Length()) % ref.SequenceLength(ref_seq_id)));
                    --pos_shift;
                }
            } else {
                // Base modification (Insertions may also include a base modification and we skip checking first if we
                ...
                if (1 < length(ref.Variants(ref_seq_id).at(cur_var).var_seq_)) {
                    ...
                    if (reverse) {
                        if (sur_pos) {
                            mod_surrounding.InsertSurroundingBasesShiftingOnLeftSide(
                                sur_pos - 1,
                                ReverseComplementorDna(suffix(ref.Variants(ref_seq_id).at(cur_var).var_seq_, 1)));
                            pos_shift -= length(ref.Variants(ref_seq_id).at(cur_var).var_seq_) - 1;
                        }
                    } else {
                        if (sur_pos + 1 < Surrounding::Length()) {
                            mod_surrounding.InsertSurroundingBasesShiftingOnRightSide(
                                sur_pos + 1, suffix(ref.Variants(ref_seq_id).at(cur_var).var_seq_, 1));
                            pos_shift += length(ref.Variants(ref_seq_id).at(cur_var).var_seq_) - 1;
                        }
                    }
                }
...
void Simulator::VariantModStartSurrounding(VariantBiasVarModifiers& bias_mod, uintAlleleId allele,
                                           uintRefSeqId ref_seq_id, const Reference& ref, uintSeqLen cur_start_position,
                                           const Surrounding& surrounding_start) const {
    // Variants in the roll around are ignored for variant modifications of surrounding (but to fill in deletions still
    // use the proper roll around base) Copy reference surrounding into allele
    ...
    if (ref.Variants(ref_seq_id).size()) {
        ...
        if (bias_mod.start_variant_pos_) {
            ...
            bias_mod.surrounding_start_.at(allele).ChangeSurroundingBase(
                pos_shift, at(ref.Variants(ref_seq_id).at(cur_var).var_seq_, 0));
            // Partial insertion (as the rest is shifted to the right)
            bias_mod.surrounding_start_.at(allele).InsertSurroundingBasesShiftingOnLeftSide(
                pos_shift, infix(ref.Variants(ref_seq_id).at(cur_var).var_seq_, 1, bias_mod.start_variant_pos_ + 1));
            pos_shift -= bias_mod.start_variant_pos_;
        }

        HandleSurroundingVariantsBeforeCenter(bias_mod.surrounding_start_.at(allele), cur_start_position, pos_shift,
                                              cur_var, allele, ref_seq_id, ref, false);

        // Handle variants after center (Shifting to/from the right)
        pos_shift = Surrounding::kStartPos;
        cur_var = bias_mod.first_variant_id_;
        if (bias_mod.start_variant_pos_) {
            if (length(ref.Variants(ref_seq_id).at(bias_mod.first_variant_id_).var_seq_) >
                bias_mod.start_variant_pos_ + 1) {
                // Partial insertion (as the rest is shifted to the left)
                if (pos_shift + 1 < Surrounding::Length()) {
                    bias_mod.surrounding_start_.at(allele).InsertSurroundingBasesShiftingOnRightSide(
                        pos_shift + 1, suffix(ref.Variants(ref_seq_id).at(bias_mod.first_variant_id_).var_seq_,
                                              bias_mod.start_variant_pos_ + 1));
                    pos_shift += length(ref.Variants(ref_seq_id).at(bias_mod.first_variant_id_).var_seq_) -
                                 bias_mod.start_variant_pos_ - 1;
                }
            }
        ...
        HandleSurroundingVariantsAfterCenter(bias_mod.surrounding_start_.at(allele), cur_start_position, pos_shift,
                                             cur_var, allele, ref_seq_id, ref, false);
    }
}

void Simulator::PrepareBiasModForCurrentStartPos(VariantBiasVarModifiers& bias_mod, uintRefSeqId ref_seq_id,
                                                 const Reference& ref, uintSeqLen cur_start_position,
    ...
    if (ref.VariantsLoaded()) {
        ...
        if (bias_mod.start_variant_pos_) {
            ...
            // Add GC from variant
            for (uintSeqLen pos = bias_mod.start_variant_pos_;
                 pos < length(ref.Variants(ref_seq_id).at(bias_mod.first_variant_id_).var_seq_) &&
                 ref.Variants(ref_seq_id).at(bias_mod.first_variant_id_).position_ + pos - bias_mod.start_variant_pos_ <
                     cur_end_position;
                 ++pos) {
            ...
            bias_mod.end_pos_shift_.at(0) =
                static_cast<intSeqShift>(1) -
                static_cast<intSeqShift>(min(
                    cur_end_position - cur_start_position,
                    static_cast<uintSeqLen>(length(ref.Variants(ref_seq_id).at(bias_mod.first_variant_id_).var_seq_)) -
                        bias_mod.start_variant_pos_));
        ...
        for (uintAlleleId allele = 0; allele < ref.NumAlleles(); ++allele) {
            if (!AlleleSkipped(bias_mod, allele, ref.Variants(ref_seq_id), cur_start_position)) {
                VariantModStartSurrounding(
                    bias_mod, allele, ref_seq_id, ref, cur_start_position,
                    surrounding_start); // Must be before HandleGCModAndEndPosShiftForNewVariants, so that
                                        // bias_mod.unhandled_variant_id_ is untouched
...
void Simulator::VariantModEndSurrounding(VariantBiasVarModifiers& bias_mod, uintAlleleId allele,
                                         uintRefSeqId ref_seq_id, const Reference& ref,
                                         uintSeqLen last_position) const {
    // Variants in the roll around are ignored for variant modifications of surrounding (but to fill in deletions still
    // use the proper roll around base) Copy reference surrounding into allele if it doesn't need to much shifting
    // afterwards
    if (ref.VariantsLoaded() && ref.Variants(ref_seq_id).size()) {
        // Handle variants after center (Shifting to/from the left)
        intSeqShift pos_shift = Surrounding::kStartPos;
        auto cur_var = bias_mod.unhandled_variant_id_.at(
            allele); // At the variants where pos == last_position if exists or after it otherwise
        if (bias_mod.unhandled_bases_in_variant_.at(allele)) {
            // Partial insertion (as the rest is shifted to the right)
            bias_mod.surrounding_end_.at(allele).InsertSurroundingBasesShiftingOnLeftSide(
                pos_shift, ReverseComplementorDna(suffix(ref.Variants(ref_seq_id).at(cur_var).var_seq_,
                                                         length(ref.Variants(ref_seq_id).at(cur_var).var_seq_) -
                                                             bias_mod.unhandled_bases_in_variant_.at(allele))));
            ...
            ++cur_var; // This insertion is handled, do not handle at again in HandleSurroundingVariantsAfterCenter
                   ...
                   ref.Variants(ref_seq_id).at(bias_mod.first_variant_id_).position_ == last_position &&
                   length(ref.Variants(ref_seq_id).at(bias_mod.first_variant_id_).var_seq_) >
                       bias_mod.start_variant_pos_ - bias_mod.end_pos_shift_.at(allele) + 1) {
            if (pos_shift) {
                // Partial insertion (as the rest is shifted to the right)
                bias_mod.surrounding_end_.at(allele).InsertSurroundingBasesShiftingOnLeftSide(
                    pos_shift - 1, ReverseComplementorDna(
                                       suffix(ref.Variants(ref_seq_id).at(bias_mod.first_variant_id_).var_seq_,
                                              bias_mod.start_variant_pos_ - bias_mod.end_pos_shift_.at(allele) + 1)));
                pos_shift -= length(ref.Variants(ref_seq_id).at(bias_mod.first_variant_id_).var_seq_) -
                             (bias_mod.start_variant_pos_ - bias_mod.end_pos_shift_.at(allele) + 1);
            }
        ...
        HandleSurroundingVariantsAfterCenter(bias_mod.surrounding_end_.at(allele), last_position, pos_shift, cur_var,
                                             allele, ref_seq_id, ref, true);

        // Handle variants before center (Shifting to/from the right)
        pos_shift = Surrounding::kStartPos;
        cur_var = bias_mod.unhandled_variant_id_.at(
            allele); // At the variants where pos == last_position if exists or after it otherwise
        if (bias_mod.unhandled_bases_in_variant_.at(allele)) {
            if (pos_shift + 1 < Surrounding::Length()) {
                // Handle the base substitution (as the insertion might include it)
                bias_mod.surrounding_end_.at(allele).ChangeSurroundingBase(
                    pos_shift + 1, Complement::Dna(at(ref.Variants(ref_seq_id).at(cur_var).var_seq_, 0)));

                // Partial insertion (as the rest is shifted to the left)
                bias_mod.surrounding_end_.at(allele).InsertSurroundingBasesShiftingOnRightSide(
                    pos_shift + 1, ReverseComplementorDna(infix(ref.Variants(ref_seq_id).at(cur_var).var_seq_, 1,
                                                                length(ref.Variants(ref_seq_id).at(cur_var).var_seq_) -
                                                                    bias_mod.unhandled_bases_in_variant_.at(allele))));
                pos_shift += length(ref.Variants(ref_seq_id).at(cur_var).var_seq_) -
                             bias_mod.unhandled_bases_in_variant_.at(allele) - 1;
            }
        } else if (bias_mod.start_variant_pos_ &&
                   ref.Variants(ref_seq_id).at(bias_mod.first_variant_id_).position_ == last_position) {
            cur_var = bias_mod.first_variant_id_;
            // Handle the base substitution (as the insertion might include it)
            bias_mod.surrounding_end_.at(allele).ChangeSurroundingBase(
                pos_shift, Complement::Dna(at(ref.Variants(ref_seq_id).at(cur_var).var_seq_, 0)));

            // Partial insertion (as the rest is shifted to the left)
            bias_mod.surrounding_end_.at(allele).InsertSurroundingBasesShiftingOnRightSide(
                pos_shift,
                ReverseComplementorDna(infix(ref.Variants(ref_seq_id).at(cur_var).var_seq_, 1,
                                             bias_mod.start_variant_pos_ - bias_mod.end_pos_shift_.at(allele) + 1)));
            pos_shift += bias_mod.start_variant_pos_ - bias_mod.end_pos_shift_.at(allele);
        }

        HandleSurroundingVariantsBeforeCenter(bias_mod.surrounding_end_.at(allele), last_position, pos_shift, cur_var,
                                              allele, ref_seq_id, ref, true);
    }
}

void Simulator::UpdateBiasModForCurrentFragmentLength(VariantBiasVarModifiers& bias_mod, uintRefSeqId ref_seq_id,
                                                      const Reference& ref, uintSeqLen cur_start_position,
                                                      uintSeqLen cur_end_position, uintSeqLen last_end_position,
    ...
    if (ref.VariantsLoaded() && cur_end_position > bias_mod.last_end_position_.at(allele)) {
        ...
        if (bias_mod.start_variant_pos_ &&
            last_end_position + 1 - cur_start_position <=
                length(ref.Variants(ref_seq_id).at(bias_mod.first_variant_id_).var_seq_) -
                    bias_mod.start_variant_pos_) {
...
inline void Simulator::PrepareEndSurroundingsForCurrentFragmentLength(VariantBiasVarModifiers& bias_mod,
                                                                      uintRefSeqId ref_seq_id, const Reference& ref,
...
void Simulator::PrepareBiasModForCurrentFragmentLength(VariantBiasVarModifiers& bias_mod, uintRefSeqId ref_seq_id,
                                                       const Reference& ref, uintSeqLen cur_start_position,
                                                       uintSeqLen fragment_length, uintAlleleId allele) const {
    auto cur_end_position = cur_start_position + fragment_length - 1;
    if (bias_mod.last_end_position_.at(allele) <=
        cur_end_position) { // Do not repeat this in case we run both strands of an allele at a given position, because
                            // it would mess up the order of PrepareEndSurroundingsForCurrentFragmentLength and
                            // UpdateBiasModForCurrentFragmentLength
        UpdateBiasModForCurrentFragmentLength(bias_mod, ref_seq_id, ref, cur_start_position, cur_end_position,
                                              bias_mod.last_end_position_.at(allele), allele);

        if (bias_mod.start_variant_pos_ &&
            fragment_length <= length(ref.Variants(ref_seq_id).at(bias_mod.first_variant_id_).var_seq_) -
                                   bias_mod.start_variant_pos_) {
            UpdateBiasModForCurrentFragmentLength(bias_mod, ref_seq_id, ref, cur_start_position, cur_end_position + 1,
                                                  cur_end_position, allele);
...
reseq::uintPercent Simulator::GetGCPercent(VariantBiasVarModifiers& bias_mod, uintRefSeqId ref_seq_id,
                                           const Reference& ref, uintSeqLen cur_end_position,
                                           uintSeqLen fragment_length, uintAlleleId allele) {
    if (cur_end_position < bias_mod.last_gc_end_) {
        return Percent(bias_mod.last_gc_ - ref.GCContentAbsolut(ref_seq_id, cur_end_position, bias_mod.last_gc_end_) +
                           bias_mod.gc_mod_.at(allele),
                       fragment_length);
    } else {
...
void Simulator::CheckForInsertedBasesToStartFrom(VariantBiasVarModifiers& bias_mod, uintRefSeqId ref_seq_id,
                                                 uintSeqLen cur_start_position, const Reference& ref) const {
    if (ref.VariantsLoaded() && bias_mod.first_variant_id_ < ref.Variants(ref_seq_id).size() &&
        ref.Variants(ref_seq_id).at(bias_mod.first_variant_id_).position_ == cur_start_position) {
        if (bias_mod.start_variant_pos_) {
            if (++bias_mod.start_variant_pos_ >=
                length(ref.Variants(ref_seq_id).at(bias_mod.first_variant_id_).var_seq_)) {
            ...
            while (bias_mod.first_variant_id_ < ref.Variants(ref_seq_id).size() &&
                   ref.Variants(ref_seq_id).at(bias_mod.first_variant_id_).position_ == cur_start_position &&
                   2 > length(ref.Variants(ref_seq_id).at(bias_mod.first_variant_id_).var_seq_)) {
                ++bias_mod.first_variant_id_; // All deletions and substitutions are handled with the first variant of
                                              // the position
            }
        }

        if (0 == bias_mod.start_variant_pos_) {
            if (bias_mod.first_variant_id_ < ref.Variants(ref_seq_id).size() &&
                ref.Variants(ref_seq_id).at(bias_mod.first_variant_id_).position_ == cur_start_position) {
                // We have an insertion to handle
                bias_mod.start_variant_pos_ = 1; // Starting from the first base is handled during the first pass of the
                                                 // while loop, so now we use the second, third, ... as start positions
            }
        }
    }
}

void Simulator::GetOrgSeq(SimPair& sim_reads, bool strand, uintAlleleId allele, uintSeqLen fragment_length,
                          uintSeqLen cur_start_position, uintSeqLen cur_end_position, uintRefSeqId ref_seq_id,
                          const Reference& ref, const DataStats& stats, const VariantBiasVarModifiers& bias_mod) const {
    if (ref.VariantsLoaded()) {
        // Forward
        ref.ReferenceSequence(sim_reads.at(strand).org_seq_, ref_seq_id, cur_start_position,
                              min(fragment_length, static_cast<uintSeqLen>(stats.ReadLengths(strand).to() +
                                                                           stats.Errors().MaxLenDeletion())),
                              false, ref.Variants(ref_seq_id), bias_mod.StartVariant(), allele);

        // Reverse
        ref.ReferenceSequence(sim_reads.at(!strand).org_seq_, ref_seq_id, cur_end_position,
                              min(fragment_length, static_cast<uintSeqLen>(stats.ReadLengths(!strand).to() +
                                                                           stats.Errors().MaxLenDeletion())),
                              true, ref.Variants(ref_seq_id),
                              bias_mod.EndVariant(ref.Variants(ref_seq_id), cur_end_position, allele), allele);
    } else {
        // Forward
        ref.ReferenceSequence(sim_reads.at(strand).org_seq_, ref_seq_id, cur_start_position,
                              min(fragment_length, static_cast<uintSeqLen>(stats.ReadLengths(strand).to() +
...
void Simulator::CTConversion(DnaString& read, const Reference& ref, uintRefSeqId seq_id, uintSeqLen start_pos,
                             uintAlleleId allele, intVariantId cur_methylation_start, GeneralRandomDistributions& rdist,
                             mt19937_64& rgen, bool reversed) const {
...
void Simulator::CTConversion(DnaString& read, const Reference& ref, uintRefSeqId seq_id, uintSeqLen start_pos,
                             uintAlleleId allele, intVariantId cur_methylation_start, GeneralRandomDistributions& rdist,
                             mt19937_64& rgen, bool reversed, const vector<Reference::Variant>& variants,
    ...
    if (reversed) {
        ...
        if (0 <= cur_var && variants.at(cur_var).position_ == ref_pos && 1 < length(variants.at(cur_var).var_seq_) &&
            variants.at(cur_var).InAllele(allele)) {
        ...
        if (0 <= cur_meth && regions.at(cur_meth).second <= ref_pos) {
            ...
            while (0 <= cur_var && regions.at(cur_meth).second <= variants.at(cur_var).position_ &&
                   read_pos < length(read)) {
                if (variants.at(cur_var).InAllele(allele)) {
                    ...
                    ref_pos = variants.at(cur_var).position_ -
                              1; // Go one before the variant, because we handle the variant here, and it is only at
                                 // exactly one ref_pos, so cannot reach into unmethylated region
                }
                --cur_var;
            }
        ...
        while (0 <= cur_meth && read_pos < length(read)) {
            ...
            while (ref_pos >= regions.at(cur_meth).first && read_pos < length(read)) {
                if (0 == var_bases_left) {
                    while (0 <= cur_var && variants.at(cur_var).position_ == ref_pos &&
                           !variants.at(cur_var).InAllele(allele)) {
                        --cur_var;
                    }
        ...
        if (cur_var < variants.size() && variants.at(cur_var).position_ == ref_pos &&
            1 < length(variants.at(cur_var).var_seq_) && variants.at(cur_var).InAllele(allele)) {
            // We cannot start at a deletions and we don't care about substitutions, so only handle insertions
            var_bases_left = length(variants.at(cur_var).var_seq_) - first_variant.second;
        }
        ...
        if (cur_meth < regions.size() && regions.at(cur_meth).first > ref_pos) {
            ...
            // Handle all variants before the next unmethylated region
            while (cur_var < variants.size() && regions.at(cur_meth).first > variants.at(cur_var).position_ &&
                   read_pos < length(read)) {
                if (variants.at(cur_var).InAllele(allele)) {
                    read_pos += variants.at(cur_var).position_ - ref_pos;
                    read_pos += length(variants.at(cur_var).var_seq_);
                    ref_pos = variants.at(cur_var).position_ +
                              1; // Go one after the variant, because we handle the variant here, and it is only at
                                 // exactly one ref_pos, so cannot reach into unmethylated region
                }
                ++cur_var;
            }
        ...
        while (cur_meth < regions.size() && read_pos < length(read)) {
            ...
            while (ref_pos < regions.at(cur_meth).second && read_pos < length(read)) {
                if (0 == var_bases_left) {
                    while (cur_var < variants.size() && variants.at(cur_var).position_ == ref_pos &&
                           !variants.at(cur_var).InAllele(allele)) {
                        ++cur_var;
                    }
...
void Simulator::CTConversion(SimPair& sim_reads, bool strand, uintAlleleId allele, uintSeqLen cur_start_position,
                             uintSeqLen cur_end_position, uintRefSeqId ref_seq_id, const Reference& ref,
                             const VariantBiasVarModifiers& bias_mod, intVariantId cur_methylation_start,
                             GeneralRandomDistributions& rdist, mt19937_64& rgen) const {
    if (ref.MethylationLoaded()) {
        if (ref.VariantsLoaded()) {
            // Forward
            CTConversion(sim_reads.at(strand).org_seq_, ref, ref_seq_id, cur_start_position, allele,
                         cur_methylation_start, rdist, rgen, false, ref.Variants(ref_seq_id), bias_mod.StartVariant());

            // Reverse
            CTConversion(sim_reads.at(!strand).org_seq_, ref, ref_seq_id, cur_end_position, allele,
                         cur_methylation_start, rdist, rgen, true, ref.Variants(ref_seq_id),
                         bias_mod.EndVariant(ref.Variants(ref_seq_id), cur_end_position, allele));
        } else {
            // Forward
            CTConversion(sim_reads.at(strand).org_seq_, ref, ref_seq_id, cur_start_position, allele,
                         cur_methylation_start, rdist, rgen, false);

            // Reverse
            CTConversion(sim_reads.at(!strand).org_seq_, ref, ref_seq_id, cur_end_position, allele,
                         cur_methylation_start, rdist, rgen, true);
        }
    }
}

bool Simulator::SimulateFromGivenBlock(const SimBlock& block, // forward block
                                       const SimUnit& unit, const Reference& ref, const DataStats& stats,
                                       const ProbabilityEstimates& estimates, GeneralRandomDistributions& rdist,
                                       mt19937_64& rgen) {
    ...
    for (auto cur_start_position = block.start_pos_; cur_start_position < block.start_pos_ + kBlockSize &&
        ...
        if (ref.MethylationLoaded()) {
            if (cur_methylation_start < ref.UnmethylatedRegions(unit.ref_seq_id_).size() &&
                ref.UnmethylatedRegions(unit.ref_seq_id_).at(cur_methylation_start).second <= cur_start_position) {
                ++cur_methylation_start;
            }
        }

        do { // while(bias_mod.start_variant_pos_)
            auto frag_len_start = max(static_cast<uintSeqLen>(1),
                                      static_cast<uintSeqLen>(stats.FragmentDistribution().InsertLengths().from()));
            PrepareBiasModForCurrentStartPos(bias_mod, unit.ref_seq_id_, ref, cur_start_position, frag_len_start,
                                             surrounding_start);
            GetPossibleAlleles(possible_alleles, ref, bias_mod, cur_start_position, unit.ref_seq_id_);

            for (auto fragment_length = frag_len_start;
                 fragment_length < stats.FragmentDistribution().InsertLengths().to(); ++fragment_length) {
                ...
                if (ProbabilityAboveThreshold(probability_chosen, unit.ref_seq_id_, fragment_length)) {
                    ...
                    if (non_zero_strands) {
                        ...
                        for (auto chosen_id : chosen_allele_ids) {
                            ...
                            PrepareBiasModForCurrentFragmentLength(bias_mod, unit.ref_seq_id_, ref, cur_start_position,
                            ...
                            if (cur_end_position <= ref.SequenceLength(unit.ref_seq_id_)) {
                                ...
                                // Determine how many read pairs are generated for this strand and allele at this
                                // position with this fragment_length
                                double adjusted_random =
                                    NonZeroThreshold(unit.ref_seq_id_, fragment_length) +
                                    rdist.ZeroToOne(rgen) *
                                        (1 -
                                         NonZeroThreshold(unit.ref_seq_id_,
                                                          fragment_length)); // Adjust the uniform value to account for
                                                                             // the previous binomial selection only
                                                                             // alleles that are above NonZeroThreshold
                                if (ref.VariantsLoaded()) {
                                    fragment_counts = stats.FragmentDistribution().GetFragmentCounts(
                                        bias_normalization_, unit.ref_seq_id_, fragment_length, gc_perc,
                                        bias_mod.surrounding_start_.at(allele), bias_mod.surrounding_end_.at(allele),
                                        adjusted_random, ref.NumAlleles());
                                } else {
                                    fragment_counts = stats.FragmentDistribution().GetFragmentCounts(
                                        bias_normalization_, unit.ref_seq_id_, fragment_length, gc_perc,
                                        surrounding_start, bias_mod.surrounding_end_.at(allele), adjusted_random);
                                }

                                if (fragment_counts) {
                                    GetOrgSeq(sim_reads, strand, allele, fragment_length, cur_start_position,
                                              cur_end_position, unit.ref_seq_id_, ref, stats, bias_mod);

                                    CTConversion(sim_reads, strand, allele, cur_start_position, cur_end_position,
                                                 unit.ref_seq_id_, ref, bias_mod, cur_methylation_start, rdist, rgen);

                                    if (ref.VariantsLoaded()) {
                                        if (!CreateReads(ref, stats, estimates, rdist, sim_reads, rgen, fragment_counts,
                                                         strand, allele, unit.ref_seq_id_, fragment_length, read_number,
                                                         &block, cur_start_position, cur_end_position,
                                                         bias_mod.StartVariant(),
                                                         bias_mod.EndVariant(ref.Variants(unit.ref_seq_id_),
                                                                             cur_end_position, allele))) {
                                            return false;
                                        }
                                    } else {
...
bool Simulator::SimulateAdapterOnlyPairs(const Reference& ref, const DataStats& stats,
                                         const ProbabilityEstimates& estimates, GeneralRandomDistributions& rdist,
                                         mt19937_64& rgen) {
...
void Simulator::SimulationThread(Simulator& self, Reference& ref, const DataStats& stats,
    ...
    // reproducible seeds to each block and the adapters
...
bool Simulator::ApplyErrorsAndQualityToFastaInput(StringSet<CharString>& input_ids, StringSet<DnaString>& input_seqs,
                                                  StringSet<CharString>& output_ids, StringSet<Dna5String>& output_seqs,
    ...
    for (uintFragCount i = 0; i < length(input_ids); ++i) {
        ...
        if (length(at(input_ids, i)) <= 2 * length(sim_read.org_seq_) + 2) {
            printErr << "Read description is too short to contain systematic error information and a sequence id: "
                     << at(input_ids, i) << std::endl;
            simulation_error_ = true;
            return false;
        }

        auto end_pos_output_id = length(at(input_ids, i)) - 2 * length(sim_read.org_seq_) -
                                 3; // Subtract the two entries for systematic errors of same length as the sequence and
                                    // the two ; separators and go to last position instead of end
        if (';' != at(at(input_ids, i), end_pos_output_id + 1) ||
            ';' != at(at(input_ids, i), end_pos_output_id + 2 + length(sim_read.org_seq_))) {
            printErr << "The two systematic error entries are not separated by a semicolon from themselves or the rest "
                        "of the ReSeq information: "
                     << at(input_ids, i) << std::endl;
            simulation_error_ = true;
            return false;
        }
        ...
        for (auto pos = block.sys_errors_.size(); pos--;) {
            ...
            block.sys_errors_.at(pos).second =
                at(at(input_ids, i), length(at(input_ids, i)) - block.sys_errors_.size() + pos) - 33;
            if (86 < block.sys_errors_.at(pos).second) {
                block.sys_errors_.at(pos).second +=
                    block.sys_errors_.at(pos).second - 86; // Decompress percentages again (odd percentages over 86 are
                                                           // removed, as fastq can store only up to 94 quality values)
            }
        }
        ...
        if (0 == end_pos_output_id) {
            printErr << "No sequence id found that is separated by a space from the ReSeq information: "
                     << at(input_ids, i) << std::endl;
            simulation_error_ = true;
            return false;
        }
        ...
        if ('1' == at(at(input_ids, i), end_pos_output_id + 1)) {
            ...
            printErr << "Template segment is " << at(at(input_ids, i), end_pos_output_id + 1)
                     << " not 1 or 2: " << at(input_ids, i) << std::endl;
            simulation_error_ = true;
            return false;
        }

        if (';' != at(at(input_ids, i), end_pos_output_id + 2)) {
            printErr << "The template segment and fragment length are not separated by a semicolon: "
                     << at(input_ids, i) << std::endl;
            simulation_error_ = true;
            return false;
        }

        readid_stream.str(toCString(CharString(infix(at(input_ids, i), end_pos_output_id + 3,
                                                     length(at(input_ids, i)) - 2 * length(sim_read.org_seq_) - 2))));
        size_t pos;
        uintSeqLen fragment_length = stoi(readid_stream.str(), &pos);
        if (pos < readid_stream.str().size()) {
            printErr << "Fragment length '" << readid_stream.str() << "' is not a pure integer: " << at(input_ids, i)
                     << std::endl;
            simulation_error_ = true;
            return false;
        }
        ...
        uintReadLen num_errors(0);
        if (!FillRead(sim_read, num_errors, template_segment, tile_id, fragment_length, &block, 0, {0, 0}, 0, stats,
                      estimates, rdist, rgen)) {
            return false;
        }
...
void Simulator::ErrorModelOnlyThread(Simulator& self, SeqFileIn& org_seq_reader, const DataStats& stats,
                                     const ProbabilityEstimates& estimates) {
    mt19937_64 rgen;
    GeneralRandomDistributions rdist(stats);
    ...
    while (keep_running && !self.simulation_error_) {
        ...
        if (keep_running) {
            ...
            if (self.ApplyErrorsAndQualityToFastaInput(input_ids, input_seqs, output_ids, output_seqs, output_quals,
                                                       rdist, rgen, stats, estimates)) {
...
bool Simulator::WriteOutSystematicErrorProfile(const string& id, vector<pair<Dna5, uintPercent>> sys_errors,
                                               Dna5String dom_err, CharString err_perc) {
    ...
    for (uintSeqLen pos = 0; pos < sys_errors.size(); ++pos) {
        ...
        if (86 < q_perc) {
            // Shorten percentage that it fits into 94 (0-93) values (odd values above 86 are joined with the even value
            // before, so e.g. 87->86)
            q_perc -= (q_perc - 85) / 2;
        }
...
bool Simulator::CreateSystematicErrorProfile(const char* destination_file, const Reference& ref, const DataStats& stats,
    ...
    if (ref.HasN()) {
        printErr << "Reference contains ambiguous bases(e.g. N). Please replace them, remove them from the scaffolds "
                    "or split scaffolds into contigs."
                 << std::endl;
        return false;
    } else {
        ...
        if (!open(dest_.at(0), destination_file)) {
            ...
            for (uintRefSeqId ref_id = 0; ref_id < ref.NumberSequences(); ++ref_id) {
                // Get sytematic error for reverse strand (error is reversed, last error at first position, this is how
                // it will be needed for simulation)
                ResetSystematicErrorCounters(ref);
                sys_errors.clear();
                ConstDna5StringReverseComplement reversed_ref(ref.ReferenceSequence(ref_id));
                SetSystematicErrors(sys_errors, reversed_ref, 0, ref.SequenceLength(ref_id), stats, estimates);
                if (!WriteOutSystematicErrorProfile(string(toCString(ref.ReferenceId(ref_id))) + " reverse", sys_errors,
                                                    dom_err, err_perc)) {
                    close(dest_.at(0));
                    return false;
                }
                ...
                ResetSystematicErrorCounters(ref);
                sys_errors.clear();
                SetSystematicErrors(sys_errors, ref.ReferenceSequence(ref_id), 0, ref.SequenceLength(ref_id), stats,
                                    estimates);
                if (!WriteOutSystematicErrorProfile(string(toCString(ref.ReferenceId(ref_id))) + " forward", sys_errors,
                                                    dom_err, err_perc)) {
                    close(dest_.at(0));
                    return false;
                }
...
bool Simulator::Simulate(const char* destination_file_first, const char* destination_file_second, Reference& ref,
                         DataStats& stats, const ProbabilityEstimates& estimates, uintNumThreads num_threads,
                         uintSeed seed, uintFragCount num_read_pairs, double coverage,
                         RefSeqBiasSimulation ref_bias_model, const string& ref_bias_file, const string& sys_error_file,
                         const string& record_base_identifier, const string& var_file, const string& meth_file) {
    // create all missing directories
    ...
    if (!open(dest_.at(0), destination_file_first)) {
        ...
        if (stats.FragmentDistribution().UpdateRefSeqBias(ref_bias_model, ref_bias_file, ref, block_seed_gen_)) {
            ...
            printInfo << "Aiming for " << total_pairs_
                      ...
                      << NumberPairsToCoverage(total_pairs_, total_ref_size, average_read_length, adapter_part)
                      << setprecision(6) << "x" << std::endl;
            ...
            if (!simulation_error_) {
                bias_normalization_ = stats.FragmentDistribution().CalculateBiasNormalization(
                    coverage_groups_, non_zero_thresholds_, ref, num_threads, total_pairs_);
                ...
                if (0.0 == bias_normalization_) {
                    ...
                    for (auto template_segment = kTemplateSegments; template_segment--;) {
                        ...
                        adapter_sys_error_.at(template_segment)
                            .resize(stats.Adapters().Counts(template_segment).size());

                        for (auto seq = stats.Adapters().Counts(template_segment).size(); seq--;) {
                            if (stats.Adapters()
                                    .Counts(template_segment)
                                    .at(seq)) { // If the adapter does not appear, we don't need it
                                ResetSystematicErrorCounters(ref);

                                adapter_sys_error_.at(template_segment)
                                    .at(seq)
                                    .reserve(length(stats.Adapters().Sequence(template_segment, seq)));

                                SetSystematicErrors(adapter_sys_error_.at(template_segment).at(seq),
                                                    stats.Adapters().Sequence(template_segment, seq), 0,
                                                    length(stats.Adapters().Sequence(template_segment, seq)), stats,
                                                    estimates);
                            }
                        }
                    }
    ...
    if (simulation_error_) {
        // Remove the simulated files so it is clear that we encountered an error and do not accidentally continue with
        // the old file
...
bool Simulator::SimulateErrorModelOnly(const string& destination_file, const string& org_seq_file, DataStats& stats,
                                       const ProbabilityEstimates& estimates, uintNumThreads num_threads,
                                       uintSeed seed) {
    // create all missing directories
    ...
    for (auto template_segment = kTemplateSegments; template_segment--;) {
        ...
        for (auto seq = stats.Adapters().Counts(template_segment).size(); seq--;) {
            if (stats.Adapters().Counts(template_segment).at(seq)) { // If the adapter does not appear, we don't need it
                ...
                SetSystematicErrors(adapter_sys_error_.at(template_segment).at(seq),
                                    stats.Adapters().Sequence(template_segment, seq), 0,
                                    length(stats.Adapters().Sequence(template_segment, seq)), stats, estimates);
            }
        }
    }
