...
TEST(VectTest, BasicFunctionality) {
    Vect<uint32_t> test;
    EXPECT_EQ(0, test.from()) << "Offset not properly initialized in class 'Vect'.\n";
    EXPECT_TRUE(test.empty()) << "Std::vector not properly initialized in class 'Vect'.\n";
    ...
    test[5] = 15;
    EXPECT_EQ(6, test.size()) << "Error assuring correct size at assessing element in class 'Vect'.\n";
    EXPECT_EQ(15, test[5]) << "Error setting element in class 'Vect'.\n";

    test.SetOffset(6);
    EXPECT_EQ(1, test.size()) << "Error assuring correct size at setting offset in class 'Vect'.\n";
    EXPECT_EQ(5, test.from()) << "Error setting highest possible offset in class 'Vect'.\n";
    EXPECT_TRUE(1 == test) << "Equality operator for class 'Vect' had wrong result for non-empty vec_.\n";

    test[7] = 25;
    EXPECT_EQ(3, test.size())
        << "Error assuring correct size at assessing element after setting offset in class 'Vect'.\n";
    EXPECT_EQ(25, test[7]) << "Error setting element after setting offset in class 'Vect'.\n";
    ...
    EXPECT_EQ(5, test.from()) << "Offset not correct after shrinking for class 'Vect'.\n";
    ...
    EXPECT_EQ(3, test.from()) << "Offset not correct after set function.\n";

    test.Shift(2);
    EXPECT_EQ(10, test[5]) << "Positive shift did not work properly.\n";
    EXPECT_EQ(5, test.from()) << "Offset not correct after positive shift.\n";
    test.Shift(-1);
    EXPECT_EQ(10, test[4]) << "Negative shift did not work properly.\n";
    EXPECT_EQ(4, test.from()) << "Offset not correct after negative shift.\n";
    test.Shift(-100);
...
TEST(VectTest, CopyAndClear) {
    // Prepare test Vect using public constructor (offset=3, values=[5,0,3])
    Vect<uint32_t> test(3, {5, 0, 3});
    EXPECT_EQ(3, test.from()) << "Offset not correctly set via constructor for class 'Vect'.\n";
    EXPECT_EQ(3, test.size()) << "Size not correctly set via constructor for class 'Vect'.\n";
    EXPECT_EQ(5, test[3]) << "Value not correctly set via constructor for class 'Vect'.\n";
    EXPECT_EQ(3, test[5]) << "Value not correctly set via constructor for class 'Vect'.\n";
    // Copy it
    Vect<uint32_t> test2(test);
    Vect<uint32_t> test3;
    test3 = test;
    EXPECT_EQ(test.size(), test3.size()) << "Size of copy is not identical for class 'Vect'.\n";
    for (auto i = test.from(); i < test.to(); ++i) {
    ...
    EXPECT_EQ(0, test.from()) << "Offset not properly reset for class 'Vect'.\n";
    ...
    for (auto i = test2.from(); i < test2.to(); ++i) {
...
TEST(VectTest, Offset) {
    ...
    EXPECT_EQ(6, test.size())
        << "Setting offset before assigning the first value does not result in correct size for class 'Vect'.\n";
    EXPECT_EQ(10, test.from())
        << "Setting offset before assigning the first value does not result in correct offset for class 'Vect'.\n";
