...
void TileStatsTest::TestProperTileFormat(const char* read_id_string, const string& format, uintTileId expected_id,
    ...
    EXPECT_EQ(expected_colon_number, test_->tile_colon_number_)
        << "tile_colon_number_ wrongly set for " + format + " format\n";
...
void TileStatsTest::TestProperTileFormatWithGivenColonNumber(const char* read_id_string, const string& format,
...
void TileStatsTest::TestErroneousTileFormat(const char* read_id_string, bool reset_accession_info,
                                            uintTileId expected_id, const string& comment,
    ...
    if (!test_->tile_accessible_) {
        EXPECT_TRUE(test_->GetTileId(tile_id, read_id_string)) << error_msg;
        EXPECT_EQ(0, tile_id)
            << "tile_accessible_ checking doesn't work properly\n"; // EnterTile might have set tile_accessible_ to
                                                                    // false and then we set tile_id simply to 0 in
                                                                    // GetTileId
...
TEST_F(TileStatsTest, TileIdentification) {
    ...
    TestErroneousTileFormat("EAS139:136:FC706VJ:2:2104:15343:197393 1:Y:18:ATCACG", false, 1,
                            "'GetTileId' did not notice tile id format change\n",
                            "The read id encoding changed and the tile could not be found at colon number 2 anymore. "
                            "Tile will be treated as 0: EAS139:136:FC706VJ:2:2104:15343:197393 "
    ...
    // Error detection
    TestErroneousTileFormat("SomeReadIdWithoutAColon", false, 1,
                            "Failed for read id without a colon with given colon number\n",
                            "The read id encoding changed and the id ended before colon number 2. Tile will be treated "
                            "as 0: SomeReadIdWithoutAColon\n");
    TestErroneousTileFormat("Some read id without a colon", true, 1,
                            "Failed for read id without a colon without given colon number\n",
                            "Exception: Premature end of read after first field.");
    TestErroneousTileFormat("EAS139:i36:FC706VJ:2:2104:15343:197393 1:Y:18:ATCACG", true, 1,
                            "Failed to notice non-integer second field\n",
                            "Exception: Failed to convert second field into an integer.");
    TestErroneousTileFormat("EAS139:136;FC706VJ:2:2104:15343:197393 1:Y:18:ATCACG", true, 1,
                            "Failed to notice wrong second delimiter\n",
                            "Exception: Wrong delimiter: ';' found instead of ':' at second occurrence.");
    TestErroneousTileFormat("SRR001666.1 071112_SLXA-EAS1_s_7:5:1J:817:345 length=36", true, 1,
    ...
    TestErroneousTileFormat("EAS139:136:FC706VJ:2:2104:153k43:197393 1:Y:18:ATCACG", true, 1,
    ...
    TestErroneousTileFormat("EAS139:136:FC706VJ:2:2104:15343.197393 1:Y:18:ATCACG", true, 1,
                            ...
                            "Exception: Wrong delimiter: '.' found instead of ':' at sixth occurrence.");
    TestErroneousTileFormat("SRR001666.1", true, 1, "Failed to notice premature end of read\n",
                            "Exception: Premature end of read after first field.");
    TestErroneousTileFormat("SRR001666.1:", true, 1, "Failed to notice premature end of read id with ':'-delimiter\n",
