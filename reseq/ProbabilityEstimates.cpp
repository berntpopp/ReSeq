...
template <uintMarginId N>
void DataStorage<N>::CombineEndBinsToMinCount(array<vector<uintMatrixIndex>, N>& dim_indices,
    ...
    for (uintMarginId cur_dim = 1; cur_dim < dim_control.size(); ++cur_dim) { // Do not do change first dimension!!!
        // Get the bin, where everything coming after will be added to (to reach kMinCountsPer1dBin) and update
        // dim_control in the process
        last_bin.at(cur_dim) = dim_control.at(cur_dim).to() - 1;
        while (dim_control.at(cur_dim).at(last_bin.at(cur_dim)) < kMinCountsPer1dBin &&
               last_bin.at(cur_dim) > dim_control.at(cur_dim).from()) {
        ...
        while (dim_control.at(cur_dim).at(first_bin.at(cur_dim)) < kMinCountsPer1dBin &&
               first_bin.at(cur_dim) < last_bin.at(cur_dim)) {
        ...
        dim_control.at(cur_dim)
            .SoftShrink(); // Space will be freed soon anyway as it is just a temporary variable, so don't do it here
        ...
        dim_indices.at(cur_dim).erase(dim_indices.at(cur_dim).begin(),
                                      dim_indices.at(cur_dim).begin() + first_bin.at(cur_dim));
    }

    // Reduce data_ to the new limits (This part does not have to be reverted later, as we anyways take the outermost
    // bins for everything that is further out)
    ...
    for (uintMarginId n = kNumMargins; n--;) {
        ...
        for (uintMatrixIndex ia = 0; ia < first_bin.at(dim_a); ++ia) {
            for (uintMatrixIndex ib = 0; ib < dim_size_.at(dim_b); ++ib) {
                data_.at(n).at(first_bin.at(dim_a) * dim_size_.at(dim_b) + ib) +=
                    data_.at(n).at(ia * dim_size_.at(dim_b) + ib);
            }
        }
        for (uintMatrixIndex ia = dim_size_.at(dim_a); --ia > last_bin.at(dim_a);) {
            for (uintMatrixIndex ib = 0; ib < dim_size_.at(dim_b); ++ib) {
                data_.at(n).at(last_bin.at(dim_a) * dim_size_.at(dim_b) + ib) +=
                    data_.at(n).at(ia * dim_size_.at(dim_b) + ib);
            }
        }
        // Add values from bins outside the new limits, to first or last bin (dim_b)
        for (uintMatrixIndex ib = 0; ib < first_bin.at(dim_b); ++ib) {
            for (uintMatrixIndex ia = first_bin.at(dim_a); ia <= last_bin.at(dim_a);
                 ++ia) { // We can ignore the other indexes as we already added their values
                data_.at(n).at(ia * dim_size_.at(dim_b) + first_bin.at(dim_b)) +=
                    data_.at(n).at(ia * dim_size_.at(dim_b) + ib);
            }
        }
        for (uintMatrixIndex ib = dim_size_.at(dim_b); --ib > last_bin.at(dim_b);) {
            for (uintMatrixIndex ia = first_bin.at(dim_a); ia <= last_bin.at(dim_a);
                 ++ia) { // We can ignore the other indexes as we already added their values
                data_.at(n).at(ia * dim_size_.at(dim_b) + last_bin.at(dim_b)) +=
                    data_.at(n).at(ia * dim_size_.at(dim_b) + ib);
            }
        }
        ...
        if (dim_indices.at(dim_b).size() != dim_size_.at(dim_b)) {
            // Inner dimension has changed, so instead of deleting small bits here and there, we start from the
            // beginning and copy the new value there and finally cut the rest
            for (uintMatrixIndex ia = 0; ia < dim_indices.at(dim_a).size(); ++ia) {
                for (uintMatrixIndex ib = 0; ib < dim_indices.at(dim_b).size(); ++ib) {
                    data_.at(n).at(ia * dim_indices.at(dim_b).size() + ib) =
                        data_.at(n).at((ia + first_bin.at(dim_a)) * dim_size_.at(dim_b) + ib + first_bin.at(dim_b));
                }
            }
...
template <uintMarginId N>
void DataStorage<N>::CombineBins(array<vector<uintMatrixIndex>, N>& initial_dim_indices_reduced,
                                 array<Vect<uintMatrixCount>, N>& dim_control) {
    ...
    for (uintMarginId cur_dim = 1; cur_dim < N; ++cur_dim) { // Do not do anything for first dimension
        // First and Last bin are above kMinCountsPer1dBin already from call to CombineEndBinsToMinCount, except if only
        // one element is left in the vector
        if (1 == dim_control.at(cur_dim).size()) {
            ...
            // Going from left to right combine bins with a count lower than kMinCountsPer1dBin with the neighbouring
            // bin that has fewer counts
            ...
            for (uintMatrixIndex old_bin = 0; old_bin < dim_control.at(cur_dim).size(); ++old_bin) {
                if (kMinCountsPer1dBin > dim_control.at(cur_dim).at(old_bin)) {
                    if (dim_control.at(cur_dim).at(old_bin + 1) >
                        dim_control.at(cur_dim).at(new_bin -
                                                   1)) { // old_bin+1 and new_bin-1 guaranteed to be valid as
                                                         // CombineEndBinsToMinCount called earlier makes sure first and
                                                         // last bin are above kMinCountsPer1dBin and don't end up here
                    ...
                    dim_control.at(cur_dim).at(new_bin) = dim_control.at(cur_dim).at(
                        old_bin); // Update control bin on the fly (Everything before new_bin is the new stuff,
                                  // everything after old_bin is still untouched and everything in between is junk)
...
template <uintMarginId N>
void DataStorage<N>::FillDiffMatrix(vector<double>& diff_matrix, uintMarginId dimension) const {
    // Fill diff_matrix
    diff_matrix.clear();
        ...
        if (dimension == dim_a || dimension == dim_b) {
            ...
            for (uintMatrixIndex ind1 = 0; ind1 < dim_size_.at(dimension); ind1++) {
                for (uintMatrixIndex ind2 = ind1 + 1; ind2 < dim_size_.at(dimension); ind2++) {
                    ...
                    diff_matrix.at(ind1 * dim_size_.at(dimension) + ind2) +=
                        sum_diff / dim_size_.at((dimension == dim_a ? dim_b : dim_a));
                }
            }
        }
    }
...
template <uintMarginId N>
double DataStorage<N>::MaxDiff(const vector<double>& diff_matrix, uintMatrixIndex combined_index,
                               vector<uintMatrixIndex>& same_combined_index,
                               const vector<uintMatrixIndex>& dim_indices_reduced,
                               const vector<uintMatrixIndex>& dim_indices_count) const {
    double max_diff(0.0);
        ...
        for (uintMatrixIndex current_index = dim_indices_reduced.size(); current_index--;) {
            if (combined_index == dim_indices_reduced.at(current_index)) {
                for (auto comparison_index : same_combined_index) {
                    // We go backwards so all comparison_index are higher than current_index which is important as
                    // diff_matrix is triangular
...
template <uintMarginId N>
bool DataStorage<N>::SetUp(const array<pair<const Vect<Vect<uintMatrixCount>>*, bool>, N*(N - 1) / 2>& margins,
                           const Vect<SeqQualityStats<uintMatrixCount>>* alternative_margin,
                           array<vector<uintMatrixIndex>, N>& dim_indices,
                           array<vector<uintMatrixIndex>, N>& initial_dim_indices_reduced, std::mutex& print_mutex) {
        ...
        for (uintMatrixIndex ia = 0; ia < dim_size_.at(dim_a); ++ia) {
            ...
            for (uintMatrixIndex ib = 0; ib < dim_size_.at(dim_b); ++ib) {
                ...
                if (margins.at(n).first) {
                    // Set data
                    data_.at(n).at(ia * dim_size_.at(dim_b) + ib) =
                        static_cast<double>((*margins.at(n).first).at(index_first)[index_second]) /
                        sum_matrix; // Second index might not exist for all first indeces
                    // Set control
                    ...
                    // Set data
                    data_.at(n).at(ia * dim_size_.at(dim_b) + ib) =
                        static_cast<double>((*alternative_margin).at(index_first)[index_second]) /
                        sum_matrix; // Second index might not exist for all first indeces
                    // Set control
    ...
    // Initial reduction of data that is only reverted after all the fitting (Reversion in LogIPF.FullExpansion):
    // Combining 1d-bins that have less than kMinCountsPer1dBin entries and reduce data so it has kMaxBinsPerDimension
    // (First dimension must not be changed, as this are the values we want to read out)
...
template <uintMarginId N>
void DataStorage<N>::Reduce(DataStorage<N>& reduced_data, const array<vector<uintMatrixIndex>, N>& dim_indices_reduced,
                            const array<vector<uintMatrixIndex>, N>& dim_indices_count) const {
    ...
    for (uintMarginId n = kNumMargins; n--;) {
        ...
        for (auto ind_a = dim_indices_reduced.at(dim_a).size(); ind_a--;) {
            for (auto ind_b = dim_indices_reduced.at(dim_b).size(); ind_b--;) {
                reduced_data.data_.at(n).at(dim_indices_reduced.at(dim_a).at(ind_a) * reduced_data.dim_size_.at(dim_b) +
                                            dim_indices_reduced.at(dim_b).at(ind_b)) +=
                    data_.at(n).at(ind_a * dim_size_.at(dim_b) + ind_b);
            }
        }
    }
}
...
template <uintMarginId N>
void DataStorage<N>::Reduce(DataStorage<N>& reduced_data, array<vector<uintMatrixIndex>, N>& dim_indices_reduced,
                            array<vector<uintMatrixIndex>, N>& dim_indices_count, uintMatrixIndex max_size_dim) const {
    ...
    for (auto dimension = N; dimension--;) {
        if (dim_size_.at(dimension) > max_size_dim) {
            ...
            dim_indices_reduced.at(dimension).resize(
                dim_size_.at(dimension),
                max_size_dim); // max_size_dim cannot be a valid index, as it is the highest reduced index+1
            ...
            while (seed_indeces.size() < max_size_dim) {
                // Add the next seed index by choosing the one with the highest minimal differences to all already
                // chosen seed values
                max_value = 0.0;

                for (auto ind = dim_size_.at(dimension); ind--;) {
                    if (max_size_dim ==
                        dim_indices_reduced.at(dimension).at(
                            ind)) { // No reduced index has been assigned yet, so it is not a seed index already
                        ...
                        for (auto red_ind = seed_indeces.size(); red_ind--;) {
                            if (ind > seed_indeces.at(red_ind)) {
                                ...
                                current_value =
                                    diff_matrix.at(seed_indeces.at(red_ind) * dim_size_.at(dimension) + ind);
                            } else {
                                // Entry in triangular matrix is [ind][seed]
                                current_value =
                                    diff_matrix.at(ind * dim_size_.at(dimension) + seed_indeces.at(red_ind));
                            }
            ...
            dim_indices_count.at(dimension).reserve(
                dim_size_.at(dimension)); // It will become this size after expansions
            ...
            for (auto ind = dim_size_.at(dimension); ind--;) {
                if (max_size_dim ==
                    dim_indices_reduced.at(dimension).at(
                        ind)) { // No reduced index has been assigned yet, so it is not one of the seed indeces
...
template <uintMarginId N>
bool DataStorage<N>::Expand(DataStorage<N>& reduced_data, array<vector<uintMatrixIndex>, N>& dim_indices_reduced,
                            array<vector<uintMatrixIndex>, N>& dim_indices_count,
                            array<vector<uintMatrixIndex>, N>& expansion_indices,
                            array<vector<uintMatrixIndex>, N>& expansion_count,
                            uintMatrixIndex max_duplications) const {
    // Check need of duplications
    uintMatrixIndex duplications_needed(0);
    array<uintMatrixIndex, N> dim_new_size;

    for (auto dimension = N; dimension--;) {
        for (auto tmp_value = dim_indices_count.at(dimension).size(); tmp_value < dim_size_.at(dimension);
             tmp_value *= 2) {
            ++duplications_needed;
        }
            ...
            for (auto dimension = N; dimension--;) {
                if (dim_new_size.at(dimension) < dim_size_.at(dimension)) {
                    ...
                    if (N == dupl_dimension) { // No dimension needed duplication yet
                        ...
                        if (dim_new_size.at(dimension) < dim_new_size.at(dupl_dimension) ||
                            (dim_new_size.at(dimension) == dim_new_size.at(dupl_dimension) &&
                             dim_size_.at(dimension) > dim_size_.at(dupl_dimension))) {
                            dupl_dimension = dimension;
                        }
                    }
                }
            }
            dim_new_size.at(dupl_dimension) *=
                2; // We don't need to adjust it to the real size as the algorithm later doesn't care
            ++duplications;
        }
    } else {
    ...
    for (auto dimension = N; dimension--;) {
        if (dim_new_size.at(dimension) >= dim_size_.at(dimension)) {
            ...
            // Prepare new sizes
            expansion_indices.at(dimension).reserve(
                dim_size_.at(dimension)); // It will become this size after final expansions
            ...
            expansion_count.at(dimension).reserve(
                dim_size_.at(dimension)); // It will become this size after final expansions
            expansion_count.at(dimension) =
                dim_indices_count.at(dimension); // Store dim_indices_count, together with the modified
                                                 // dim_indices_count values can be adjusted proportionally
            ...
            for (uintMatrixIndex combined_index = max_diff.size(); combined_index--;) {
                max_diff.at(combined_index) =
                    MaxDiff(diff_matrix, combined_index, same_combined_index, dim_indices_reduced.at(dimension),
                            dim_indices_count.at(dimension));
            }
            ...
            while (max_diff.size() < dim_new_size.at(dimension)) {
                ...
                for (auto ind = max_diff.size(); --ind;) {
                    ...
                    if (max_diff.at(ind) > max_diff.at(split_index) ||
                        (max_diff.at(ind) == max_diff.at(split_index) &&
                         dim_indices_count.at(dimension).at(ind) > dim_indices_count.at(dimension).at(split_index))) {
                        split_index = ind;
                    }
                }
                ...
                same_combined_index.clear();
                for (uintMatrixIndex current_index = dim_size_.at(dimension);
                     current_index-- && split_ind1 == split_ind2;) {
                    if (split_index == dim_indices_reduced.at(dimension).at(current_index)) {
                        for (auto comparison_index : same_combined_index) {
                            // We go backwards so all comparison_index are higher than current_index which is important
                            // as diff_matrix is triangular
                            if (diff_matrix.at(current_index * dim_size_.at(dimension) + comparison_index) ==
                                max_diff.at(split_index)) {
                ...
                expansion_indices.at(dimension).at(max_diff.size()) = expansion_indices.at(dimension).at(
                    split_index); // Point to the original index (from before the expansion)
                ...
                // Update max_diff
                max_diff.at(split_index) = MaxDiff(diff_matrix, split_index, same_combined_index,
                                                   dim_indices_reduced.at(dimension), dim_indices_count.at(dimension));
                max_diff.push_back(MaxDiff(diff_matrix, max_diff.size(), same_combined_index,
                                           dim_indices_reduced.at(dimension), dim_indices_count.at(dimension)));
            }
        }
    }
...
void ProbabilityEstimates::IterativeProportionalFitting(const DataStats& stats, IPFDataSelector selected_data,
    ...
    case IPFDataSelector::kIPFQuality:
        if (stats.Qualities().BaseQualityForErrorRateReference(template_segment, tile_id, ref_base).size()) {
            ...
            descriptor << "Quality segment " << static_cast<uintTempSeqPrint>(template_segment) << ", tile_id "
                       << tile_id << ", ref_base " << ref_base;

            // Run the iterative proportional fitting
            quality_.at(template_segment)
                .at(tile_id)
                .at(ref_base)
                .IterativeProportionalFitting(error_during_fitting_, precision_improved_, precision_aim, max_iterations,
                                              margins, alternative_margin, descriptor.str(), print_mutex_);
            return;
        }
        break;
    case IPFDataSelector::kIPFSequenceQuality:
        if (stats.Qualities().SequenceQualityMeanForGCPerTileReference(template_segment, tile_id).size()) {
            ...
            // Write the description for printing
            stringstream descriptor;
            descriptor << "Sequence quality segment " << static_cast<uintTempSeqPrint>(template_segment) << ", tile_id "
                       << tile_id;

            // Run the iterative proportional fitting
            sequence_quality_.at(template_segment)
                .at(tile_id)
                .IterativeProportionalFitting(error_during_fitting_, precision_improved_, precision_aim, max_iterations,
                                              margins, alternative_margin, descriptor.str(), print_mutex_);
            return;
    ...
    case IPFDataSelector::kIPFBaseCall:
        if (stats.Qualities().BaseQualityForErrorRateReference(template_segment, tile_id, ref_base, dom_error).size()) {
            ...
            // rate)
            array<pair<const Vect<Vect<uintMatrixCount>>*, bool>, 10> margins;
            auto alternative_margin =
                DefineMarginsBaseCall(stats, margins, template_segment, tile_id, ref_base, dom_error);

            // Write the description for printing
            stringstream descriptor;
            descriptor << "Base-call segment " << static_cast<uintTempSeqPrint>(template_segment) << ", tile_id "
                       << tile_id << ", ref_base " << ref_base << ", dom_error " << dom_error;

            // Run the iterative proportional fitting
            base_call_.at(template_segment)
                .at(tile_id)
                .at(ref_base)
                .at(dom_error)
                .IterativeProportionalFitting(error_during_fitting_, precision_improved_, precision_aim, max_iterations,
    ...
    case IPFDataSelector::kIPFDominantError:
        if (stats.Coverage().GCByDistance(ref_base, last_ref_base, dom_error).size()) {
            ...
            // Write the description for printing
            stringstream descriptor;
            descriptor << "Dominant-error ref_base " << ref_base << ", last_ref_base " << last_ref_base
                       << ", dom_last5 " << dom_error;

            // Run the iterative proportional fitting
            dom_error_.at(ref_base)
                .at(last_ref_base)
                .at(dom_error)
                .IterativeProportionalFitting(error_during_fitting_, precision_improved_, precision_aim, max_iterations,
    ...
    case IPFDataSelector::kIPFInDels:
        if (stats.Errors().InDelByInDelPos(template_segment, last_ref_base).size()) {
            ...
            // Write the description for printing
            stringstream descriptor;
            descriptor << "InDel type " << static_cast<uintInDelTypePrint>(template_segment) << ", last_call "
                       << last_ref_base;
...
void ProbabilityEstimates::PrepareResult() {
    for (auto template_segment = 2; template_segment--;) {
        ...
        for (auto tile_id = quality_.at(template_segment).size(); tile_id--;) {
            for (auto ref_base = 4; ref_base--;) {
                ...
                quality_result_.at(template_segment)
                    .at(tile_id)
                    .at(ref_base)
                    .GetResults(quality_.at(template_segment).at(tile_id).at(ref_base).estimates_,
                                quality_.at(template_segment).at(tile_id).at(ref_base).dim_indices_);
        ...
        for (auto tile_id = sequence_quality_.at(template_segment).size(); tile_id--;) {
            ...
            sequence_quality_result_.at(template_segment)
                .at(tile_id)
                .GetResults(sequence_quality_.at(template_segment).at(tile_id).estimates_,
                            sequence_quality_.at(template_segment).at(tile_id).dim_indices_);
            ...
            for (auto ref_base = 4; ref_base--;) {
                for (auto dom_error = 5; dom_error--;) {
                    ...
                    base_call_result_.at(template_segment)
                        ...
                        .at(dom_error)
                        .GetResults(
                            base_call_.at(template_segment).at(tile_id).at(ref_base).at(dom_error).estimates_,
                            base_call_.at(template_segment).at(tile_id).at(ref_base).at(dom_error).dim_indices_);
        ...
        for (auto last_base = 5; last_base--;) {
            for (auto dom_last5 = 5; dom_last5--;) {
                ...
                dom_error_result_.at(ref_base).at(last_base).at(dom_last5).GetResults(
                    ...
                    dom_error_.at(ref_base).at(last_base).at(dom_last5).dim_indices_);
                dom_error_result_.at(ref_base).at(last_base).at(dom_last5).ImputeMissingValues();
                dom_error_.at(ref_base)
                    .at(last_base)
                    .at(dom_last5)
                    .Clear(); // Free space by removing the calculation storage for the ipf
            }
            error_rate_.at(ref_base).at(last_base).FullExpansion();
            error_rate_result_.at(ref_base).at(last_base).GetResults(
                error_rate_.at(ref_base).at(last_base).estimates_, error_rate_.at(ref_base).at(last_base).dim_indices_);
            error_rate_result_.at(ref_base).at(last_base).ImputeMissingValues();
            error_rate_.at(ref_base)
                .at(last_base)
                .Clear(); // Free space by removing the calculation storage for the ipf
        }
    }
    ...
    for (auto type = 2; type--;) {
        for (auto last_call = 6; last_call--;) {
            ...
            indels_result_.at(type).at(last_call).GetResults(indels_.at(type).at(last_call).estimates_,
                                                             indels_.at(type).at(last_call).dim_indices_);
...
bool ProbabilityEstimates::Estimate(const DataStats& stats, uintNumFits max_iterations, double precision_aim,
    ...
    if (string("") == input) {
        ...
        if (stats_creation_time_ != stats.CreationTime()) {
            printInfo << "Statistics file has been updated, the probability estimates will be overwritten."
                      << std::endl;
            stats_creation_time_ = stats.CreationTime();
            SetVariables(stats.Tiles().NumTiles());
        }
    ...
    if (error_during_fitting_) {
        DeleteFile(output); // Remove the probabilities file if one existed previously so it is clear that we
    ...
    if (QualityStats::kWriteOut4dMatrixCsvs) {
        ...
        for (uintQualPrint pq = stats.Qualities().BaseQualityForPrecedingQualityReference(0, 0, 0).from();
             pq < stats.Qualities().BaseQualityForPrecedingQualityReference(0, 0, 0).to(); ++pq) {
            if (stats.Qualities().BaseQualityForPrecedingQualityReference(0, 0, 0).at(pq).size()) {
                for (uintQualPrint sq = stats.Qualities().BaseQualityForSequenceQualityReference(0, 0, 0).from();
                     sq < stats.Qualities().BaseQualityForSequenceQualityReference(0, 0, 0).to(); ++sq) {
                    if (stats.Qualities().BaseQualityForSequenceQualityReference(0, 0, 0).at(sq).size()) {
                        for (uintReadLen pos = stats.Qualities().PrecedingQualityForPositionReference(0, 0, 0).from();
                             pos < stats.Qualities().PrecedingQualityForPositionReference(0, 0, 0).to(); ++pos) {
                            if (stats.Qualities().PrecedingQualityForPositionReference(0, 0, 0).at(pos).size()) {
                                ...
                                for (uintQual q = 0; q < probabilities.size(); ++q) {
                                    if (1.0e-10 < probabilities.at(q)) {
                                        myfile << quality_result_.at(0).at(0).at(0).PossibleValues().at(q) << ", " << pq
                                               << ", " << sq << ", " << pos << ", " << probabilities.at(q) << '\n';
                                    }
                                }
                            }
                        }
        ...
        for (uintQualPrint pq = stats.Qualities().BaseQualityForPrecedingQualityReference(1, 0, 1).from();
             pq < stats.Qualities().BaseQualityForPrecedingQualityReference(1, 0, 1).to(); ++pq) {
            if (stats.Qualities().BaseQualityForPrecedingQualityReference(1, 0, 1).at(pq).size()) {
                for (uintPercentPrint er = stats.Qualities().SequenceQualityForErrorRateReference(1, 0, 1).from();
                     er < stats.Qualities().SequenceQualityForErrorRateReference(1, 0, 1).to(); ++er) {
                    if (stats.Qualities().SequenceQualityForErrorRateReference(1, 0, 1).at(er).size()) {
                        for (uintReadLen pos = stats.Qualities().PrecedingQualityForPositionReference(1, 0, 1).from();
                             pos < stats.Qualities().PrecedingQualityForPositionReference(1, 0, 1).to(); ++pos) {
                            if (stats.Qualities().PrecedingQualityForPositionReference(1, 0, 1).at(pos).size()) {
                                ...
                                for (uintQual q = 0; q < probabilities.size(); ++q) {
                                    if (1.0e-10 < probabilities.at(q)) {
                                        myfile << quality_result_.at(1).at(0).at(1).PossibleValues().at(q) << ", " << pq
                                               << ", " << er << ", " << pos << ", " << probabilities.at(q) << '\n';
                                    }
                                }
                            }
                        }
