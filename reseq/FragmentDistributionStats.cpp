...
void BiasCalculationVectors::RemoveUnnecessarySites() {
    ...
    for (uintSeqLen cur_site = 0; cur_site < sites_.size(); ++cur_site) {
        if (0.0 !=
            sites_.at(cur_site).bias_) { // Filter out sides where at least one surrounding couldn't be determined
            zero_site = false;           // sites with zero for gc are ok as we use splines there

            for (uintSurPos sur_pos = Surrounding::Length(); sur_pos--;) {
                if (!sur_count_.at(kNumBases * sur_pos + sites_.at(cur_site).start_surrounding_.BaseAt(sur_pos))) {
                    zero_site = true;
                    break;
                }
                if (!sur_count_.at(kNumBases * sur_pos + sites_.at(cur_site).end_surrounding_.BaseAt(sur_pos))) {
                    zero_site = true;
                    break;
                }
            }
            ...
            if (!zero_site) {
                ...
                for (uintSurPos sur_pos = Surrounding::Length(); sur_pos--;) {
                    sur_sites_.at(kNumBases * sur_pos + sites_.at(cur_site).start_surrounding_.BaseAt(sur_pos)) += 2;
                    sur_sites_.at(kNumBases * sur_pos + sites_.at(cur_site).end_surrounding_.BaseAt(sur_pos)) += 2;
                }
            }
        }
    }
...
void BiasCalculationVectors::NormGC() {
    ...
    array<pair<uintFragCount, uintPercent>, kGCBins> sites;
    ...
    for (uintPercent gci = gc_sites_.size(); gci-- && sum_sites < Divide(total_sites_ * kPercentGCSitesForNormalization,
                                                                         static_cast<uintFragCount>(100));) {
        sum_sites += sites.at(gci).first;
        sum_bias += gc_bias_.at(sites.at(gci).second);
        ++num_bias;
    }

    // Normalize using the biases of the gc values having kPercentGCSitesForNormalization of the sites to take only well
    // determined values
    for (uintPercent gc = gc_sites_.size(); gc--;) {
    ...
    if (kSurMult) {
        for (auto sur_pos = 0; sur_pos < Surrounding::Length(); ++sur_pos) {
            ...
            for (auto base = 0; base < 4; ++base) {
                sur_sum += x.at(kNumBases * sur_pos + base);
                if (sur_count_.at(kNumBases * sur_pos + base)) {
                    ++valid_sur;
                }
            }
            for (auto base = 0; base < 4; ++base) {
                sur_bias_.at(kNumBases * sur_pos + base) = valid_sur * x.at(kNumBases * sur_pos + base) / sur_sum;
            }
        }
    } else {
        for (auto sur_pos = 0; sur_pos < Surrounding::Length(); ++sur_pos) {
...
pair<double, double> BiasCalculationVectors::SurBiasAtSiteSplit(const FragmentSite& site) {
    ...
    for (uintSurPos sur_pos = Surrounding::Length(); sur_pos--;) {
        if (kSurMult) {
            bias.first *= sur_bias_.at(kNumBases * sur_pos + site.start_surrounding_.BaseAt(sur_pos));
            bias.second *= sur_bias_.at(kNumBases * sur_pos + site.end_surrounding_.BaseAt(sur_pos));
        } else {
            bias.first += sur_bias_.at(kNumBases * sur_pos + site.start_surrounding_.BaseAt(sur_pos));
            bias.second += sur_bias_.at(kNumBases * sur_pos + site.end_surrounding_.BaseAt(sur_pos));
        }
...
void BiasCalculationVectors::DefineStartingKnots() {
    ...
    if (k < gc_knots_.size() && 102 > gc_knots_.at(k)) {
        ++k; // gc_knots_.at(k) is the last gc with a count, so don't overwrite that one in the filling procedure coming
             // next
    }

    // In case there are not enough gc bins with counts
    if (k < gc_knots_.size()) {
        // Fill rest of array with invalid gc, to not jump over an unfilled bin that accidentally happens to have the
        // right gc
        ...
        while (k < gc_knots_.size() && gc < kGCBins) {
...
void BiasCalculationVectors::PrepareSplines(array<T, 3>& lin_comb_splines, S& x, W& l, U& c, U& z, V& mu, V& h,
                                            T& beta) {
    // https://en.wikipedia.org/wiki/Spline_(mathematics) "Computation of Natural Cubic Splines" slightly adapted to get
    // the linear combination of the spline parameters a
...
template <typename T>
void BiasCalculationVectors::GetSplineCoefficients(double& a, double& b, double& c, double& d, uintSeqLen k,
                                                   const vector<double>& spline_pars,
                                                   const array<T, 3>& lin_comb_splines) {
...
void BiasCalculationVectors::GetSplineCoefficients(double& a, double& b, double& c, double& d, uintPercent k,
                                                   const vector<double>& spline_pars) {
...
void BiasCalculationVectors::CalculateSpline(const vector<double>& spline_pars) {
    // Constant bias values for gc below any observed sites: Extrapolate the linear function at the lowest measured GC
    // by half a GC unit and use this value
    ...
    // Constant bias values for gc above any observed sites: Extrapolate the linear function at the highest measured GC
    // by half a GC unit and use this value
    uintPercent cur_gc = gc_knots_.at(k) - gc_knots_.at(k - 1);
    double upper_const_no_logit =
        spline_pars.at(k + 1) + 0.5 * (b + c * cur_gc); // d=0 since second derivative must be 0 (natural spline)
    double upper_const;
    if (kGCExp) {
...
void BiasCalculationVectors::CalculateSplineGrad(const vector<double>& spline_pars, vector<double>& grad,
                                                 uintNumFits grad_offset) {
    ...
    for (uintPercent gc = gc_knots_.at(0); gc <= gc_knots_.at(gc_knots_.size() - 1); ++gc) {
        if (gc == gc_knots_.at(k + 1)) {
            ...
            uintPercent cur_gc = gc - gc_knots_.at(k);
            for (uintPercent ai = 0; ai < gc_spline_pars_.size() - 1; ++ai) {
                grad.at(grad_offset + ai + 1) += (lin_comb_gc_splines_.at(0).at(k).at(ai) * cur_gc +
                                                  lin_comb_gc_splines_.at(1).at(k).at(ai) * cur_gc * cur_gc +
                                                  lin_comb_gc_splines_.at(2).at(k).at(ai) * cur_gc * cur_gc * cur_gc) *
                                                 gc_bias_grad_.at(gc);
            }
        }
    ...
    for (uintPercent gc = gc_knots_.at(gc_knots_.size() - 1) + 1; gc < gc_count_.size(); ++gc) {
        grad.at(grad_offset + k + 1) += gc_bias_grad_.at(gc);
        for (uintPercent ai = 0; ai < gc_spline_pars_.size() - 1; ++ai) {
            grad.at(grad_offset + ai + 1) +=
                0.5 *
                (lin_comb_gc_splines_.at(0).at(k - 1).at(ai) + lin_comb_gc_splines_.at(1).at(k - 1).at(ai) * cur_gc) *
                gc_bias_grad_.at(gc);
        }
    }
}
...
double BiasCalculationVectors::LogLikeGcSpline(const vector<double>& x, vector<double>& grad, void* f_data) {
    ...
    for (uintPercent gc = 0; gc < calc.gc_count_.size(); ++gc) {
        loglike += calc.gc_bias_sum_.at(gc).first + calc.gc_count_.at(gc) * log(calc.gc_bias_.at(gc)) -
                   calc.gc_bias_.at(gc) * calc.gc_bias_sum_.at(gc).second;
    }
    ...
    if (grad.size()) {
        ...
        for (uintPercent gc = 0; gc < calc.gc_count_.size(); ++gc) {
            ...
            calc.gc_bias_grad_.at(gc) =
                (calc.gc_count_.at(gc) - calc.gc_bias_sum_.at(gc).second * calc.gc_bias_.at(gc));
        }
...
double BiasCalculationVectors::OptimizeSpline() {
    ...
    if (!kGCExp) {
        max_val /= 1.6; // The maximum normalized value should be 1.6, so that we stay in the mostly linear part of the
                        // 2*InvLogit
    }
...
double BiasCalculationVectors::OptimizeSpline(uintPercent knot, uintPercentShift shift) {
    ...
    if ((shift < 0 && gc_knots_.at(knot - 1) >= gc_knots_.at(knot) + shift) ||
        (shift > 0 && gc_knots_.at(knot + 1) <= gc_knots_.at(knot) + shift) ||
        (0 == gc_count_.at(gc_knots_.at(knot) + shift))) {
...
void BiasCalculationVectors::GetGCSpline() {
    ...
    // Optimize gc spline
    // Knots are adjusted by comparing the likelihoods to the ones with all inner knots in turn moved by 1 to range up
    // or down and taking the best likelihood in a greedy fashion Iterate until likelihood cannot be improved by moving
    // an inner knot by 1 to range gc
    ...
    while (bestlike != 0) { // loglikes[0] is current knot position
        ...
        for (uintNumFits cur_like = loglikes.size(); --cur_like;) { // Leave out 0
            if (cur_like != bestlike) { // Otherwise we still have the value from last iteration
                loglikes.at(cur_like) = OptimizeSpline(OptimizeSplineIdToKnot(cur_like, kMaxKnotShift),
                                                       OptimizeSplineIdToShift(cur_like, kMaxKnotShift));
            }
        }
        ...
        if (bestlike != 0) {
            ...
            uintNumFits flipped_bestlike =
                bestlike - 2 * (static_cast<uintPercentShift>((bestlike - 1) % (2 * kMaxKnotShift)) - kMaxKnotShift) -
                1;
            loglikes.at(flipped_bestlike) = loglikes.at(0);
            loglikes.at(0) = loglikes.at(bestlike);
            // Apply the knot movement
            gc_knots_.at(OptimizeSplineIdToKnot(bestlike, kMaxKnotShift)) +=
                OptimizeSplineIdToShift(bestlike, kMaxKnotShift);
            // Store at which new position the original likelihood is, so that we don't recalculate it
            bestlike = flipped_bestlike;
        }
    }

    // Get gc bias for best spline (We require a higher precision here as the surrounding gradient assumes that the gc
    // gradients are exactly zero, so we don't want to deviate much)
...
double BiasCalculationVectors::LogLikelihoodPoisson(const vector<double>& x, vector<double>& grad, void* f_data) {
    ...
    for (auto& site : calc.sites_) {
        ...
        if (grad.size()) {
            ...
            for (uintSurPos sur_pos = Surrounding::Length(); sur_pos--;) {
                auto i_start = kNumBases * sur_pos + site.start_surrounding_.BaseAt(sur_pos);
                auto i_end = kNumBases * sur_pos + site.end_surrounding_.BaseAt(sur_pos);
    ...
    if (grad.size()) {
        for (uintPercent gc = calc.gc_count_.size(); gc--;) {
            for (auto sur = calc.sur_bias_.size(); sur--;) {
                calc.grad_gc_bias_sum_.at(gc).at(sur).first *=
                    2; // Multiplication with 2 here to save two multiplication per site
...
double BiasCalculationVectors::GetDispersion(double bias, double a, double b) {
    ...
    if (r > bias * 1e10) {
        // r is so much larger than the bias that r + bias is having precision issues, so shift it back into precision
        // (it anyways is approximately poisson at this point)
        r = bias * 1e10;
    }
    return r;
}

void BiasCalculationVectors::LogLikelihoodNbinomSite(double& loglike, double& a, double& b, double& ga, double& gb,
                                                     vector<double>& grad, const FragmentSite& site) {
    ...
    double grad_term2 =
        2 * log(1 / (bias / r + 1)); // 2* because of forward+reverse; Be careful to handle r->inf without numerical
                                     // instabilities, should result in r*grad_term2 -> -2*bias
    ...
    if (grad.size()) {
        ...
        for (uintSurPos sur_pos = Surrounding::Length(); sur_pos--;) {
            if (kSurMult) {
                sur_grad_.at(kNumBases * sur_pos + site.start_surrounding_.BaseAt(sur_pos)) +=
                    grad_term1; // Division by bias later
                sur_grad_.at(kNumBases * sur_pos + site.end_surrounding_.BaseAt(sur_pos)) += grad_term1;
            } else {
                sur_grad_.at(kNumBases * sur_pos + site.start_surrounding_.BaseAt(sur_pos)) +=
                    grad_term1 * cur_grad_start;
                sur_grad_.at(kNumBases * sur_pos + site.end_surrounding_.BaseAt(sur_pos)) += grad_term1 * cur_grad_end;
            }
        }
    }
}
...
double BiasCalculationVectors::LogLikelihoodConstDispersion(const std::vector<double>& x, std::vector<double>& grad,
                                                            void* f_data) {
    BiasCalculationVectors& calc(*reinterpret_cast<BiasCalculationVectors*>(f_data));

    ++calc.func_calls_const_disp_;
    double loglike(0.0), gr(0.0), gr0(0.0), r(x.at(0)),
        bias; // Calculate log likelihood for negative binomial with constant dispersion

    for (auto site = calc.sites_.begin() + calc.start_site_dispersion_fit_;
         site != calc.sites_.begin() + calc.end_site_dispersion_fit_; ++site) {
...
void BiasCalculationVectors::WriteOutParameterInfo(const char* context) {
    ...
    std::array<double, kGCBins> unnormalized_gc;
    for (auto gc = unnormalized_gc.size(); gc--;) {
        unnormalized_gc.at(gc) = gc_bias_.at(gc);
    }
    NormGC();

    output << context << ", " << ref_seq_bin_ << ", " << insert_length_ << ", " << total_counts_ << ", " << total_sites_
           << ", " << func_calls_ << ", " << loglike_ << ", ";
...
void BiasCalculationVectors::WriteOutDispersionInfo(const char* context) {
    ...
    start_site_dispersion_fit_ =
        ((total_sites_) % kDispersionBinSize) / 2; // Ignore equally many sites in the beginning and end
    ...
    while (end_site_dispersion_fit_ < sites_.size()) {
        ...
        for (auto s = start_site_dispersion_fit_; s < end_site_dispersion_fit_; ++s) {
            ...
            ++duplication_count_part_.at(
                min(sites_.at(s).count_forward_, static_cast<uintDupCount>(kMaxDuplications + 1)));
            ++duplication_count_part_.at(
                min(sites_.at(s).count_reverse_, static_cast<uintDupCount>(kMaxDuplications + 1)));
        }
        ...
        output << ref_seq_bin_ << ", " << insert_length_ << ", " << context << ", " << prediction_mean << ", "
               << sample_mean << ", " << 2 * (end_site_dispersion_fit_ - start_site_dispersion_fit_) << ", "
               << max_duplication;
...
template <class T>
void InsertLengthSpline::GetSampledValues(const Vect<double>& insert_lengths_bias, const T& insert_lengths) {
    ...
    for (uintSeqLen s = 0; s < sample_positions_.size(); ++s) {
        ...
        sampled_values_.at(s) = insert_lengths.at(sample_positions_.at(s)) /
                                insert_lengths_bias.at(sample_positions_.at(s)); // Count/Bias ratio
    }
...
double InsertLengthSpline::Chi2(const std::vector<double>& x, std::vector<double>& grad, void* f_data) {
    ...
    chi2 += (self.sampled_values_.at(self.sample_positions_.size() - 1) - estimate) *
            (self.sampled_values_.at(self.sample_positions_.size() - 1) - estimate);

    if (grad.size()) {
        grad.at(grad.size() - 1) =
            -2.0 * (self.sampled_values_.at(self.sample_positions_.size() - 1) - estimate) * estimate;
    }
...
double InsertLengthSpline::OptimizeParameters(uintSeqLen shifted_knot, intSeqShift shifted_positions) {
    if (0 > shifted_positions) {
        if (-shifted_positions > knots_.at(shifted_knot) ||
            knots_.at(shifted_knot - 1) >= knots_.at(shifted_knot) + shifted_positions) {
...
void InsertLengthSpline::ShiftOptimization() {
    ...
    do { // while(0 != best_shifted_positions)
        ...
        for (uintSeqLen shifted_knot = 1; shifted_knot < knots_.size() - 1; ++shifted_knot) {
            for (intSeqShift shifted_positions = kKnotShiftLength; shifted_positions < kKnotMaxShift;
                 shifted_positions += kKnotShiftLength) {
...
void InsertLengthSpline::FillInWithFittedRatios(Vect<double>& insert_lengths_bias,
                                                const Vect<uintFragCount>& insert_lengths) {
    ...
    for (; k < knots_.size() - 1; ++k) {
        ...
        for (uintSeqLen len = knots_.at(k) + 1; len < knots_.at(k + 1); ++len) {
            ...
            insert_lengths_bias.at(len) =
                insert_lengths.at(len) / exp(a + b * cur_len + c * cur_len * cur_len + d * cur_len * cur_len * cur_len);
        }
    }

    insert_lengths_bias.at(knots_.at(k)) =
        insert_lengths.at(knots_.at(k)) / exp(pars_.at(k + 1)); // Set last knot to its value defined by the parameter
...
void InsertLengthSpline::FillInWithFittedRatios(vector<double>& normalization,
                                                const Vect<double>& insert_lengths_bias) {
    // Everything before the first knot is zero
    ...
    for (; k < knots_.size() - 1; ++k) {
        ...
        for (uintSeqLen len = knots_.at(k) + 1; len < knots_.at(k + 1); ++len) {
            uintSeqLen cur_len = len - knots_.at(k);
            normalization.at(len) = insert_lengths_bias.at(len) *
                                    exp(a + b * cur_len + c * cur_len * cur_len + d * cur_len * cur_len * cur_len);
        }
    }

    normalization.at(knots_.at(k)) = insert_lengths_bias.at(knots_.at(k)) *
                                     exp(pars_.at(k + 1)); // Set last knot to its value defined by the parameter
...
template <class T> bool InsertLengthSpline::CheckFit(const Vect<double>& insert_lengths_bias, const T& insert_lengths) {
    ...
    // Start at low insert length and stop after a removal of a sample, fluctuations decrease with insert length and
    // therefore the causal samples are more likely to sit at the beginning
    uintSeqLen s(0);
    for (uintSeqLen len = knots_.at(0); len < knots_.at(knots_.size() - 1); ++len) {
        ...
        if (sample_positions_.at(s) == len) {
            ...
            if (insert_lengths.at(len) / insert_lengths_bias.at(len) <
                    sampled_values_.at(s) / outlier_factor || // Factor too low
                insert_lengths.at(len) / insert_lengths_bias.at(len) >
                    sampled_values_.at(s) * outlier_factor) { // Factor too high
            ...
            if (insert_lengths.at(len) / insert_lengths_bias.at(len) <
                min(sampled_values_.at(s - 1), sampled_values_.at(s)) / outlier_factor) {
                // Factor too low
                if (1 == s) {
                    // We must not remove first sample, so remove second
                    sample_to_delete = 1;
            ...
            } else if (insert_lengths.at(len) / insert_lengths_bias.at(len) >
                       max(sampled_values_.at(s - 1), sampled_values_.at(s)) * outlier_factor) {
                // Factor too high
                if (1 == s) {
                    // We must not remove first sample, so remove second
                    sample_to_delete = 1;
...
void InsertLengthSpline::FillInWithMeanRatio(Vect<double>& insert_lengths_bias,
                                             const Vect<uintFragCount>& insert_lengths) {
...
void InsertLengthSpline::FillInWithMeanRatio(std::vector<double>& normalization,
                                             const Vect<double>& insert_lengths_bias) {
...
bool InsertLengthSpline::GetSamplePositions(const Vect<uintFragCount>& insert_lengths) {
    ...
    for (uintSeqLen k = 1; k < num_samples - found_zeros; ++k) {
        ...
        while (0 == insert_lengths.at(
                        sample_positions_.at(k))) { // Last sample cannot be a zero, so no further checks necessary
            ++found_zeros;
...
bool InsertLengthSpline::FitInsertLengthWithSpline(Vect<double>& insert_lengths_bias,
                                                   const Vect<uintFragCount>& insert_lengths) {
...
bool InsertLengthSpline::InterpolateNormalizationWithSpline(vector<double>& normalization,
                                                            const Vect<double>& insert_lengths_bias) {
...
void FragmentDistributionStats::PrepareBiasCalculation(const Reference& ref, uintSeqLen maximum_insert_length,
                                                       uintSeqLen max_ref_seq_bin_size,
                                                       const vector<uintFragCount>& reads_per_frag_len_bin,
                                                       const vector<uintFragCount>& lowq_reads_per_frag_len_bin,
    ...
    for (uintRefSeqId ref_seq = 0; ref_seq < ref.NumberSequences(); ++ref_seq) {
        if (!ref.ReferenceSequenceExcluded(ref_seq)) {
            for (uintSeqLen cur_bin = 0; cur_bin <= ref.SequenceLength(ref_seq) / maximum_insert_length; ++cur_bin) {
                ...
                lowq_reads_per_ref_seq_bin.at(ref_seq_bin) +=
                    lowq_reads_per_frag_len_bin.at(ref_seq_start_bin + cur_bin);

                // Add it also to all bins the fragment might overlap, as we don' know the exact start position of the
                // fragment
                if (cur_bin < ref.SequenceLength(ref_seq) / maximum_insert_length) {
                    auto ref_seq_bin2 =
                        GetRefSeqBin(ref_seq, cur_bin * maximum_insert_length + maximum_insert_length, ref_seq_bin);
                    if (ref_seq_bin != ref_seq_bin2) {
                        reads_per_ref_seq_bin.at(ref_seq_bin2) +=
                            reads_per_frag_len_bin.at(ref_seq_start_bin + cur_bin);
                        lowq_reads_per_ref_seq_bin.at(ref_seq_bin2) +=
                            lowq_reads_per_frag_len_bin.at(ref_seq_start_bin + cur_bin);
                    }
                }
    ...
    for (uintRefSeqBin ref_bin = 0; ref_bin < fragment_sites_by_ref_seq_bin_.size(); ++ref_bin) {
        fragment_sites_by_ref_seq_bin_.at(ref_bin).resize(reads_per_ref_seq_bin.at(ref_bin) /
                                                          2); // Sites are defined by a read pair, so /2
    }
    ...
    if (BiasCalculationVectors::kParameterInfoFile) {
        ...
        for (auto gc = 0; gc < kGCBins; ++gc) {
            myfile << ", " << "RawGCbias" << gc << ", " << "GCbias" << gc << ", " << "GCcount" << gc << ", "
                   << "GCsites" << gc;
        }
    ...
    if (BiasCalculationVectors::kDispersionInfoFile) {
        ...
        myfile << "ref_seq_bin, insert_length, mean_fit, prediction_mean, sample_mean, num_sites, max_duplication, "
                  "counts1, counts2, counts3, calls_prediction, dispersion_prediction, calls_sample, dispersion_sample"
               << std::endl;
        myfile.close();
    }
}

inline void FragmentDistributionStats::CheckLowQExclusion(vector<pair<uintSeqLen, uintSeqLen>>& lowq_site_exclusion,
                                                          uintSeqLen excluded_pos, uintFragCount lowq_count,
                                                          const vector<uintFragCount>& tmp_frag_count) {
    if (lowq_count > kMinLowQFractionForExclusion * tmp_frag_count.at(excluded_pos) - 1e-10) {
        ...
        if (0 == lowq_site_exclusion.size()) {
            ...
            for (auto last_pos = lowq_site_exclusion.back().second;
                 last_pos < excluded_pos && separating_high_q < kMinHighQToSeparateLowQ; ++last_pos) {
            ...
            if (separating_high_q < kMinHighQToSeparateLowQ) {
                ...
                if (1 == lowq_site_exclusion.back().second - lowq_site_exclusion.back().first) {
                    ...
                    separating_high_q = 0;
                    while (lowq_site_exclusion.back().first &&
                           separating_high_q + tmp_frag_count.at(lowq_site_exclusion.back().first - 1) <
                               kMinHighQToSeparateLowQ / 2) {
                ...
                if (1 == lowq_site_exclusion.back().second - lowq_site_exclusion.back().first) {
                    ...
                    separating_high_q = 0;
                    while (lowq_site_exclusion.back().second < tmp_frag_count.size() &&
...
void FragmentDistributionStats::CheckLowQExclusions(uintRefSeqBin ref_seq_bin, vector<uintFragCount>& tmp_frag_count,
                                                    const Reference& reference) {
    // Handle low quality site starts
    auto ref_seq = ref_seq_bin_def_.at(ref_seq_bin).first;
    auto tmp_size =
        RefSeqSplitLength(ref_seq, reference) + ref_seq_start_bin_.at(ref_seq + 1) - ref_seq_start_bin_.at(ref_seq);
    ...
    if (lowq_site_start_by_ref_seq_bin_.at(ref_seq_bin).size()) {
        ...
        sort(lowq_site_start_by_ref_seq_bin_.at(ref_seq_bin).begin(),
             lowq_site_start_by_ref_seq_bin_.at(ref_seq_bin).end());
        ...
        for (uintFragCount site_id = 1; site_id < lowq_site_start_by_ref_seq_bin_.at(ref_seq_bin).size(); ++site_id) {
            if (lowq_site_start_by_ref_seq_bin_.at(ref_seq_bin).at(site_id) == excluded_pos) {
                ...
                CheckLowQExclusion(lowq_site_start_exclusion_.at(ref_seq_bin), excluded_pos, lowq_count,
                                   tmp_frag_count);
        ...
        if (lowq_site_start_exclusion_.at(ref_seq_bin).size()) {
            if (1 == lowq_site_start_exclusion_.at(ref_seq_bin).back().second -
                         lowq_site_start_exclusion_.at(ref_seq_bin).back().first) {
                // Remove the single random event
                lowq_site_start_exclusion_.at(ref_seq_bin).pop_back();
            } else {
                // Extend last region at the end
                uintFragCount separating_high_q = 0;
                while (lowq_site_start_exclusion_.at(ref_seq_bin).back().second < tmp_frag_count.size() &&
    ...
    for (uintFragCount site_id = 0; site_id < fragment_sites_by_ref_seq_bin_cur_id_.at(ref_seq_bin); ++site_id) {
        ++tmp_frag_count.at(fragment_sites_by_ref_seq_bin_.at(ref_seq_bin).at(site_id).first / 2 +
                            fragment_sites_by_ref_seq_bin_.at(ref_seq_bin).at(site_id).second);
    }
    ...
    if (lowq_site_end_by_ref_seq_bin_.at(ref_seq_bin).size()) {
        ...
        sort(lowq_site_end_by_ref_seq_bin_.at(ref_seq_bin).begin(),
             lowq_site_end_by_ref_seq_bin_.at(ref_seq_bin).end());
        ...
        if (lowq_site_end_exclusion_.at(ref_seq_bin).size()) {
            if (1 == lowq_site_end_exclusion_.at(ref_seq_bin).back().second -
                         lowq_site_end_exclusion_.at(ref_seq_bin).back().first) {
                // Remove the single random event
                lowq_site_end_exclusion_.at(ref_seq_bin).pop_back();
            } else {
                // Extend last region at the end
                uintFragCount separating_high_q = 0;
                while (lowq_site_end_exclusion_.at(ref_seq_bin).back().second < tmp_frag_count.size() &&
...
void FragmentDistributionStats::SortFragmentSites(uintRefSeqBin ref_seq_bin,
                                                  vector<uintSeqLen>& num_sites_per_insert_length) {
    ...
    if (max_used_length) {
        ...
        for (uintSeqLen insert_length = 1; insert_length < max_used_length + 1; ++insert_length) {
            fragment_sites_by_ref_seq_bin_by_insert_length_.at(ref_seq_bin)
                .at(insert_length)
                .reserve(num_sites_per_insert_length.at(insert_length));
        }

        // Sort into bins
        for (uintFragCount site_id = 0; site_id < fragment_sites_by_ref_seq_bin_cur_id_.at(ref_seq_bin); ++site_id) {
            fragment_sites_by_ref_seq_bin_by_insert_length_.at(ref_seq_bin)
                .at(fragment_sites_by_ref_seq_bin_.at(ref_seq_bin).at(site_id).second)
                .push_back(fragment_sites_by_ref_seq_bin_.at(ref_seq_bin).at(site_id).first);
        }
    }
...
void FragmentDistributionStats::UpdateBiasCalculationParams(
    uintRefSeqBin ref_seq_bin, uint32_t queue_spot,
    vector<pair<uintSeqLen, pair<uintRefSeqBin, uintSeqLen>>>& tmp_params, mutex& print_mutex) {
    ...
    if (fragment_sites_by_ref_seq_bin_by_insert_length_.at(ref_seq_bin).size()) {
        ...
        if (ref_seq_in_nxx_.at(GetRefSeqId(ref_seq_bin))) {
            ...
            tmp_params.clear();
            for (uintSeqLen insert_length = 1;
                 insert_length < fragment_sites_by_ref_seq_bin_by_insert_length_.at(ref_seq_bin).size();
                 ++insert_length) {
                if (fragment_sites_by_ref_seq_bin_by_insert_length_.at(ref_seq_bin).at(insert_length).size()) {
                    ...
                    bias_calc_params_.at(queue_spot * qbin_size + insert_length - 1)
                        .Clear(ref_seq_bin); // If we won't set anything to it later we have to clear it from it's
                                             // previous content
                }
            }

            sort(tmp_params.begin(), tmp_params.end(),
                 std::greater<pair<uintSeqLen, pair<uintRefSeqBin, uintSeqLen>>>());

            for (uintNumFits n = 0; n < tmp_params.size(); ++n) {
                bool calculate_param = false;
                if (n < BiasCalculationVectors::kNumFitsInsertLength &&
                    (0 == n ||
                     1 == n * num_fits_per_ref_bin_ / BiasCalculationVectors::kNumFitsInsertLength -
                              (n - 1) * num_fits_per_ref_bin_ / BiasCalculationVectors::kNumFitsInsertLength)) {
                    calculate_param = calculate_bias_;
                }

                bias_calc_params_.at(queue_spot * qbin_size + tmp_params.at(n).second.second - 1)
                    .Set(tmp_params.at(n).second.first, tmp_params.at(n).second.second, calculate_param);
            }

            if (tmp_params.size() < num_fits_per_ref_bin_) {
                params_fitted_ +=
                    num_fits_per_ref_bin_ - tmp_params.size(); // Do not have to be fitted, so are already done

                lock_guard<mutex> lock(print_mutex);
                printInfo << "Finished "
                          << static_cast<uintPercentPrint>(
                                 Percent(params_fitted_, static_cast<uintNumFits>(tmp_gc_bias_.at(0).size())))
                          << "% of the bias fits." << std::endl;
            }
        } else {
            for (uintSeqLen insert_length = 1;
                 insert_length < fragment_sites_by_ref_seq_bin_by_insert_length_.at(ref_seq_bin).size();
                ...
                if (fragment_sites_by_ref_seq_bin_by_insert_length_.at(ref_seq_bin).at(insert_length).size()) {
                    bias_calc_params_.at(queue_spot * qbin_size + insert_length - 1)
                        .Set(ref_seq_bin, insert_length, false);
                } else {
                    bias_calc_params_.at(queue_spot * qbin_size + insert_length - 1)
                        .Clear(ref_seq_bin); // If we won't set anything to it later we have to clear it from it's
                                             // previous content
        ...
        for (uintSeqLen insert_length = fragment_sites_by_ref_seq_bin_by_insert_length_.at(ref_seq_bin).size();
...
void FragmentDistributionStats::FillParams(vector<BiasCalculationParams>& params, const Reference& reference,
                                           const vector<uintSeqLen>& sample_frag_length) const {
...
void FragmentDistributionStats::FillParamsSimulation(vector<BiasCalculationParams>& params, const Reference& reference,
                                                     const vector<uintSeqLen>& sample_frag_length) const {
...
void FragmentDistributionStats::CountEndExclusion(uintFragCount& lowq_sites, uintSeqLen& cur_end_exclusion,
                                                  uintSeqLen last_cut, uintSeqLen new_cut, uintRefSeqBin ref_seq_bin) {
    while (cur_end_exclusion < lowq_site_end_exclusion_.at(ref_seq_bin).size() &&
           last_cut > lowq_site_end_exclusion_.at(ref_seq_bin).at(cur_end_exclusion).second) {
        ++cur_end_exclusion; // Ignore everything where we cannot end, because it is before fragment length, or where we
                             // are overlapping with start exclusion
    }
    if (cur_end_exclusion < lowq_site_end_exclusion_.at(ref_seq_bin).size() &&
        last_cut > lowq_site_end_exclusion_.at(ref_seq_bin).at(cur_end_exclusion).first) {
    ...
    while (cur_end_exclusion < lowq_site_end_exclusion_.at(ref_seq_bin).size() &&
           lowq_site_end_exclusion_.at(ref_seq_bin).at(cur_end_exclusion).second <= new_cut) {
        lowq_sites += lowq_site_end_exclusion_.at(ref_seq_bin).at(cur_end_exclusion).second -
                      lowq_site_end_exclusion_.at(ref_seq_bin).at(cur_end_exclusion).first;
        ++cur_end_exclusion;
    }

    if (cur_end_exclusion < lowq_site_end_exclusion_.at(ref_seq_bin).size() &&
        lowq_site_end_exclusion_.at(ref_seq_bin).at(cur_end_exclusion).first < new_cut &&
        last_cut <= lowq_site_end_exclusion_.at(ref_seq_bin).at(cur_end_exclusion).first) {
        lowq_sites += new_cut - lowq_site_end_exclusion_.at(ref_seq_bin).at(cur_end_exclusion).first;
    }
}

void FragmentDistributionStats::CountDuplicates(FragmentDuplicationStats& duplications,
                                                const BiasCalculationParamsSplitSeqs& params,
                                                const Reference& reference) {
    duplications.AddDuplicates(
        fragment_sites_by_ref_seq_bin_by_insert_length_.at(params.ref_seq_bin_).at(params.fragment_length_));

    // fragment_sites_by_ref_seq_bin_by_insert_length_ already sorted from AddDuplicates call
    uintFragCount highq_counts(
        fragment_sites_by_ref_seq_bin_by_insert_length_.at(params.ref_seq_bin_).at(params.fragment_length_).size());
    uintSeqLen cur_start_exclusion(0);
    uintSeqLen cur_end_exclusion(0);
    for (auto pos :
         fragment_sites_by_ref_seq_bin_by_insert_length_.at(params.ref_seq_bin_).at(params.fragment_length_)) {
        while (cur_start_exclusion < lowq_site_start_exclusion_.at(params.ref_seq_bin_).size() &&
               pos >= lowq_site_start_exclusion_.at(params.ref_seq_bin_).at(cur_start_exclusion).second) {
            ++cur_start_exclusion;
        }
        while (cur_end_exclusion < lowq_site_end_exclusion_.at(params.ref_seq_bin_).size() &&
               pos + params.fragment_length_ >=
                   lowq_site_end_exclusion_.at(params.ref_seq_bin_).at(cur_end_exclusion).second) {
            ++cur_end_exclusion;
        }

        if ((cur_start_exclusion < lowq_site_start_exclusion_.at(params.ref_seq_bin_).size() &&
             pos >= lowq_site_start_exclusion_.at(params.ref_seq_bin_).at(cur_start_exclusion).first) ||
            (cur_end_exclusion < lowq_site_end_exclusion_.at(params.ref_seq_bin_).size() &&
             pos + params.fragment_length_ >=
                 lowq_site_end_exclusion_.at(params.ref_seq_bin_).at(cur_end_exclusion).first)) {
            --highq_counts;
        }
    }

    uintSeqLen bin_end;
    if (params.ref_seq_bin_ + 1 < ref_seq_bin_def_.size() &&
        ref_seq_bin_def_.at(params.ref_seq_bin_).first == ref_seq_bin_def_.at(params.ref_seq_bin_ + 1).first) {
    ...
    if (0 == ref_seq_bin_def_.at(params.ref_seq_bin_).second) {
        ...
        correction = 0;
        current_region = reference.NextExclusionRegion(ref_seq_bin_def_.at(params.ref_seq_bin_).first,
                                                       ref_seq_bin_def_.at(params.ref_seq_bin_).second) -
                         1; // The -1 is important as this function returns the region after the position, but
                            // CorrectPositionRemovingExcludedRegions needs the one before the position
    }

    reference.CorrectPositionRemovingExcludedRegions(correction, current_region,
                                                     ref_seq_bin_def_.at(params.ref_seq_bin_).first, bin_end);
    ...
    while (cur_start_exclusion < lowq_site_start_exclusion_.at(params.ref_seq_bin_).size() &&
           highq_sites >= lowq_site_start_exclusion_.at(params.ref_seq_bin_).at(cur_start_exclusion).second) {
        CountEndExclusion(lowq_sites, cur_end_exclusion, last_cut,
                          lowq_site_start_exclusion_.at(params.ref_seq_bin_).at(cur_start_exclusion).first +
                              params.fragment_length_,
                          params.ref_seq_bin_);

        lowq_sites += lowq_site_start_exclusion_.at(params.ref_seq_bin_).at(cur_start_exclusion).second -
                      lowq_site_start_exclusion_.at(params.ref_seq_bin_).at(cur_start_exclusion).first;
        last_cut =
            lowq_site_start_exclusion_.at(params.ref_seq_bin_).at(cur_start_exclusion).second + params.fragment_length_;
        ++cur_start_exclusion;
    }
    if (cur_start_exclusion < lowq_site_start_exclusion_.at(params.ref_seq_bin_).size() &&
        highq_sites > lowq_site_start_exclusion_.at(params.ref_seq_bin_).at(cur_start_exclusion).first) {
        CountEndExclusion(lowq_sites, cur_end_exclusion, last_cut,
                          lowq_site_start_exclusion_.at(params.ref_seq_bin_).at(cur_start_exclusion).first +
                              params.fragment_length_,
                          params.ref_seq_bin_);
        ...
        CountEndExclusion(lowq_sites, cur_end_exclusion, last_cut, highq_sites + params.fragment_length_,
                          params.ref_seq_bin_);
    }
    highq_sites -= lowq_sites;

    if (highq_sites) {
        auto end_pos = reference.SequenceLength(ref_seq_bin_def_.at(params.ref_seq_bin_).first) -
                       params.fragment_length_ + 1; // bin_end above removes the EndExclusion
        if (params.ref_seq_bin_ + 1 < ref_seq_bin_def_.size() &&
            ref_seq_bin_def_.at(params.ref_seq_bin_).first == ref_seq_bin_def_.at(params.ref_seq_bin_ + 1).first) {
            end_pos = ref_seq_bin_def_.at(params.ref_seq_bin_ + 1).second;
        }

        corrected_abundance_.at(ref_seq_bin_def_.at(params.ref_seq_bin_).first) +=
            Divide(highq_counts * (end_pos - ref_seq_bin_def_.at(params.ref_seq_bin_).second), highq_sites);
...
void FragmentDistributionStats::AddFragmentsToSites(vector<FragmentSite>& sites,
                                                    const vector<uintSeqLen>& fragment_positions) {
...
reseq::uintSeqLen FragmentDistributionStats::ExcludeLowQualitySites(vector<FragmentSite>& sites,
                                                                    uintRefSeqBin ref_seq_bin,
                                                                    uintSeqLen insert_length) {
    for (auto& region : lowq_site_start_exclusion_.at(ref_seq_bin)) {
        for (auto pos = region.first; pos < region.second && pos < sites.size();
             ++pos) { // Depending on insert_length the last bases of reference sequence cannot be a start position
            sites.at(pos).bias_ = 0.0;
        }
    }
    for (auto& region : lowq_site_end_exclusion_.at(ref_seq_bin)) {
        for (auto pos = max(insert_length, region.first); pos < region.second && pos - insert_length < sites.size();
             ++pos) {
...
void FragmentDistributionStats::CalculateBiasByBin(BiasCalculationVectors& tmp_calc, const Reference& reference,
                                                   FragmentDuplicationStats& duplications, uintRefSeqBin ref_seq_bin,
                                                   uintSeqLen insert_length) {
    // Prepare fits
    ...
    AddFragmentsToSites(tmp_calc.sites_,
                        fragment_sites_by_ref_seq_bin_by_insert_length_.at(ref_seq_bin).at(insert_length));
    ...
    // CountsInHighQRegion * (SitesInHighQRegion + SitesInLowQRegion)/SitesInHighQRegion ~ CountsInHighQRegion +
    // CountsInLowQRegion
    auto end_pos = reference.SequenceLength(ref_seq_bin_def_.at(ref_seq_bin).first) - insert_length + 1;
    if (ref_seq_bin + 1 < ref_seq_bin_def_.size() &&
        ref_seq_bin_def_.at(ref_seq_bin).first == ref_seq_bin_def_.at(ref_seq_bin + 1).first) {
        end_pos = ref_seq_bin_def_.at(ref_seq_bin + 1).second;
    }

    corrected_abundance_.at(ref_seq_bin_def_.at(ref_seq_bin).first) +=
        Divide(tmp_calc.total_counts_ * (end_pos - ref_seq_bin_def_.at(ref_seq_bin).second),
               tmp_calc.sites_.size() - excluded_sites);
    ...
    if (kMinSites > tmp_calc.total_sites_ || kMinCounts > tmp_calc.total_counts_ ||
        tmp_calc.total_sites_ > kMaxSitesPerCount * tmp_calc.total_counts_) {
        return;
    }
    ...
    tmp_calc.optimizer_poisson_.set_max_objective(BiasCalculationVectors::LogLikelihoodPoisson,
                                                  reinterpret_cast<void*>(&tmp_calc));
    ...
    try {
        if (nlopt::MAXEVAL_REACHED == tmp_calc.optimizer_poisson_.optimize(tmp_calc.fit_pars_, tmp_calc.loglike_)) {
            // printWarn << "Optimizer reached maximum number of likelihood calculations for seq " << ref_seq_bin << "
            // and length " << insert_length << std::endl;
            tmp_calc.func_calls_ += 10000;
            tmp_calc.converged_ = false;
        }

    } catch (const std::exception& e) {
        // printWarn << "Poisson optimizer crashed for seq " << ref_seq_bin << " and length " << insert_length << "
    ...
    tmp_calc.optimizer_spline_.set_max_objective(BiasCalculationVectors::LogLikeGcSpline,
                                                 reinterpret_cast<void*>(&tmp_calc));
    tmp_calc.GetGCSpline();

    // Fit negative binomial to downsampled sites with fixed biases to estimate dispersion parameters
    tmp_calc.optimizer_nbinom_.set_max_objective(BiasCalculationVectors::LogLikelihoodNbinom,
                                                 reinterpret_cast<void*>(&tmp_calc));
    ...
    try {
        ...
        // printWarn << "Nbinom optimizer crashed for seq " << ref_seq_bin << " and length " << insert_length << " with:
        // " << e.what() << std::endl;
        tmp_calc.func_calls_ += 20000;
        tmp_calc.converged_ = false;
    }
...
void FragmentDistributionStats::AcquireBiases(const BiasCalculationVectors& calc, mutex& print_mutex) {
    ...
    if (tmp_gc_bias_.at(0).size() > 20 && !(pars_fitted % (tmp_gc_bias_.at(0).size() / 20))) {
        ...
        printInfo << "Finished "
                  << static_cast<uintPercentPrint>(
                         Percent(pars_fitted, static_cast<uintNumFits>(tmp_gc_bias_.at(0).size())))
                  << "% of the bias fits." << std::endl;
    }
}
...
bool FragmentDistributionStats::StoreBias() {
    ...
    for (uintNumFits par = 0; par < tmp_dispersion_parameters_.size(); ++par) {
        ...
        dispersion_parameters_.at(par) =
            tmp_dispersion_parameters_.at(par).at(tmp_dispersion_parameters_.at(par).size() / 2);
...
double FragmentDistributionStats::MedianFragmentCoverage(const Reference& ref) const {
    ...
    for (uintRefSeqId ref_seq = 0; ref_seq < ref.NumberSequences(); ++ref_seq) {
        if (corrected_abundance_.at(ref_seq)) {
            ...
            frag_coverage.emplace_back(static_cast<double>(corrected_abundance_.at(ref_seq)) /
                                           ref.SequenceLength(ref_seq),
                                       filtered_sites_.at(ref_seq));
        }
    }
...
bool FragmentDistributionStats::CalculateInsertLengthAndRefSeqBias(const Reference& reference,
                                                                   uintNumThreads num_threads) {
    ...
    for (auto ref_seq = abundance_.size(); ref_seq--;) {
        if (0 == fragment_lengths_used_for_correction_.at(ref_seq) ||
            Divide(uintFragCount(filtered_sites_.at(ref_seq)),
                   uintFragCount(fragment_lengths_used_for_correction_.at(ref_seq))) < kMinSites) {
            corrected_abundance_.at(ref_seq) =
                0.0; // Do not fit reference sequences, where the corrected abundance is based on a low number of sites,
                     // instead later take the median ref_seq_bias
        }
    ...
    double normalization(
        static_cast<double>(sum_frag_len) /
        sum_ref_seq); // Due to adapter reads insert length and ref seq do have different sums, so we account for that
    ...
    for (uintRefSeqId ref_seq = 0; ref_seq < reference.NumberSequences(); ++ref_seq) {
        if (0.0 != ref_seq_bias_.at(ref_seq)) {
            ref_seq_bias_.at(ref_seq) /= static_cast<double>(reference.SequenceLength(ref_seq)) /
                                         (reference.SequenceLength(ref_seq) - reference.SumExcludedBases(ref_seq));
        }
    }

    // Replace the ref_seq_bias and corrected_abundance for non-fitted reference sequences with weighted median
    // ref_seq_bias and the corresponding corrected abundance
    uintFragCount total_filtered_sites(0);
    ...
    for (uintRefSeqId ref_seq = 0; ref_seq < reference.NumberSequences(); ++ref_seq) {
        if (0 == fragment_lengths_used_for_correction_.at(ref_seq) ||
            ...
            if (reference.ReferenceSequenceExcluded(ref_seq)) {
                ...
                corrected_abundance_.at(ref_seq) =
                    sum * reference.SequenceLength(ref_seq) /
                    (reference.SequenceLength(ref_seq) - reference.SumExcludedBases(ref_seq)) *
                    ref_seq_bias_.at(ref_seq);
            }
        }
    }
    ...
    printInfo << "Fitted reference sequence and insert length biases with a precision of " << precision << " needing "
              << iteration << " iteration" << (iteration == 1 ? "" : "s") << std::endl;
...
void FragmentDistributionStats::BiasSumThread(const FragmentDistributionStats& self, const Reference& reference,
                                              ...
                                              atomic<uintNumFits>& current_param, atomic<uintNumFits>& finished_params,
                                              vector<vector<double>>& bias_sum, std::mutex& print_mutex) {
    decltype(params.size()) cur_par(current_param++);

    for (; cur_par < params.size(); cur_par = current_param++) {
        bias_sum.at(params.at(cur_par).ref_seq_id).at(params.at(cur_par).fragment_length) =
            reference.SumBias(params.at(cur_par).ref_seq_id, params.at(cur_par).fragment_length, 1.0,
        ...
        if (params.size() > 20 && !(++finished_params % (params.size() / 20))) {
            ...
            printInfo << "Finished "
                      << static_cast<uintPercentPrint>(
                             Percent(finished_params, static_cast<uintNumFits>(params.size())))
                      << "% of the bias sums." << std::endl;
        }
    }
}
...
void FragmentDistributionStats::BiasNormalizationThread(
    const FragmentDistributionStats& self, const Reference& reference, const vector<BiasCalculationParams>& params,
    atomic<uintNumFits>& current_param, vector<double>& norm, mutex& result_mutex,
    const vector<uintRefSeqId>& coverage_groups, vector<vector<array<double, 2>>>& max_bias) {
    decltype(params.size()) cur_par(current_param++);
    ...
    for (; cur_par < params.size(); cur_par = current_param++) {
        tmp_norm.at(params.at(cur_par).fragment_length) += reference.SumBias(
            tmp_max_bias.at(coverage_groups.at(params.at(cur_par).ref_seq_id)).at(params.at(cur_par).fragment_length),
            params.at(cur_par).ref_seq_id, params.at(cur_par).fragment_length,
            self.ref_seq_bias_.at(params.at(cur_par).ref_seq_id) *
                self.insert_lengths_bias_.at(params.at(cur_par).fragment_length),
            self.gc_fragment_content_bias_, self.fragment_surroundings_bias_);
    }
...
double FragmentDistributionStats::CalculateNonZeroThreshold(double bias_normalization, double max_bias,
                                                            uintAlleleId num_alleles) const {
    // Threshold that random number from fragment generation has to reach or fragment count will be zero and does not
    // have to be calculated
    double max_mean = bias_normalization * max_bias;
    double max_dispersion(Dispersion(max_mean) / num_alleles);
    max_mean /= num_alleles;

    return pow(max_dispersion / (max_dispersion + max_mean),
               max_dispersion); // Lowest possible F(0) for negative binomial
}
...
void FragmentDistributionStats::SetUniformBias() {
    ...
    gc_fragment_content_bias_.Set(0, kGCBins, 1.0);

    fragment_surroundings_bias_.SetUniform();

    // For the insert_lengths_bias_ we follow the insert length distribution, because uniform is most likely not what
    // people want here
    insert_lengths_bias_.Clear();
...
reseq::uintRefSeqBin FragmentDistributionStats::CreateRefBins(const Reference& ref, uintSeqLen max_ref_seq_bin_size) {
    ...
    for (uintRefSeqId ref_seq = 1; ref_seq < ref_seq_start_bin_.size(); ++ref_seq) {
        start_id +=
            DivideAndCeil(ref.SequenceLength(ref_seq - 1) - ref.SumExcludedBases(ref_seq - 1), max_ref_seq_bin_length_);
    ...
    for (uintRefSeqId ref_seq = 0; ref_seq < ref_seq_start_bin_.size() - 1; ++ref_seq) {
        auto num_bins =
            DivideAndCeil(ref.SequenceLength(ref_seq) - ref.SumExcludedBases(ref_seq), max_ref_seq_bin_length_);
        if (num_bins) {
...
void FragmentDistributionStats::Prepare(const Reference& ref, uintSeqLen maximum_insert_length,
                                        uintSeqLen max_ref_seq_bin_size,
                                        const vector<uintFragCount>& reads_per_frag_len_bin,
                                        const vector<uintFragCount>& lowq_reads_per_frag_len_bin,
                                        uintNumThreads num_threads) {
    ...
    tmp_gc_fragment_content_.resize(kGCBins);
    ...
    PrepareBiasCalculation(ref, maximum_insert_length, max_ref_seq_bin_size, reads_per_frag_len_bin,
                           lowq_reads_per_frag_len_bin, num_threads);
}

void FragmentDistributionStats::FillInOutskirtContent(const Reference& reference,
                                                      const BamAlignmentRecord& record_start,
                                                      uintSeqLen fragment_start_pos, uintSeqLen fragment_end_pos) {
    bool reversed_fragment =
        hasFlagLast(record_start); // record is forward, so if it is sequenced last, fragment is reversed
    ...
    if (reversed_fragment) {
        ...
        for (; outskirt_pos < 2 * kOutskirtRange && ref_pos < reference.SequenceLength(record_start.rNextId);
             ++outskirt_pos) {
            if (!IsN(ref_seq, ref_pos)) {
                ++tmp_outskirt_content_.at(false).at(at(ref_seq, ref_pos)).at(outskirt_pos);
            }
...
void FragmentDistributionStats::HandleReferenceSequencesUntil(uintRefSeqId still_needed_reference_sequence,
    ...
    while (still_needed_ref_bin + 1 < ref_seq_start_bin_.at(still_needed_reference_sequence + 1) &&
           still_needed_position > ref_seq_bin_def_[still_needed_ref_bin + 1].second) {
...
bool FragmentDistributionStats::FinalizeBiasCalculation(const Reference& reference, uintNumThreads num_threads,
                                                        FragmentDuplicationStats& duplications) {
    if (calculate_bias_) {
        if (0 == current_bias_result_) {
            ...
            printInfo << current_bias_result_ << " of " << tmp_gc_bias_.at(0).size() << " bias fits ["
                      << static_cast<uintPercentPrint>(
                             Percent(current_bias_result_, static_cast<uintNumFits>(tmp_gc_bias_.at(0).size())))
                      << "%] converged." << std::endl;
    ...
    if (calculate_bias_) {
        ...
        printInfo << "Finished bias calculation excluding " << excluded_lowq_positions_ << " of " << ref_size << " ["
                  << static_cast<uintPercentPrint>(Percent(excluded_lowq_positions_, ref_size))
                  << "%] reference positions within " << excluded_lowq_regions_
                  << " regions due to low quality mappings" << std::endl;
    } else {
...
double FragmentDistributionStats::CorrectedCoverage(const Reference& ref, uintReadLen average_read_len,
                                                    const Vect<uintNucCount>& cov_dist) {
    ...
    if (corrected_coverage < cov_dist.to()) {
        ...
        if (seq_cov < cov_dist.to()) {
            // Find peak around coverage of longest sequence (peak ends in both directions when the value drops below
            // PeakMaxValue/kCovPeakFactor)
            ...
            while (
                (0 < left_pos || cov_dist.to() > right_pos + 1) &&
                ((max(left_min_after_max, min(left_min_before_max, min(right_min_before_max, right_min_after_max))) >=
                      left_max / kCovPeakFactor ||
                  left_max < right_max) &&
                 (max(right_min_after_max, min(right_min_before_max, min(left_min_before_max, left_min_after_max))) >=
                      right_max / kCovPeakFactor ||
                  left_max > right_max))) {
                if (0 < left_pos) {
            ...
            printInfo << "Found main coverage peak from " << peak_from << " to " << peak_to << " with the maximum at "
                      << peak_pos << '.' << std::endl;
            ...
            // Only accept the correction if it moves the coverage in the right direction (a safeguard against errors if
            // we are at the ends of the coverage vector)

            if ((new_corrected_coverage >= corrected_coverage && peak_pos >= sim_peak_cov) ||
                (new_corrected_coverage <= corrected_coverage && peak_pos <= sim_peak_cov)) {
...
bool FragmentDistributionStats::UpdateRefSeqBias(RefSeqBiasSimulation model, const std::string& bias_file,
    ...
    case RefSeqBiasSimulation::kKeep:
        if (ref.NumberSequences() == ref_seq_bias_.size()) {
            ...
            printWarn << "Cannot keep reference biases, because they don't match the reference. Will remove all "
                         "reference biases. Reference has "
    ...
    case RefSeqBiasSimulation::kFile: {
        ...
        if (fbias.is_open()) {
            ...
            while (getline(fbias, line)) {
                if (empty_line) {
                    if (errors < kMaxErrorsShownPerFile) {
                        printErr << "Error reading in reference sequence biases. Line " << nline << " is empty in file "
                                 << bias_file << std::endl;
                    }
                    ...
                    if (0 == line.size()) {
                        ...
                        if (bias_sep == string::npos) {
                            if (errors < kMaxErrorsShownPerFile) {
                                printErr << "Error reading in reference sequence biases. Line " << nline
                                         << " does not have a field separator in file " << bias_file << std::endl;
                            }
                            IncreaseErrorCounter(errors);
                        } else {
                            try {
                                ...
                                if (errors < kMaxErrorsShownPerFile) {
                                    printErr << "Could not convert bias starting at postion " << bias_sep + 1
                                             << " in line " << nline << " in file " << bias_file
                            ...
                            if (0.0 > bias) {
                                if (errors < kMaxErrorsShownPerFile) {
                                    printErr << "Bias in line " << nline << " in file " << bias_file
                                             << " is negative: " << bias << std::endl;
                                }
                                IncreaseErrorCounter(errors);
                            }
                            ...
                            if (id_len == string::npos) {
                                id_len = bias_sep; // No additional sequence description included, so just take the
                                                   // identifier
                            }
            ...
            for (uintRefSeqId seq = 0; seq < ref.NumberSequences(); ++seq) {
                if (!bias_found.at(seq)) {
                    if (errors < kMaxErrorsShownPerFile) {
                        printErr << "Could not find bias for reference sequence " << ref.ReferenceId(seq) << " in file "
                                 << bias_file << std::endl;
                    }
                    IncreaseErrorCounter(errors);
                }
            ...
            printInfo << "Read in " << ref_seq_bias_.size() << " biases from " << nline - (empty_line ? 1 : 0)
                      << " biases in file " << bias_file << std::endl;
        } else {
...
double FragmentDistributionStats::CalculateBiasNormalization(vector<uintRefSeqId>& coverage_groups,
                                                             vector<vector<array<double, 2>>>& non_zero_thresholds,
                                                             const Reference& reference, uintNumThreads num_threads,
                                                             uintFragCount total_reads) const {
    // Select the insert_length_sample_positions
    InsertLengthSpline insert_length_spline;
    if (!insert_length_spline.GetSamplePositions(insert_lengths_)) {
        return 0.0;
    ...
    for (auto& thresholds : non_zero_thresholds) {
        thresholds.resize(
            insert_lengths_.to(),
            {0.0, 0.0}); // First use for maximum bias: non_zero_thresholds[coverageGroup][insertLength] = maxBias;
    }
    ...
    for (auto& group : non_zero_thresholds) {
        ...
        for (uintSeqLen s = 1; s < insert_length_spline.sample_positions_.size(); ++s) {
            for (auto frag_len = insert_length_spline.sample_positions_.at(s - 1) + 1;
                 frag_len < insert_length_spline.sample_positions_.at(s); ++frag_len) {
                group.at(frag_len).at(0) = max_ratio * insert_lengths_bias_.at(frag_len);
            }
        }

        for (auto frag_len =
                 insert_length_spline.sample_positions_.at(insert_length_spline.sample_positions_.size() - 1) + 1;
             frag_len < group.size(); ++frag_len) {
            group.at(frag_len).at(0) = max_ratio * insert_lengths_bias_.at(frag_len);
        }
    }
    ...
    double full_normalization =
        total_reads /
        (normalization *
         2); // Bias is equal for both strands so it is only calculated for one, but the number of fragments at a site
             // is calculated separately per strand: This means we have to half the calculated number of reads
...
reseq::uintAlleleId FragmentDistributionStats::DrawNumberNonZeroStrands(uintAlleleId num_possible_alleles,
                                                                        double zero_probability,
                                                                        double probability_chosen) const {
...
reseq::uintDupCount FragmentDistributionStats::GetFragmentCounts(double bias_normalization, uintRefSeqId ref_seq_id,
                                                                 uintSeqLen fragment_length, uintPercent gc,
                                                                 const Surrounding& fragment_start,
                                                                 const Surrounding& fragment_end,
                                                                 double probability_chosen,
                                                                 uintAlleleId num_alleles) const {
    double bias(Reference::Bias(ref_seq_bias_.at(ref_seq_id), insert_lengths_bias_[fragment_length],
                                gc_fragment_content_bias_[gc], fragment_surroundings_bias_.Bias(fragment_start),
                                fragment_surroundings_bias_.Bias(fragment_end)));
    if (0.0 < bias) {
...
bool FragmentDistributionStats::WriteRefSeqBias(const std::string& bias_file, const Reference& reference) {
    if (reference.NumberSequences() != ref_seq_bias_.size()) {
        printErr << "Reference and reseq file do not match. The reference has " << reference.NumberSequences()
                 << " sequences and the reseq file has biases for " << ref_seq_bias_.size() << " sequences"
                 << std::endl;
        return false;
    }
