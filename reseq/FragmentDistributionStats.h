...
class BiasCalculationParamsSplitSeqs {
  public:
    ...
    BiasCalculationParamsSplitSeqs() : ref_seq_bin_(0), fragment_length_(0), bias_calculation_(false) {}

    BiasCalculationParamsSplitSeqs(uintRefSeqBin ref_seq_bin, uintSeqLen fragment_length, bool bias_calculation)
        : ref_seq_bin_(ref_seq_bin), fragment_length_(fragment_length), bias_calculation_(bias_calculation) {}
    ...
    void Clear(uintRefSeqBin ref_seq_bin) {
        ref_seq_bin_ = ref_seq_bin; // It is important to have the correct one in here, because it may define the vector
                                    // that is cleared and we don't want to clear an active one
...
class BiasCalculationVectors {
  ...
  public:
    ...
    static void PrepareSplines(std::array<T, 3>& lin_comb_splines, S& x, W& l, U& c, U& z, V& mu, V& h, T& beta);
    void PrepareSplines();
    template <typename T>
    static void GetSplineCoefficients(double& a, double& b, double& c, double& d, uintSeqLen k,
                                      const std::vector<double>& spline_pars, const std::array<T, 3>& lin_comb_splines);
    void GetSplineCoefficients(double& a, double& b, double& c, double& d, uintPercent k,
                               const std::vector<double>& spline_pars);
    void CalculateSpline(const std::vector<double>& spline_pars);
    void CalculateSplineGrad(const std::vector<double>& spline_pars, std::vector<double>& grad,
    ...
    void LogLikelihoodNbinomSite(double& loglike, double& a, double& b, double& ga, double& gb,
                                 std::vector<double>& grad, const FragmentSite& site);
...
class InsertLengthSpline {
  private:
    const uintSeqLen kInsertLengthSamplingDistance = 20; // Every kInsertLengthSamplingDistance length the bias is
                                                         // calculated and then a spline interpolates between them
  ...
  public:
    ...
    bool InterpolateNormalizationWithSpline(std::vector<double>& normalization,
...
class FragmentDistributionStats {
    ...
    class ThreadData {
      ...
      public:
        ThreadData(uintSeqLen maximum_insert_length, uintSeqLen start_exclusion_length)
            : last_bin_(0), last_added_region_(0), correction_(start_exclusion_length) {
  ...
  private:
    ...
    const uintCovCount kCovPeakFactor =
        10; // A coverage peak ends, when the value drops below PeakMaxValue/kCovPeakFactor
    ...
    std::vector<std::vector<std::vector<uintSeqLen>>>
        fragment_sites_by_ref_seq_bin_by_insert_length_; // fragment_sites_by_ref_seq_bin_by_insert_length_[ReferenceSequenceBinId][FragmentLength][UniqueId]
                                                         ...
                                                         // startPositionCorrectedForExcludedRegionsRelativeToBinStart*2
                                                         // + 0/1(forward/reverse)
    std::atomic<uintRefSeqBin> num_handled_reference_sequence_bins_; // num_handled_reference_sequence_bins_ =
                                                                     // #ofAlreadyHandledReferenceSequences =
                                                                     // IdOfFirstUnhandledReferenceSequence
    ...
    uintRefSeqId GetRefSeqId(uintRefSeqBin ref_seq_bin) { return ref_seq_bin_def_.at(ref_seq_bin).first; }
    inline void IncreaseErrorCounter(uintErrorCount& errors);
    void PrepareBiasCalculation(const Reference& ref, uintSeqLen maximum_insert_length, uintSeqLen max_ref_seq_bin_size,
                                const std::vector<uintFragCount>& reads_per_frag_len_bin,
                                const std::vector<uintFragCount>& lowq_reads_per_frag_len_bin,
                                uintNumThreads num_threads);
    inline void CheckLowQExclusion(std::vector<std::pair<uintSeqLen, uintSeqLen>>& lowq_site_exclusion,
    ...
    UpdateBiasCalculationParams(uintRefSeqBin ref_seq_bin, uint32_t queue_spot,
                                std::vector<std::pair<uintSeqLen, std::pair<uintRefSeqBin, uintSeqLen>>>& tmp_params,
                                std::mutex& print_mutex);
    void FillParams(std::vector<BiasCalculationParams>& params, const Reference& reference,
                    const std::vector<uintSeqLen>& sample_frag_length) const;
    void FillParamsSimulation(std::vector<BiasCalculationParams>& params, const Reference& reference,
                              const std::vector<uintSeqLen>& sample_frag_length) const;

    void CountEndExclusion(uintFragCount& lowq_sites, uintSeqLen& cur_end_exclusion, uintSeqLen last_cut,
                           uintSeqLen new_cut, uintRefSeqBin ref_seq_bin);
    void CountDuplicates(FragmentDuplicationStats& duplications, const BiasCalculationParamsSplitSeqs& params,
                         const Reference& reference);
    ...
    uintSeqLen ExcludeLowQualitySites(std::vector<FragmentSite>& sites, uintRefSeqBin ref_seq_bin,
                                      uintSeqLen insert_length);
    void CalculateBiasByBin(BiasCalculationVectors& tmp_calc, const Reference& reference,
    ...
    static void BiasNormalizationThread(const FragmentDistributionStats& self, const Reference& reference,
                                        const std::vector<BiasCalculationParams>& params,
                                        std::atomic<uintNumFits>& current_param, std::vector<double>& norm,
                                        std::mutex& result_mutex, const std::vector<uintRefSeqId>& coverage_groups,
  ...
  public:
    FragmentDistributionStats()
        : num_fits_per_ref_bin_(0),   // Will be set later depending on number of bins
          ...
          num_handled_reference_sequence_bins_(0), current_bias_result_(0),
          dispersion_parameters_(
              {0, 1e-100}) // Initialize with poisson to avoid writing random values into file if saving without fitting
    {}
    ...
    const std::vector<double>& FragmentSurroundingBiasByBase(uintBaseCall nucleotide) const {
        return fragment_surrounding_bias_by_base_.at(nucleotide);
    }
    ...
    void AddFragmentSite(uintRefSeqId ref_seq_id, uintSeqLen length, uintSeqLen position, uintTempSeq template_segment,
                         const Reference& reference, ThreadData& thread) {
        auto bin_coords = GetRefSeqBin(ref_seq_id, position, reference, thread);
        fragment_sites_by_ref_seq_bin_.at(bin_coords.first)
            .at(fragment_sites_by_ref_seq_bin_cur_id_.at(bin_coords.first)++) = {
            (bin_coords.second << 1) + template_segment, length};
    }
    void AddLowQSiteStart(uintRefSeqId ref_seq_id, uintSeqLen position, const Reference& reference,
                          ThreadData& thread) {
        auto bin_coords = GetRefSeqBin(ref_seq_id, position, reference, thread);
        lowq_site_start_by_ref_seq_bin_.at(bin_coords.first)
            .at(lowq_site_start_by_ref_seq_bin_cur_id_.at(bin_coords.first)++) = bin_coords.second;
    }
    void AddLowQSiteEnd(uintRefSeqId ref_seq_id, uintSeqLen position, const Reference& reference, ThreadData& thread) {
        auto bin_coords = GetRefSeqBin(ref_seq_id, position, reference, thread);
        lowq_site_end_by_ref_seq_bin_.at(bin_coords.first)
            .at(lowq_site_end_by_ref_seq_bin_cur_id_.at(bin_coords.first)++) = bin_coords.second;
        if (bin_coords.second < tmp_insert_lengths_.size() - 1 && bin_coords.first &&
            ref_seq_bin_def_.at(bin_coords.first).first == ref_seq_bin_def_.at(bin_coords.first - 1).first) {
            // Add position also to the previous bin as fragments starting there potentially end on this position
            lowq_site_end_by_ref_seq_bin_.at(bin_coords.first - 1)
                .at(lowq_site_end_by_ref_seq_bin_cur_id_.at(bin_coords.first - 1)++) =
                bin_coords.second + RefSeqSplitLength(ref_seq_id, reference);
        }
    }
    ...
    uintRefSeqBin GetRefSeqBin(uintRefSeqId ref_seq_id, uintSeqLen position, uintRefSeqBin last_bin) {
        ...
        while (new_bin + 1 < ref_seq_start_bin_.at(ref_seq_id + 1) &&
               ref_seq_bin_def_.at(new_bin + 1).second <= position) {
            ++new_bin;
        }
    ...
    std::pair<uintRefSeqBin, uintSeqLen> GetRefSeqBin(uintRefSeqId ref_seq_id, uintSeqLen position,
                                                      const Reference& reference, ThreadData& thread) {
        ...
        if (thread.last_bin_ < new_bin) {
            if (ref_seq_bin_def_.at(new_bin).first != ref_seq_bin_def_.at(thread.last_bin_).first) {
                ...
                thread.correction_ =
                    (new_bin - ref_seq_start_bin_.at(ref_seq_id)) * RefSeqSplitLength(ref_seq_id, reference) +
                    reference.StartExclusion(ref_seq_id);
            } else {
            ...
            if (ref_seq_bin_def_.at(new_bin).first != ref_seq_bin_def_.at(thread.last_bin_).first) {
                ...
                thread.correction_ =
                    (new_bin - ref_seq_start_bin_.at(ref_seq_id)) * RefSeqSplitLength(ref_seq_id, reference) +
                    reference.SumExcludedBases(ref_seq_id);
            } else {
                // Start positions at start of this bin and not the last
        ...
        reference.CorrectPositionRemovingExcludedRegions(thread.correction_, thread.last_added_region_, ref_seq_id,
                                                         position);
    ...
    uintSeqLen MaxRefSeqBinLength(const Reference& reference) {
        ...
        for (uintRefSeqId seq = 0; seq < reference.NumberSequences(); ++seq) {
            if (!reference.ReferenceSequenceExcluded(seq)) {
                utilities::SetToMax(max,
                                    RefSeqSplitLength(seq, reference) + ref_seq_start_bin_.at(seq + 1) -
                                        ref_seq_start_bin_.at(
                                            seq)); // Due to rounding the last bin of a reference sequence can have more
                                                   // than RefSeqSplitLength bases. The number of bin is an overestimate
                                                   // for this, but this little increase compared to RefSeqSplitLength
                                                   // doesn't justify more thinking for a better upper limit
            }
        }
        return max;
    }

    void Prepare(const Reference& ref, uintSeqLen maximum_insert_length, uintSeqLen max_ref_seq_bin_size,
                 const std::vector<uintFragCount>& reads_per_frag_len_bin,
                 const std::vector<uintFragCount>& lowq_reads_per_frag_len_bin, uintNumThreads num_threads);
    void FillInOutskirtContent(const Reference& reference, const seqan::BamAlignmentRecord& record_start,
    ...
    bool UpdateRefSeqBias(RefSeqBiasSimulation model, const std::string& bias_file, const Reference& ref,
                          std::mt19937_64& rgen);
    double CalculateBiasNormalization(std::vector<uintRefSeqId>& coverage_groups,
                                      std::vector<std::vector<std::array<double, 2>>>& non_zero_thresholds,
    ...
    uintAlleleId DrawNumberNonZeroStrands(uintAlleleId num_possible_alleles, double zero_probability,
                                          double probability_chosen) const;
    static uintDupCount NegativeBinomial(double p, double r, double probability_chosen);
    double Dispersion(double bias) const {
        return BiasCalculationVectors::GetDispersion(bias, dispersion_parameters_.at(0), dispersion_parameters_.at(1));
    }
    uintDupCount GetFragmentCounts(double bias_normalization, uintRefSeqId ref_seq_id, uintSeqLen fragment_length,
                                   uintPercent gc, const Surrounding& fragment_start, const Surrounding& fragment_end,
                                   double probability_chosen, uintAlleleId num_alleles = 1) const;
