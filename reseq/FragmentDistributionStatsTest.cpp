...
void FragmentDistributionStatsTest::SetExlusionRegionsAtEnds() {
    ...
    species_reference_.sum_of_excluded_bases_.resize(species_reference_.NumberSequences(),
                                                     2 * Reference::kMinDistToContigEnds);
    for (auto ref_seq = species_reference_.NumberSequences(); ref_seq--;) {
        ...
        species_reference_.excluded_regions_.at(ref_seq).emplace_back(species_reference_.SequenceLength(ref_seq) -
                                                                          Reference::kMinDistToContigEnds,
                                                                      species_reference_.SequenceLength(ref_seq));
    }
...
void FragmentDistributionStatsTest::AddExlusionRegionsDrosophila() {
    species_reference_.excluded_regions_.at(1200).emplace(species_reference_.excluded_regions_.at(1200).begin() + 1,
                                                          50000, 53000);
    species_reference_.sum_of_excluded_bases_.at(1200) += 3000;

    species_reference_.excluded_regions_.at(1225).emplace(species_reference_.excluded_regions_.at(1225).begin() + 1,
                                                          50000, 50001);
...
void FragmentDistributionStatsTest::TestOutskirtContent(const FragmentDistributionStats& test,
                                                        uintTempSeq template_segment, uintSeqLen at_pos,
                                                        uintNucCount cont_a, uintNucCount cont_c, uintNucCount cont_g,
                                                        uintNucCount cont_t, const char* context) {
    EXPECT_EQ(cont_a, test.outskirt_content_.at(template_segment).at(0)[at_pos])
        << "outskirt_content_[" << static_cast<uintTempSeqPrint>(template_segment) << "] position " << at_pos
        << " wrong for " << context << '\n';
    EXPECT_EQ(cont_c, test.outskirt_content_.at(template_segment).at(1)[at_pos])
        << "outskirt_content_[" << static_cast<uintTempSeqPrint>(template_segment) << "] position " << at_pos
        << " wrong for " << context << '\n';
    EXPECT_EQ(cont_g, test.outskirt_content_.at(template_segment).at(2)[at_pos])
        << "outskirt_content_[" << static_cast<uintTempSeqPrint>(template_segment) << "] position " << at_pos
        << " wrong for " << context << '\n';
    EXPECT_EQ(cont_t, test.outskirt_content_.at(template_segment).at(3)[at_pos])
        << "outskirt_content_[" << static_cast<uintTempSeqPrint>(template_segment) << "] position " << at_pos
        << " wrong for " << context << '\n';
}
...
void FragmentDistributionStatsTest::CreateCoverageDataHelper(uintPercent gc_perc, uintSeqLen start_pos,
                                                             uintSeqLen fragment_length, mt19937_64& rgen) {
    ...
    for (uintSeqLen entries = count; entries--;) {
        test_->fragment_sites_by_ref_seq_bin_.at(ref_seq_id)
            .push_back({(start_pos - Reference::kMinDistToContigEnds) << 1, fragment_length}); // Forward
    ...
    for (uintSeqLen entries = count; entries--;) {
        test_->fragment_sites_by_ref_seq_bin_.at(ref_seq_id)
            .push_back({((start_pos - Reference::kMinDistToContigEnds) << 1) + 1, fragment_length}); // Reverse
        ++test_->fragment_sites_by_ref_seq_bin_cur_id_.at(ref_seq_id);
    }
}
...
void FragmentDistributionStatsTest::CreateCoverageData(uintSeqLen fragment_length) {
    ...
    uintSeqLen gc = species_reference_.GCContentAbsolut(n_count, ref_seq_id, dist_ref_seq_ends,
                                                        dist_ref_seq_ends + fragment_length);
    ...
    for (uintSeqLen start_pos = dist_ref_seq_ends;
         ++start_pos < length(ref_seq) - fragment_length - dist_ref_seq_ends - 1;) {
...
template <size_t N>
void FragmentDistributionStatsTest::CheckDrawnCounts(double bias, std::array<double, N> thresholds,
                                                     const Surrounding& start_sur, const Surrounding& end_sur) {
    ...
    for (uintDupCount i = 0; i < thresholds.size(); ++i) {
        EXPECT_EQ(
            i, test_->GetFragmentCounts(bias_normalization, 0, 367, 43, start_sur, end_sur, thresholds.at(i) - delta))
            << "Lower gate: " << i << ' ' << thresholds.at(i) << " for bias " << bias << " dispersion pars "
            << test_->dispersion_parameters_.at(0) << ' ' << test_->dispersion_parameters_.at(1) << std::endl;
        EXPECT_EQ(i + 1, test_->GetFragmentCounts(bias_normalization, 0, 367, 43, start_sur, end_sur,
                                                  thresholds.at(i) + delta))
            << "Upper gate: " << i << ' ' << thresholds.at(i) << " for bias " << bias << " dispersion pars "
            << test_->dispersion_parameters_.at(0) << ' ' << test_->dispersion_parameters_.at(1) << std::endl;
    }

    // Test CalculateNonZeroThreshold
    auto non_zero_threshold = test_->CalculateNonZeroThreshold(
        bias_normalization, test_->gc_fragment_content_bias_[43] / other_bias_negation / bias_normalization);
    EXPECT_NEAR(thresholds[0], non_zero_threshold, delta)
        << "Bias " << bias << " dispersion pars " << test_->dispersion_parameters_.at(0) << ' '
        << test_->dispersion_parameters_.at(1) << std::endl;
    EXPECT_EQ(0,
              test_->GetFragmentCounts(bias_normalization - delta, 0, 367, 43, start_sur, end_sur, non_zero_threshold))
        << "Bias " << bias << " dispersion pars " << test_->dispersion_parameters_.at(0) << ' '
        << test_->dispersion_parameters_.at(1) << std::endl;
    EXPECT_EQ(0, test_->GetFragmentCounts(1e-10, 0, 367, 43, start_sur, end_sur, non_zero_threshold))
        << "Bias " << bias << " dispersion pars " << test_->dispersion_parameters_.at(0) << ' '
        << test_->dispersion_parameters_.at(1) << std::endl;
}
...
void FragmentDistributionStatsTest::TestSrr490124Equality(const FragmentDistributionStats& test, const char* context) {
    ...
    // samtools view -q 10 ecoli-SRR490124-4pairs.sam | awk '(NR <= 3 || NR == 12){if(1==NR%2){store=$4}else{print
    // store, $4, $4-store+length($10)}}'
    ...
    // samtools view -q 10 ecoli-SRR490124-4pairs.sam | awk '(NR <= 3 || NR ==
    // 12){if(1==NR%2){store=$4}else{system("samtools faidx ecoli-GCF_000005845.2_ASM584v2_genomic.fa " $3 ":" store "-"
    // $4+length($10)-1)}}' | seqtk seq | awk '(0==NR%2){len=length($1); print (gsub("G","",$1)+gsub("C","",$1))/len}'
    EXPECT_EQ(2, test.gc_fragment_content_.size())
        << "SRR490124-4pairs gc_fragment_content_ wrong for " << context << '\n';
    EXPECT_EQ(8, test.gc_fragment_content_[52])
        << "SRR490124-4pairs gc_fragment_content_ wrong for " << context << '\n';
    EXPECT_EQ(1, test.gc_fragment_content_[53])
        << "SRR490124-4pairs gc_fragment_content_ wrong for " << context << '\n';

    // cat <(samtools view ecoli-SRR490124-4pairs.bam -q 10 -f 144 -F 32 | awk '($4+2000>$8){system("samtools faidx
    // ecoli-GCF_000005845.2_ASM584v2_genomic.fa NC_000913.3:" $8-10 "-" $4+length($10)-1+10)}' | seqtk seq) <(samtools
    // view ecoli-SRR490124-4pairs.bam -q 10 -f 80 -F 32 | awk '($4+2000>$8){system("samtools faidx
    // ecoli-GCF_000005845.2_ASM584v2_genomic.fa NC_000913.3:" $8-10 "-" $4+length($10)-1+10)}' | seqtk seq -r) | awk
    // '(0==NR%2){print ">0", substr($0,1,10); print substr($0,length($0)-9,10); print ">1", substr($0,11,10); print
    // substr($0,length($0)-19,10);print ">2", substr($0,21,10); print substr($0,length($0)-29,10);}' | seqtk seq -r |
    // awk '{if(1==NR%2){sur=substr($0, 2, 1); print "start", sur, $2}else{print "end", sur, $0}}' | awk
    // 'BEGIN{d["A"]=0;d["C"]=1;d["G"]=2;d["T"]=3}{mult=1;sur=0;for(i=length($3);i>0;i-=1){sur+=mult*d[substr($3,i,1)];mult*=4};
    // print $1, $2, $3, sur}' | sort
    EXPECT_EQ(8, test.fragment_surroundings_.counts_.at(0).at(39619))
        << "SRR490124-4pairs fragment_surroundings_ not correct for " << context << '\n';
    EXPECT_EQ(1, test.fragment_surroundings_.counts_.at(0).at(1009062))
        << "SRR490124-4pairs fragment_surroundings_ not correct for " << context << '\n';
    EXPECT_EQ(8, test.fragment_surroundings_.counts_.at(1).at(561168))
        << "SRR490124-4pairs fragment_surroundings_ not correct for " << context << '\n';
    EXPECT_EQ(1, test.fragment_surroundings_.counts_.at(1).at(996771))
        << "SRR490124-4pairs fragment_surroundings_ not correct for " << context << '\n';
    EXPECT_EQ(1, test.fragment_surroundings_.counts_.at(2).at(307041))
        << "SRR490124-4pairs fragment_surroundings_ not correct for " << context << '\n';
    EXPECT_EQ(8, test.fragment_surroundings_.counts_.at(2).at(856654))
        << "SRR490124-4pairs fragment_surroundings_ not correct for " << context << '\n';
    EXPECT_EQ(8, test.fragment_surroundings_.counts_.at(0).at(607462))
        << "SRR490124-4pairs fragment_surroundings_ not correct for " << context << '\n';
    EXPECT_EQ(1, test.fragment_surroundings_.counts_.at(0).at(983881))
        << "SRR490124-4pairs fragment_surroundings_ not correct for " << context << '\n';
    EXPECT_EQ(8, test.fragment_surroundings_.counts_.at(1).at(354618))
        << "SRR490124-4pairs fragment_surroundings_ not correct for " << context << '\n';
    EXPECT_EQ(1, test.fragment_surroundings_.counts_.at(1).at(765926))
        << "SRR490124-4pairs fragment_surroundings_ not correct for " << context << '\n';
    EXPECT_EQ(1, test.fragment_surroundings_.counts_.at(2).at(333453))
        << "SRR490124-4pairs fragment_surroundings_ not correct for " << context << '\n';
    EXPECT_EQ(8, test.fragment_surroundings_.counts_.at(2).at(405510))
        << "SRR490124-4pairs fragment_surroundings_ not correct for " << context << '\n';

    // samtools view -q 10 ecoli-SRR490124-4pairs.sam | awk '(163==$2){system("samtools faidx
    // ecoli-GCF_000005845.2_ASM584v2_genomic.fa " $3 ":" $4-20 "-" $4-1)}(83==$2){system("samtools faidx
    // ecoli-GCF_000005845.2_ASM584v2_genomic.fa " $3 ":" $4+length($10) "-" $4+length($10)+19)}' | awk
    // '(NR==7){store1=$0}(NR==8){store2=$0}(NR != 7 && NR !=8){print $0}(NR==10){print store1; print store2}' | seqtk
    // seq -r | awk '(2==NR%4){store=$0}(0==NR%4){print $0 store}' | awk '{for(pos=1;pos<=length($0);pos+=1){print
    // substr($0,pos,1), pos-1}}' | sort | uniq -c | sort -k2,2 -k3,3n
    EXPECT_EQ(0, test.outskirt_content_.at(0).at(0).size())
        << "SRR490124-4pairs outskirt_content_[0][0].size() not correct for " << context << '\n';
    EXPECT_EQ(0, test.outskirt_content_.at(0).at(1).size())
        << "SRR490124-4pairs outskirt_content_[0][1].size() not correct for " << context << '\n';
    EXPECT_EQ(0, test.outskirt_content_.at(0).at(2).size())
        << "SRR490124-4pairs outskirt_content_[0][2].size() not correct for " << context << '\n';
    EXPECT_EQ(0, test.outskirt_content_.at(0).at(3).size())
        << "SRR490124-4pairs outskirt_content_[0][3].size() not correct for " << context << '\n';
    EXPECT_EQ(37, test.outskirt_content_.at(1).at(0).size())
        << "SRR490124-4pairs outskirt_content_[1][0].size() not correct for " << context << '\n';
    EXPECT_EQ(36, test.outskirt_content_.at(1).at(1).size())
        << "SRR490124-4pairs outskirt_content_[1][1].size() not correct for " << context << '\n';
    EXPECT_EQ(40, test.outskirt_content_.at(1).at(2).size())
        << "SRR490124-4pairs outskirt_content_[1][2].size() not correct for " << context << '\n';
    EXPECT_EQ(37, test.outskirt_content_.at(1).at(3).size())
        << "SRR490124-4pairs outskirt_content_[1][3].size() not correct for " << context << '\n';
    // samtools view -q 10 ecoli-SRR490124-4pairs.sam | awk '(163==$2){system("samtools faidx
    // ecoli-GCF_000005845.2_ASM584v2_genomic.fa " $3 ":" $4-20 "-" $4-1)}(83==$2){system("samtools faidx
    // ecoli-GCF_000005845.2_ASM584v2_genomic.fa " $3 ":" $4+length($10) "-" $4+length($10)+19)}' | awk
    // '(NR==7){store1=$0}(NR==8){store2=$0}(NR != 7 && NR !=8){print $0}(NR==10){print store1; print store2}' | seqtk
    // seq -r | awk '(2==NR%4){store=$0}(0==NR%4){print $0 store}' | awk '{for(pos=1;pos<=length($0);pos+=1){print
    // substr($0,pos,1), pos-1}}' | sort | uniq -c | sort -k3,3n -k2,2
...
void FragmentDistributionStatsTest::TestDuplicates(const FragmentDistributionStats& test) {
    // samtools view -q 10 -f 3 ecoli-duplicates.bam | awk '(0 != substr($1,12,1) && ($4>$8 || $4==$8 &&
    // int($2%256/128))){len=0; for(i=1;i<=length($6);i+=1){e=substr($6,i,1);if(e ~
    // /^[0-9]/){num=num*10+e}else{if("M"==e || "D"==e){len+=num}; num=0}}; print $4-$8+len}' | sort -n | uniq -c
...
void FragmentDistributionStatsTest::TestAdapters(const FragmentDistributionStats& test, const char* context, bool bwa) {
    ...
    EXPECT_EQ(1, test.insert_lengths_[80])
        << "insert_lengths_[80] wrong with low mapping quality adapters for " << context;
    ...
    if (bwa) {
        // samtools view ecoli-SRR490124-adapter-bwa.bam -q 10 -f 16 -F 32 | awk '($4+2000>$8){system("samtools faidx
        // ecoli-GCF_000005845.2_ASM584v2_genomic.fa NC_000913.3:" $8 "-" $8-$9-1)}' | seqtk seq | awk
        // '(0==NR%2){len=length($0); print int(gsub(/[CG]/,"",$0)*100/len+0.5)}' | sort -n
        ...
        // samtools view ecoli-SRR490124-adapter.bam -q 10 -f 16 -F 32 | awk '($4+2000>$8){system("samtools faidx
        // ecoli-GCF_000005845.2_ASM584v2_genomic.fa NC_000913.3:" $8 "-" $4+length($10)-1)}' | seqtk seq | awk
        // '(0==NR%2){len=length($0); print int(gsub(/[CG]/,"",$0)*100/len+0.5)}' | sort -n
    ...
    // cat <(samtools view ecoli-SRR490124-adapter.bam -q 10 -f 144 -F 32 | awk '($4+2000>$8){system("samtools faidx
    // ecoli-GCF_000005845.2_ASM584v2_genomic.fa NC_000913.3:" $8-10 "-" $4+length($10)-1+10)}' | seqtk seq) <(samtools
    // view ecoli-SRR490124-adapter.bam -q 10 -f 80 -F 32 | awk '($4+2000>$8){system("samtools faidx
    // ecoli-GCF_000005845.2_ASM584v2_genomic.fa NC_000913.3:" $8-10 "-" $4+length($10)-1+10)}' | seqtk seq -r) | awk
    // '(0==NR%2){print ">0", substr($0,1,10); print substr($0,length($0)-9,10); print ">1", substr($0,11,10); print
    // substr($0,length($0)-19,10);print ">2", substr($0,21,10); print substr($0,length($0)-29,10);}' | seqtk seq -r |
    // awk '{if(1==NR%2){sur=substr($0, 2, 1); print "start", sur, $2}else{print "end", sur, $0}}' | awk
    ...
    if (bwa) {
        // samtools view ecoli-SRR490124-adapter-bwa.bam -q 10 -f 80 -F 32 | awk '($4+2000>$8){system("samtools faidx
        // ecoli-GCF_000005845.2_ASM584v2_genomic.fa NC_000913.3:" $8-20 "-" $8-$9-1+20)}' | seqtk seq -r | awk
        // '(0==NR%2){print substr($0,1,20) substr($0,length($0)-19,20)}' | awk '{for(pos=1;pos<=length($0);++pos){print
        // substr($0,pos,1), pos-1}}' | sort -k1,1 -k2,2n | uniq -c
        ...
        // samtools view ecoli-SRR490124-adapter.bam -q 10 -f 80 -F 32 | awk '($4+2000>$8){system("samtools faidx
        // ecoli-GCF_000005845.2_ASM584v2_genomic.fa NC_000913.3:" $8-20 "-" $4+length($10)-1+20)}' | seqtk seq -r | awk
        // '(0==NR%2){print substr($0,1,20) substr($0,length($0)-19,20)}' | awk '{for(pos=1;pos<=length($0);++pos){print
        // 1, substr($0,pos,1), pos-1}}' | sort -k1,2 -k3,3n
...
void FragmentDistributionStatsTest::TestBiasCalculationVectorsPreprocessing() {
    ...
    test.GetCounts();
    // cat <(seqtk seq reference-test-withN.fa | awk '(2==NR)') | awk '{for(i=51;i<=length($0)-50-10; i+=1){print
    // substr($0,i-10,30), i-51}}' | awk '{print ">"$1, $2; print $1}' | seqtk seq -r | awk
    // '(1==NR%2){forward=substr($1,2,length($1)-1); pos=$2}(0==NR%2){print pos, substr(forward,11,10), forward, $0}' |
    // awk '(0==$1 || 40==$1 || 50==$1 || 112==$1 || 331==$1){print gsub(/[GC]/,"",$2)}' | sort | uniq -c
    ...
    // substr($0,i-10,30), i-51}}' | awk '{print ">"$1, $2; print $1}' | seqtk seq -r | awk
    // '(1==NR%2){forward=substr($1,2,length($1)-1); pos=$2}(0==NR%2){print pos, substr(forward,11,10), forward, $0}' |
    // awk '(0==$1 || 40==$1 || 50==$1 || 112==$1 || 331==$1){pos=5; print substr($3,pos+1,1); print
    // substr($4,pos+1,1)}' | sort | uniq -c
    ...
    // substr($0,i-10,30), i-51}}' | awk '{print ">"$1, $2; print $1}' | seqtk seq -r | awk
    // '(1==NR%2){forward=substr($1,2,length($1)-1); pos=$2}(0==NR%2){print pos, substr(forward,11,10), forward, $0}' |
    // awk '(0==$1 || 40==$1 || 50==$1 || 112==$1 || 331==$1){print gsub(/[GC]/,"",$2), substr($3,1,10),
    // substr($3,11,10), substr($3,21,10), substr($4,1,10), substr($4,11,10), substr($4,21,10)}' removes also not needed
    // gc, which is not the implementation anymore: cat <(seqtk seq reference-test-withN.fa | awk '(2==NR)') | awk
    // '{for(i=51;i<=length($0)-50-10; i+=1){print substr($0,i-10,30), i-51}}' | awk '{print ">"$1, $2; print $1}' |
    // seqtk seq -r | awk '(1==NR%2){forward=substr($1,2,length($1)-1); pos=$2}(0==NR%2){print pos,
    // substr(forward,11,10), forward, $0}' | awk '(0 == gsub("N","",$3)){print gsub(/[GC]/,"",$2), substr($3,1,10),
    // substr($3,11,10), substr($3,21,10), substr($4,1,10), substr($4,11,10), substr($4,21,10), $1}' | awk '((0 == $1 ||
    // 3 == $1 || 4 == $1 || 5 == $1) && substr($2,3,1) != "C" && substr($5,3,1) != "C" && substr($2,7,1) != "C" &&
    // substr($5,7,1) != "C" && substr($2,7,1) != "G" && substr($5,7,1) != "G" && substr($3,4,1) != "C" &&
    // substr($6,4,1) != "C" && substr($3,7,1) != "G" && substr($6,7,1) != "G" && substr($4,2,1) != "C" &&
    // substr($7,2,1) != "C" && substr($4,4,1) != "C" && substr($7,4,1) != "C" && substr($4,4,1) != "G" &&
    // substr($7,4,1) != "G" && substr($4,8,1) != "G" && substr($7,8,1) != "G")' | wc -l cat <(seqtk seq
    // reference-test-withN.fa | awk '(2==NR)') | awk '{for(i=51;i<=length($0)-50-10; i+=1){print substr($0,i-10,30),
    // i-51}}' | awk '{print ">"$1, $2; print $1}' | seqtk seq -r | awk '(1==NR%2){forward=substr($1,2,length($1)-1);
    // pos=$2}(0==NR%2){print pos, substr(forward,11,10), forward, $0}' | awk '(0 == gsub("N","",$3)){print
    // gsub(/[GC]/,"",$2), substr($3,1,10), substr($3,11,10), substr($3,21,10), substr($4,1,10), substr($4,11,10),
    // substr($4,21,10), $1}' | awk '(substr($2,3,1) != "C" && substr($5,3,1) != "C" && substr($2,7,1) != "C" &&
    // substr($5,7,1) != "C" && substr($2,7,1) != "G" && substr($5,7,1) != "G" && substr($3,4,1) != "C" &&
    // substr($6,4,1) != "C" && substr($3,7,1) != "G" && substr($6,7,1) != "G" && substr($4,2,1) != "C" &&
    // substr($7,2,1) != "C" && substr($4,4,1) != "C" && substr($7,4,1) != "C" && substr($4,4,1) != "G" &&
    // substr($7,4,1) != "G" && substr($4,8,1) != "G" && substr($7,8,1) != "G")' | wc -l
    EXPECT_EQ(26, test.sites_.size());

    // cat <(seqtk seq reference-test-withN.fa | awk '(2==NR)') | awk '{for(i=51;i<=length($0)-50-10; i+=1){print
    // substr($0,i-10,30), i-51}}' | awk '{print ">"$1, $2; print $1}' | seqtk seq -r | awk
    // '(1==NR%2){forward=substr($1,2,length($1)-1); pos=$2}(0==NR%2){print pos, substr(forward,11,10), forward, $0}' |
    // awk '(0 == gsub("N","",$3)){print gsub(/[GC]/,"",$2), substr($3,1,10), substr($3,11,10), substr($3,21,10),
    // substr($4,1,10), substr($4,11,10), substr($4,21,10), $1}' | awk '((0 == $1 || 3 == $1 || 4 == $1 || 5 == $1) &&
    // substr($2,3,1) != "C" && substr($5,3,1) != "C" && substr($2,7,1) != "C" && substr($5,7,1) != "C" &&
    // substr($2,7,1) != "G" && substr($5,7,1) != "G" && substr($3,4,1) != "C" && substr($6,4,1) != "C" &&
    // substr($3,7,1) != "G" && substr($6,7,1) != "G" && substr($4,2,1) != "C" && substr($7,2,1) != "C" &&
    // substr($4,4,1) != "C" && substr($7,4,1) != "C" && substr($4,4,1) != "G" && substr($7,4,1) != "G" &&
    // substr($4,8,1) != "G" && substr($7,8,1) != "G"){print $1}' | sort | uniq -c
    ...
    // '(1==NR%2){forward=substr($1,2,length($1)-1); pos=$2}(0==NR%2){print pos, substr(forward,11,10), forward, $0}' |
    // awk '(0 == gsub("N","",$3)){print gsub(/[GC]/,"",$2), substr($3,1,10), substr($3,11,10), substr($3,21,10),
    // substr($4,1,10), substr($4,11,10), substr($4,21,10), $1}' | awk '(substr($2,3,1) != "C" && substr($5,3,1) != "C"
    // && substr($2,7,1) != "C" && substr($5,7,1) != "C" && substr($2,7,1) != "G" && substr($5,7,1) != "G" &&
    // substr($3,4,1) != "C" && substr($6,4,1) != "C" && substr($3,7,1) != "G" && substr($6,7,1) != "G" &&
    // substr($4,2,1) != "C" && substr($7,2,1) != "C" && substr($4,4,1) != "C" && substr($7,4,1) != "C" &&
    // substr($4,4,1) != "G" && substr($7,4,1) != "G" && substr($4,8,1) != "G" && substr($7,8,1) != "G"){pos=5; print
    // substr($2,pos+1,1); print substr($5,pos+1,1)}' | sort | uniq -c
    ...
    // substr($3,4,1) != "C" && substr($6,4,1) != "C" && substr($3,7,1) != "G" && substr($6,7,1) != "G" &&
    // substr($4,2,1) != "C" && substr($7,2,1) != "C" && substr($4,4,1) != "C" && substr($7,4,1) != "C" &&
    // substr($4,4,1) != "G" && substr($7,4,1) != "G" && substr($4,8,1) != "G" && substr($7,8,1) != "G"){pos=3; print
    // substr($3,pos+1,1); print substr($6,pos+1,1)}' | sort | uniq -c
...
void FragmentDistributionStatsTest::TestBiasCalculationVectorsSpline() {
    ...
    // void GetSplineCoefficients(double &a, double &b, double &c, double &d, uintPercent k, const std::vector<double>
    // &spline_pars); void CalculateSpline(const std::vector<double> &spline_pars); void CalculateSplineGrad(const
    // std::vector<double> &spline_pars, std::vector<double> &grad, uintNumFits grad_offset); void GetGCSpline();
}
...
void FragmentDistributionStatsTest::TestBiasCalculationVectorsLikelihoods() {
    ...
    // static double LogLikelihoodConstDispersion(const std::vector<double> &x, std::vector<double> &grad, void*
    // f_data);
}
...
void FragmentDistributionStatsTest::TestBiasCalculation() {
    ...
    array<FragmentDistributionStats::ThreadData, 4> thread_data = {
        FragmentDistributionStats::ThreadData(100, Reference::kMinDistToContigEnds),
        FragmentDistributionStats::ThreadData(100, Reference::kMinDistToContigEnds),
        FragmentDistributionStats::ThreadData(100, Reference::kMinDistToContigEnds),
        FragmentDistributionStats::ThreadData(100, Reference::kMinDistToContigEnds)};
    mutex print_mutex;
    ...
    EXPECT_DOUBLE_EQ(1.0,
                     *max_element(test_->gc_fragment_content_bias_.begin(), test_->gc_fragment_content_bias_.end()));

    Surrounding::intType pos_base = 262144;
    EXPECT_NEAR(-0.4,
                accumulate(test_->fragment_surroundings_bias_.bias_.at(1).begin(),
                           test_->fragment_surroundings_bias_.bias_.at(1).begin() + pos_base, 0.0) /
                    pos_base,
                0.2);
    EXPECT_NEAR(-0.4,
                accumulate(test_->fragment_surroundings_bias_.bias_.at(1).begin() + pos_base,
                           test_->fragment_surroundings_bias_.bias_.at(1).begin() + 2 * pos_base, 0.0) /
                    pos_base,
                0.2);
    EXPECT_NEAR(0.4,
                accumulate(test_->fragment_surroundings_bias_.bias_.at(1).begin() + 2 * pos_base,
                           test_->fragment_surroundings_bias_.bias_.at(1).begin() + 3 * pos_base, 0.0) /
                    pos_base,
                0.2);
    EXPECT_NEAR(0.4,
                accumulate(test_->fragment_surroundings_bias_.bias_.at(1).begin() + 3 * pos_base,
                           test_->fragment_surroundings_bias_.bias_.at(1).begin() + 4 * pos_base, 0.0) /
                    pos_base,
                0.2);

    // Testing the fragment length makes no sense if we only have a few fragment length, adding a separate estimation
    // test would be good
    // for(uintSeqLen fragment_length=65; fragment_length <= 95; fragment_length += 5){
    //	EXPECT_NEAR(1.0, test_->insert_lengths_bias_.at(fragment_length), 0.1) << "Fragment length " << fragment_length
    //<< " wrong\n";
    //}
...
void FragmentDistributionStatsTest::TestUniformBias() {
    ...
    EXPECT_DOUBLE_EQ(Surrounding::Size(), accumulate(test_->fragment_surroundings_bias_.bias_.at(0).begin(),
                                                     test_->fragment_surroundings_bias_.bias_.at(0).end(), 0.0));
    EXPECT_DOUBLE_EQ(Surrounding::Size(), accumulate(test_->fragment_surroundings_bias_.bias_.at(1).begin(),
                                                     test_->fragment_surroundings_bias_.bias_.at(1).end(), 0.0));
    EXPECT_DOUBLE_EQ(Surrounding::Size(), accumulate(test_->fragment_surroundings_bias_.bias_.at(2).begin(),
                                                     test_->fragment_surroundings_bias_.bias_.at(2).end(), 0.0));
    EXPECT_DOUBLE_EQ(1.0, *max_element(test_->fragment_surroundings_bias_.bias_.at(0).begin(),
                                       test_->fragment_surroundings_bias_.bias_.at(0).end()));
    EXPECT_DOUBLE_EQ(1.0, *max_element(test_->fragment_surroundings_bias_.bias_.at(1).begin(),
                                       test_->fragment_surroundings_bias_.bias_.at(1).end()));
    EXPECT_DOUBLE_EQ(1.0, *max_element(test_->fragment_surroundings_bias_.bias_.at(2).begin(),
                                       test_->fragment_surroundings_bias_.bias_.at(2).end()));
}

void FragmentDistributionStatsTest::TestDrawCounts() {
    // Test DrawNumberNonZeroStrands( uintAlleleId num_possible_alleles, double zero_probability, double
    // probability_chosen )
    ...
    for (auto& block : test_->fragment_surroundings_bias_.bias_) {
        block.clear();
        block.resize(Surrounding::Size(),
                     -1000.0); // Set to big negative number, so that bias will be very close to zero
    }
    ...
    CheckDrawnCounts(1.0, array<double, 6>({0.3678794, 0.7357589, 0.9196986, 0.9810118, 0.9963402, 0.9994058}),
                     start_sur, end_sur); // R: ppois(0:5, 1.0)
    CheckDrawnCounts(0.5, array<double, 6>({0.6065307, 0.9097960, 0.9856123, 0.9982484, 0.9998279, 0.9999858}),
                     start_sur, end_sur); // R: ppois(0:5, 0.5)
    CheckDrawnCounts(0.1, array<double, 4>({0.9048374, 0.9953212, 0.9998453, 0.9999962}), start_sur,
                     end_sur); // R: ppois(0:5, 0.1)

    // Constant dispersion
    test_->dispersion_parameters_.at(1) = 5.0;
    CheckDrawnCounts(1.0, array<double, 6>({0.6988271, 0.8152983, 0.8735339, 0.9091223, 0.9328479, 0.9494559}),
                     start_sur, end_sur); // R: pnbinom(0:5, size=0.2, mu=1.0)
    CheckDrawnCounts(0.5, array<double, 6>({0.7783705, 0.8895663, 0.9372217, 0.9621840, 0.9764482, 0.9850067}),
                     start_sur, end_sur); // R: pnbinom(0:5, size=0.2, mu=0.5)
    CheckDrawnCounts(0.1, array<double, 6>({0.9221079, 0.9835818, 0.9958765, 0.9988819, 0.9996834, 0.9999078}),
                     start_sur, end_sur); // R: pnbinom(0:5, size=0.2, mu=0.1)

    // Bias dependent dispersion
    test_->dispersion_parameters_.at(0) = 5000.0;
    test_->dispersion_parameters_.at(1) = 10000.0;

    CheckDrawnCounts(1.0, array<double, 6>({0.9993591, 0.9994258, 0.9994591, 0.9994813, 0.9994979, 0.9995113}),
                     start_sur, end_sur); // R: pnbinom(0:5, size=1.0/(5000+1.0*10000), mu=1.0)
    CheckDrawnCounts(0.5, array<double, 6>({0.9995396, 0.9995896, 0.9996145, 0.9996312, 0.9996437, 0.9996537}),
                     start_sur, end_sur); // R: pnbinom(0:5, size=0.5/(5000+0.5*10000), mu=0.5)
    CheckDrawnCounts(0.1, array<double, 6>({0.9998550, 0.9998717, 0.9998800, 0.9998856, 0.9998897, 0.9998931}),
                     start_sur, end_sur); // R: pnbinom(0:5, size=0.1/(5000+0.1*10000), mu=0.1)
}
...
void FragmentDistributionStatsTest::TestRefSeqSplitting() {
    ...
    EXPECT_EQ(1230 + 17 + 3,
              test_->CreateRefBins(
                  species_reference_,
                  40000)); // Maximum sequence length is 88768, so 40000 to have sequences with more than one cut)
    ...
    FragmentDistributionStats::ThreadData thread(
        0, Reference::kMinDistToContigEnds); // Set the first two to zero so no vector space is acquired
    array<uintRefSeqId, 18> ref_seq_ids = {0,    0,    0,    0,    1,    1,    1200, 1200, 1200,
                                           1200, 1200, 1200, 1225, 1225, 1225, 1225, 1225, 1225};
    array<uintSeqLen, 18> positions = {50,    33219, 33220, 66388, 50,    33265, 50,    28137, 28138,
                                       59225, 59226, 87314, 50,    29605, 29606, 59162, 59163, 88717};
    array<uintRefSeqBin, 18> ref_seq_bins = {0,    0,    1,    1,    2,    2,    1206, 1206, 1207,
                                             1207, 1208, 1208, 1240, 1240, 1241, 1241, 1242, 1242};
    array<uintSeqLen, 18> corrected_positions = {0,     33169, 0,     33168, 0,     33215, 0,     28087, 0,
                                                 28087, 0,     28088, 0,     29555, 0,     29555, 0,     29554};
    ...
    for (auto i = 0; i < ref_seq_ids.size(); ++i) {
        ...
        EXPECT_EQ(corrected_positions.at(i), result.second)
            << "RefSeq: " << ref_seq_ids.at(i) << " Pos: " << positions.at(i);
    }
    ...
    // seqtk seq drosophila-GCF_000001215.4_cut.fna | awk '(NR%2==0){print
    // (length($0)-100)/(1+int((length($0)-100)/40000))}' | sort -nr | head -n5
...
void FragmentDistributionStatsTest::TestRefBinProcessing() {
    ...
    std::array<uintSeqLen, 18> frag_length = {250, 500, 750,  1000, 250, 500, 750,  1000, 250,
                                              500, 750, 1000, 250,  500, 750, 1000, 250,  500};
    ...
    array<uintSeqLen, 18> positions = {
        50,    33219, 33220, 65389, 50,    32766, 50,    28137, 28138, 59225,
        59226, 86315, 50,    29605, 29606, 59162, 59163, 88218}; // Remove kMinDistToContigEnds and Fragment length from
                                                                 // last position in last bin of reference sequence
    array<uintSeqLen, 18> corrected_positions = {0,     33169, 0,     32169, 0,     32716, 0,     28087, 0,
                                                 28087, 0,     27089, 0,     29555, 0,     29555, 0,     29055};
    std::array<uint16_t, 18> orientation = {0, 0, 1, 0, 0, 1, 0, 0, 1, 1, 1, 0, 1, 1, 0, 1, 1, 0};

    for (auto i = ref_seq_ids.size(); i--;) {
        for (auto k = 2; k--;) {
            test_->AddFragmentSite(ref_seq_ids.at(i), frag_length.at(2 * i + k), positions.at(2 * i + k),
                                   orientation.at(2 * i + k), species_reference_, thread);
        }
    }

    for (auto i = ref_seq_ids.size(); i--;) {
        for (auto k = 2; k--;) {
            EXPECT_EQ(2 * corrected_positions.at(2 * i + k) + orientation.at(2 * i + k),
                      test_->fragment_sites_by_ref_seq_bin_.at(used_bins.at(i)).at(!k).first)
                << "Bin " << used_bins.at(i) << " k " << k;
            EXPECT_EQ(frag_length.at(2 * i + k),
                      test_->fragment_sites_by_ref_seq_bin_.at(used_bins.at(i)).at(!k).second)
                << "Bin " << used_bins.at(i) << " k " << k;
        }
    }
    ...
    for (auto i = 0; i < ref_seq_ids.size(); ++i) {
        for (auto k = 0; k < 2; ++k) {
            EXPECT_EQ(2 * corrected_positions.at(2 * i + k) + orientation.at(2 * i + k),
                      test_->fragment_sites_by_ref_seq_bin_.at(used_bins.at(i)).at(k).first)
                << "Bin " << used_bins.at(i) << " k " << k;
            EXPECT_EQ(frag_length.at(2 * i + k), test_->fragment_sites_by_ref_seq_bin_.at(used_bins.at(i)).at(k).second)
                << "Bin " << used_bins.at(i) << " k " << k;
        }
    }
    ...
    for (auto i = ref_seq_ids.size(); i--;) {
        ...
        for (auto k = 2; k--;) {
            species_reference_.GetFragmentSites(sites, ref_seq_ids.at(i), frag_length.at(2 * i + k), start.at(i),
                                                end.at(i));
            test_->AddFragmentsToSites(sites, test_->fragment_sites_by_ref_seq_bin_by_insert_length_.at(used_bins.at(i))
                                                  .at(frag_length.at(2 * i + k)));

            if (0 == k) {
                EXPECT_EQ(!orientation.at(2 * i + k), sites.at(0).count_forward_)
                    << "Bin " << used_bins.at(i) << " k " << k;
                EXPECT_EQ(orientation.at(2 * i + k), sites.at(0).count_reverse_)
                    << "Bin " << used_bins.at(i) << " k " << k;
            } else {
                EXPECT_EQ(!orientation.at(2 * i + k), sites.at(corrected_positions.at(2 * i + k)).count_forward_)
                    << "Bin " << used_bins.at(i) << " k " << k;
                EXPECT_EQ(orientation.at(2 * i + k), sites.at(corrected_positions.at(2 * i + k)).count_reverse_)
                    << "Bin " << used_bins.at(i) << " k " << k;
            }
        }
    }
...
TEST_F(FragmentDistributionStatsTest, BiasCalculation) {
    // samtools faidx ecoli-GCF_000005845.2_ASM584v2_genomic.fa NC_000913.3:100-199 | awk '(NR==1){print $0
    // "_1000times"}(NR==2){for(i=0;i<1000;i++){print $0}}' | seqtk seq > reference-bias-calculation.fa
    string test_dir;
    ASSERT_TRUE(GetTestDir(test_dir));
...
TEST_F(FragmentDistributionStatsTest, UpdateRefSeqBias) {
    ...
    test_->ref_seq_bias_.resize(1);
    EXPECT_TRUE(
        test_->UpdateRefSeqBias(RefSeqBiasSimulation::kFile, test_dir + "ref-bias-test.txt", species_reference_, rgen));
