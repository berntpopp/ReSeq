        ...
        if ("queryProfile" == unrecognized_opts.at(0)) {
            ...
            opt_desc.add_options() // Returns a special object with defined operator ()
                ("adapterFile", value<string>(), "Fasta file with adapter sequences [(AutoDetect)]")(
                    ...
                    "[(AutoDetect)]")("bamIn,b", value<string>(),
                                      "Position sorted bam/sam file with reads mapped to refIn")(
                    ...
                    "statsOut,S", value<string>(),
                    "Stores the real data statistics for reuse in given file [<bamIn>.reseq]")(
            ...
            opt_desc_sim.add_options()("firstReadsOut,1", value<string>(),
                ...
                "Divides the original probability of correct base calls(no substitution error) by this value and "
                "renormalizes")(
                ...
                "Base Identifier for the simulated fastq records, followed by a count and other information about the "
                "read")("refBias", value<string>(),
                        "Way to select the reference biases for simulation (keep [from refIn]/no [biases]/draw [with "
                        "replacement from original biases]/file) [keep/no]")(
                "refBiasFile", value<string>(),
                "File to read reference biases from: One sequence per file (identifier bias)")(
            ...
            if (general_opts_map.count("help")) {
                ...
                if (opts_map.end() == it_ref_in && opts_map.end() == it_ref_out &&
                    ...
                    if (opts_map.end() != it_ref_in && !species_reference.ReadFasta(ref_input.c_str())) {
                        ...
                        GetDataStats(real_data_stats, stats_file, loaded_stats, stats_only, max_ref_seq_bin_size,
                                     opts_map, opt_desc_full, usage_str, num_threads);
                        ...
                        if (!stats_only) { // Data stats have been loaded or computed
                            ...
                            PrepareProbabilityEstimation(probs_in, probs_out, stats_file + ".ipf", loaded_stats,
                                                         opts_map);
                            ...
                            if (!stop_after_estimation || opts_map.count("writeSysError")) {
                                if ((opts_map.end() != it_ref_out) &&
                                    ...
                                    uintSeed seed;
                                    if (!WriteSysError(sys_error_file, seed, stop_after_estimation, opts_map,
                                                       opt_desc_full, usage_str, species_reference, real_data_stats,
                                                       probabilities)) {
                                        return 1;
                                    } else {
                                        if (!stop_after_estimation) {
                                            ...
                                            if (opts_map.end() == it_ref_bias) {
                                                if (opts_map.end() == it_ref_bias_file) {
                                                    if (opts_map.end() == it_ref_out) {
                                                        printInfo << "Keeping reference sequence biases from read "
                                                                     "in[default]."
                                                                  << std::endl;
                                                    ...
                                                    printInfo << "Reading reference sequence biases from file."
                                                              << std::endl;
                                                ...
                                                if ("keep" == ref_bmodel) {
                                                    printInfo << "Keeping reference sequence biases from read in."
                                                              << std::endl;
                                                    ref_bias_model = RefSeqBiasSimulation::kKeep;
                                                    ...
                                                    printInfo << "Drawing with replacement from reference sequence "
                                                                 "biases from read in."
                                                              << std::endl;
                                                    ...
                                                    printErr << "Unknown option for refBias: " << ref_bmodel
                                                             << std::endl;
                                                    if (0 < kVerbosityLevel) {
                                                        cerr << usage_str;
                                                        cerr << opt_desc_full << std::endl;
                                            ...
                                            if (RefSeqBiasSimulation::kFile == ref_bias_model) {
                                                if (opts_map.end() == it_ref_bias_file) {
                                                    printErr << "refBiasFile option mandatory if for refBias option "
                                                                "file was chosen"
                                                             << std::endl;
                                                    if (0 < kVerbosityLevel) {
                                                        cerr << usage_str;
                                                ...
                                                if (opts_map.end() != it_ref_bias_file) {
                                                    printErr << "refBiasFile option only allowed if for refBias option "
                                                                "file was chosen"
                                                             << std::endl;
                                                    if (0 < kVerbosityLevel) {
                                                        cerr << usage_str;
                                            ...
                                            if (RefSeqBiasSimulation::kError == ref_bias_model) {
                                                ...
                                                if (opts_map.end() == it_var_file && opts_map.count("vcfIn") &&
                                                    ...
                                                    printErr << "vcfIn specified but not used as stats were loaded. "
                                                                "Did you mean vcfSim?"
                                                             << std::endl;
                                                    if (0 < kVerbosityLevel) {
                                                        cerr << usage_str;
                                                    ...
                                                    if (opts_map.end() == it_var_file) {
                                                        printInfo << "Simulating reference allele/population."
                                                                  << std::endl;
                                                    } else {
                                                    ...
                                                    Simulator sim;
                                                    if (!sim.Simulate(sim_output_first.c_str(),
                                                                      sim_output_second.c_str(), species_reference,
                                                                      real_data_stats, probabilities, num_threads, seed,
                                                                      num_read_pairs, coverage, ref_bias_model,
                                                                      ref_bias_file, sys_error_file,
            ...
            opt_desc.add_options() // Returns a special object with defined operator ()
                ("errorMutliplier", value<double>(&error_multiplier)->default_value(1.0),
                 "Divides the original probability of correct base calls(no substitution error) by this value and "
                 "renormalizes")("input,i", value<string>(), "Input file (fasta format, gz and bz2 supported) [stdin]")(
