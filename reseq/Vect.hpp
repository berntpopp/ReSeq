...
class Vect { // std::vector that removes 0s at the beginning and has an offset variable instead and automatically
  ...
  public:
    // Constructors
    Vect() {}
    Vect(typename std::vector<T>::size_type reserved) { this->reserve(reserved); }
    Vect(const typename std::vector<T>::size_type offset, const typename std::vector<T>& values)
        : vec_{offset, values} {}
    ...
    void SetOffset(typename std::vector<T>::size_type
                       offset) { // Sets the offset to the given value if that doesn't conflict with values already in
                                 // the array. In case of a conflict it sets the offset as high as possible
        if (vec_.second.empty()) {
            ...
            if (offset > vec_.first) {
                ...
                while (leading_zeros < vec_.second.size() && 0 == vec_.second[leading_zeros] &&
                       leading_zeros <
                           offset - vec_.first) { // Check if all entries from current offset to new offset are zero and
                                                  // if not how many leading zeros can be removed
                    ++leading_zeros;
                }
    ...
    void Shrink() { // Shrink vector to minimum size: Remove leading and trailing zeros + clear all reserved space that
                    // is not used
        SoftShrink();
    ...
    void PopBack(typename std::vector<T>::size_type num_elements) {
        vec_.second.resize(vec_.second.size() - num_elements);
    }
    void Set(typename std::vector<T>::size_type from, typename std::vector<T>::size_type to, const T& value) {
        ...
        for (auto i = to;
             from < i--;) { //-- included in second term so it also works for 0==from given unsigned integers
            (*this)[i] = value;
        }
    }
...
template <typename T>
bool operator==(typename std::vector<T>::size_type val, const Vect<T>& vect) { // Compares the size_t value to the size
...
void GetLimitsSecondDimension(const T<U>& vect2d, typename std::vector<U>::size_type& from,
                              typename std::vector<U>::size_type& to) {
...
void GetIndicesFirstDimension(const T<U>& vect2d, std::vector<V>& indices) {
    indices.clear();
...
template <template <typename> class T, typename U, typename V>
void GetIndicesSecondDimension(const T<U>& vect2d, std::vector<V>& indices) {
...
void SetVect2d(T<U<V>>& vect, typename std::vector<U<V>>::size_type outer_from,
               typename std::vector<U<V>>::size_type outer_to, typename std::vector<V>::size_type inner_from,
               typename std::vector<V>::size_type inner_to, V value) {
...
inline uint64_t SumVect(const T<uint64_t, std::allocator<uint64_t>>& vect) { // For std containers
    return SumVectHelper(vect);
}

template <template <class, class> class T>
inline uint64_t SumVect(const T<utilities::VectorAtomic<uint64_t>, std::allocator<utilities::VectorAtomic<uint64_t>>>&
                            vect) { // For std containers
    return SumVectHelperAtomic(vect);
}

template <template <class, class> class T>
inline double SumVectD(const T<double, std::allocator<double>>& vect) { // For std containers
    return SumVectHelperDouble(vect);
}
