...
class SurroundingCountAtomic {
  private:
    std::array<std::vector<utilities::VectorAtomic<uintFragCount>>, Surrounding::kNumBlocks>
        counts_; // count_[BlockNumber][(262144*nucAt0+...+(256*nucAt5)+(64*nucAt6)+(16*nucAt7)+(4*nucAt8)+(nucAt9)] =
                 // #fragments (always facing inwards, Block0 is outside fragment, Block1 first in fragment ..., value
                 // 262144 is first base in block/fragment(block1), 256 is 5th base in block, ...)
...
class SurroundingCount {
  private:
    ...
    template <class Archive> void serialize(Archive& ar, const unsigned int UNUSED(version)) { ar & counts_; }
...
class SurroundingBias {
  private:
    ...
    // Boost archive functions
    friend class boost::serialization::access;
    template <class Archive> void serialize(Archive& ar, const unsigned int UNUSED(version)) { ar & bias_; }
        ...
        if (start_pos + kmer.Length() <= length(seq)) {
            ...
            for (uintSeqLen pos = start_pos + 1; pos + kmer.Length() <= length(seq); ++pos) {
                ...
                if (kmer.sur_.at(0) == start_kmer) {
                    // If we hit the start kmer again, we are probably in a segment with short repeats, so we remove
                    // counts again and stop
