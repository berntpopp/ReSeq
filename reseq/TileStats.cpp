...
inline void TileStats::ReadUntil(stringstream& id_stream, char* buffer, uint16_t buffer_size, char until,
...
inline void TileStats::CheckDelimiter(char delimiter, char expected_delimiter, const string& occurence) const {
    if (expected_delimiter != delimiter) {
        throw runtime_error(string("Wrong delimiter: '") + delimiter + "' found instead of '" + expected_delimiter +
                            "' at " + occurence + " occurrence.");
    }
}
...
reseq::uintTile TileStats::GetKnownTile(const CharString& read_id, uint16_t tile_colon_number, bool print_warnings,
                                        stringstream* error_message) const {
    uint16_t pos(0);
    ...
    if (++pos >= length(read_id)) {
        if (print_warnings) {
            (error_message ? *error_message : printWarn)
                << "The read id encoding changed and the id ended before colon number " << tile_colon_number
                << ". Tile will be treated as 0: " << read_id << std::endl;
        }
        return 0;
    }
    ...
    if (pos >= length(read_id)) {
        if (print_warnings) {
            (error_message ? *error_message : printWarn)
                << "The read id encoding changed and the id ended before colon number " << tile_colon_number + 1
                << ". Tile will be treated as 0: " << read_id << std::endl;
        }
        return 0;
    }
    ...
    try {
        ...
        if (print_warnings) {
            (error_message ? *error_message : printWarn)
                << "The read id encoding changed and the tile could not be found at colon number " << tile_colon_number
                << " anymore. Tile will be treated as 0: " << read_id << "\nException thrown: " << e.what()
                << std::endl;
        }
        return 0;
    }
...
reseq::uintTile TileStats::GetTile(const CharString& read_id, uint16_t& tile_colon_number, bool& tile_accessible,
                                   stringstream* error_message) const {
    uint32_t tile(0); // Make it a bigger int, because it also has to accept a position info to check for integer
                      // convertibility of that string
    ...
    if (tile_colon_number) { // The colon number where to find the tile is know from previous reads
        ...
        // Old Illumina format:						@HWUSI-EAS100R:6:73:941:1973#0/1							@(unique
        // instrument name):(flowcell lane):(tile number):(x-coordinate):(y-coordinate)#(index number)/(member of pair)
        // New Illumina format since Casava 1.8:	@EAS139:136:FC706VJ:2:2104:15343:197393 1:Y:18:ATCACG		@(unique
        // instrument name):(run id):(flowcell id):(flowcell lane):(tile number):(x-coordinate):(y-coordinate) (member
        // of pair):(filtered?):(control bits):(index) NCBI Sequence Read Archive:				@SRR001666.1
        // 071112_SLXA-EAS1_s_7:5:1:817:345 length=36		@(NCBI identifier) (original format identifier as above)
        // length=(read length)
        ...
        try {
            ...
            if ('#' == delimiter || ' ' == delimiter || '_' == delimiter ||
                char_traits<char>::eof() == delimiter) { // Old Illumina format
                ...
                if (tile &&
                    strlen(buffer) ==
                        snprintf(
                            buffer, kReadIdBufferSize, "%u",
                            tile)) { // Make sure atoi found digits to convert and all characters it found where digits
                    tile_colon_number = 2;
                } else {
                ...
                if (' ' != delimiter && '_' != delimiter && char_traits<char>::eof() != delimiter) {
                    throw runtime_error(string("Delimiter '") + delimiter +
                                        "' is non of the possible delimiters at seventh occurence (' ','_',eof)");
                }

                // Not checking for the part after the space delimiter, because it is frequently changed by data
                // processing software like error correction Tile is already set correctly
                tile_colon_number = 4;
            }
        } catch (const exception& e) {
            if (error_message) {
...
bool TileStats::EnterTile(const CharString& read_id, stringstream* error_message) {
    ...
    // If we have a tile everything is fine, if tiles have been set to being ignored everything is fine, if tiles are
    // found to be inaccessible not everything is fine, but we still want to continue Only if we already found
    // accessible tiles and then find inaccessible tiles we want to quit
