...
inline double ProbabilityEstimatesTest::CalculateAbsolutPrecision(uintMatrixCount correct_margin, double marginal_sum,
                                                                  double precision) {
    ...
    // Precision is not evaluated for correct_margings equal to 0, because it would be infinity, as we still check for
    // this cases everything considered a perfect fit (<epsilon) is accepted here
    return max(kEpsilon, marginal_sum * sum_factor);
}

void ProbabilityEstimatesTest::SetUpDataQual(DataStats& stats, uintBaseCall base,
    ...
    stats.qualities_.base_quality_for_preceding_quality_per_tile_reference_.at(kTemplateSegment).at(base)[0] =
        margins.at(1);
    stats.qualities_.preceding_quality_for_error_rate_per_tile_reference_.at(kTemplateSegment).at(base)[0] =
        margins.at(2);
    ...
    stats.qualities_.preceding_quality_for_position_per_tile_reference_.at(kTemplateSegment).at(base)[0] =
        margins.at(5);
    stats.qualities_.base_quality_for_sequence_quality_per_tile_reference_.at(kTemplateSegment).at(base)[0] =
        margins.at(6);
    stats.qualities_.sequence_quality_for_error_rate_per_tile_reference_.at(kTemplateSegment).at(base)[0] =
        margins.at(7);
    stats.qualities_.preceding_quality_for_sequence_quality_per_tile_reference_.at(kTemplateSegment).at(base)[0] =
        margins.at(8);
    stats.qualities_.sequence_quality_for_position_per_tile_reference_.at(kTemplateSegment).at(base)[0] = margins.at(9);
}

void ProbabilityEstimatesTest::SetUpDataBaseCall(
    DataStats& stats, const vector<Vect<Vect<uintMatrixCount>>>& margins,
    const Vect<SeqQualityStats<uintMatrixCount>>& margin_quality_position) {
    ...
    stats.qualities_.base_quality_stats_per_tile_per_error_reference_.at(kTemplateSegment).at(0).at(0)[0] =
        margin_quality_position;
    ...
    stats.qualities_.base_quality_for_error_rate_per_tile_per_error_reference_.at(kTemplateSegment).at(0).at(0)[0] =
        margins.at(7);
    stats.qualities_.error_rate_for_position_per_tile_per_error_reference_.at(kTemplateSegment).at(0).at(0)[0] =
        margins.at(8);
...
void ProbabilityEstimatesTest::SetUpDataErrorRate(DataStats& stats,
                                                  const vector<Vect<Vect<uintMatrixCount>>>& margins) {
...
void ProbabilityEstimatesTest::TestLogArrayCalcNormalize() {
    ...
    for (uintMarginId n = 3; n--;) {
        ...
        for (uintMatrixIndex p1 = 3; p1--;) {
            for (uintMatrixIndex p2 = 3; p2--;) {
                ...
                // Introduce factors that cancel each other out that are close to the numeric limit to see whether that
                // is a problem for the normalization
                if (2 == n) {
    ...
    // Now check if values got as small as they are supposed to be (Nothing can be higher than the max of the real
    // distribution 1.5)
    for (uintMarginId n = 3; n--;) {
        for (uintMatrixIndex p1 = 3; p1--;) {
            for (uintMatrixIndex p2 = 3; p2--;) {
                EXPECT_TRUE(1.6 > test_normalized.dim2_.at(n).at(
                                      p1 * 3 +
                                      p2)); // Chose 1.6, that is good enough and very generous towards precision issues
            }
        }
    }
    ...
    for (uintMatrixIndex p1 = 3; p1--;) {
        for (uintMatrixIndex p2 = 3; p2--;) {
            for (uintMatrixIndex p3 = 3; p3--;) {
                ...
                EXPECT_FLOAT_EQ(test.dim2_.at(2).at(p3 * 3 + p2) * test.dim2_.at(1).at(p3 * 3 + p1) *
                                    test.dim2_.at(0).at(p2 * 3 + p1),
                                test_normalized.dim2_.at(2).at(p3 * 3 + p2) *
                                    test_normalized.dim2_.at(1).at(p3 * 3 + p1) *
                                    test_normalized.dim2_.at(0).at(p2 * 3 + p1));
            }
        }
    }
}
...
void ProbabilityEstimatesTest::TestDataStorageReduceAndExpand() {
    ...
    for (uintMarginId n = 3; n--;) {
        ...
        for (auto i = dim_indices_reduced.at(n).size(); i--;) {
            ...
            EXPECT_TRUE(dim_indices_reduced.at(n).at(i) < 3)
                << "dim_indices_reduced[" << n << "][" << i << "] invalid index";
        }
        for (auto i = dim_indices_reduced.at(n).size(); --i;) {
            // First index has to be different then all other indeces
            EXPECT_TRUE(dim_indices_reduced.at(n).at(i) != dim_indices_reduced.at(n).at(0))
                << "dim_indices_reduced[" << n << "][" << i << "] is equal to first index";
        }
        for (auto i = dim_indices_reduced.at(n).size() - 1; i--;) {
            if (i < 4) {
                // Last index has to be different then all other indeces
                EXPECT_TRUE(dim_indices_reduced.at(n).at(i) !=
                            dim_indices_reduced.at(n).at(dim_indices_reduced.at(n).size() - 1))
                    << "dim_indices_reduced[" << n << "][" << i << "] is equal to last index";
            } else {
                // Special case for dimension 3, where the last index exists three times
                EXPECT_TRUE(dim_indices_reduced.at(n).at(i) ==
                            dim_indices_reduced.at(n).at(dim_indices_reduced.at(n).size() - 1))
                    << "dim_indices_reduced[" << n << "][" << i << "] is not equal to last index";
            }
        }
        for (auto i = dim_indices_reduced.at(n).size() - (2 == n ? 4 : 2); --i;) {
            // Middle indeces have to be equal
            EXPECT_TRUE(dim_indices_reduced.at(n).at(i) ==
                        dim_indices_reduced.at(n).at(dim_indices_reduced.at(n).size() - (2 == n ? 4 : 2)))
                << "dim_indices_reduced[" << n << "][" << i << "] is not equal to middle indeces";
        }

        EXPECT_EQ(3, dim_indices_count.at(n).size()) << "dim_indices_count[" << n << "].size() wrong";
        EXPECT_EQ(1, dim_indices_count.at(n).at(dim_indices_reduced.at(n).at(0)))
            << "dim_indices_count[" << n << "][" << dim_indices_reduced.at(n).at(0) << "] wrong";
        EXPECT_EQ((0 == n ? 1 : 3), dim_indices_count.at(n).at(dim_indices_reduced.at(n).at(1)))
            << "dim_indices_count[" << n << "][" << dim_indices_reduced.at(n).at(1) << "] wrong";
        EXPECT_EQ((2 == n ? 3 : 1),
                  dim_indices_count.at(n).at(dim_indices_reduced.at(n).at(dim_indices_reduced.at(n).size() - 1)))
            << "dim_indices_count[" << n << "][" << dim_indices_reduced.at(n).at(dim_indices_reduced.at(n).size() - 1)
            << "] wrong";
    }
    ...
    for (uintMarginId n = 6; n--;) {
        ...
        for (auto i = dim_indices_reduced.at(dim_a).size(); i--;) {
            for (auto j = dim_indices_reduced.at(dim_b).size(); j--;) {
                EXPECT_DOUBLE_EQ(reduced_data.data_.at(n).at(dim_indices_reduced.at(dim_a).at(i) *
                                                                 dim_indices_count.at(dim_b).size() +
                                                             dim_indices_reduced.at(dim_b).at(j)),
                                 test.data_.at(n).at(i * dim_indices_reduced.at(dim_b).size() + j) *
                                     dim_indices_count.at(dim_a).at(dim_indices_reduced.at(dim_a).at(i)) *
                                     dim_indices_count.at(dim_b).at(dim_indices_reduced.at(dim_b).at(j)));
            }
        }
    }
    ...
    EXPECT_TRUE(
        test.Expand(reduced_data, dim_indices_reduced, dim_indices_count, expansion_indices, expansion_count, 1))
        << "Expansion returns wrong value";
    for (uintMarginId n = 3; n--;) {
        ...
        if (0 == n) {
            ...
            for (auto ind = dim_indices_reduced.at(n).size(); ind--;) {
                EXPECT_EQ(ind, dim_indices_reduced.at(n).at(ind))
                    << "dim_indices_reduced[" << n << "] did not go back to original";
            }
        } else if (2 == n) {
            bool count_two_left(true);
            for (auto count : dim_indices_count.at(n)) {
                EXPECT_TRUE(1 == count || (count_two_left && 2 == count))
                    << "dim_indices_count[" << n << "] has wrong counts after first expansion";
                if (2 == count) {
            ...
            for (auto ind : dim_indices_reduced.at(n)) {
                EXPECT_TRUE(tmp_counts.at(ind)--)
                    << "dim_indices_count[" << n << "] does not fit with dim_indices_reduced after first expansion";
            }
        }

        EXPECT_EQ(1, expansion_count.at(n).at(expansion_indices.at(n).at(dim_indices_reduced.at(n).at(0))))
            << "expansion_count[" << n << "] wrong";
        EXPECT_EQ((0 == n ? 1 : 3),
                  expansion_count.at(n).at(expansion_indices.at(n).at(dim_indices_reduced.at(n).at(1))))
            << "expansion_count[" << n << "] wrong";
        EXPECT_EQ((2 == n ? 3 : 1), expansion_count.at(n).at(expansion_indices.at(n).at(
                                        dim_indices_reduced.at(n).at(dim_indices_reduced.at(n).size() - 1))))
            << "expansion_count[" << n << "] wrong";
    }
    ...
    EXPECT_TRUE(
        ...
        << "Expansion returns wrong value";
    for (uintMarginId n = 3; n--;) {
        EXPECT_EQ((2 == n ? 6 : 3 + 2 * n), dim_indices_count.at(n).size())
            << "dim_indices_count[" << n << "].size() wrong";
        if (2 != n) {
            for (auto count : dim_indices_count.at(n)) {
...
void ProbabilityEstimatesTest::TestDataStorageReduceAndExpand2() {
    ...
    for (uint16_t test_case = 0; test_case < 2; ++test_case) {
        ...
        if (0 == test_case) {
            ...
            EXPECT_TRUE(test.Expand(reduced_data, dim_indices_reduced, dim_indices_count, expansion_indices,
                                    expansion_count, 1))
                << "Expansion returns wrong value";
            ...
            for (uintMarginId n = 3; n--;) {
                ...
                EXPECT_EQ((2 == n ? 2 : 1), expansion_indices.at(n).size())
                    << "expansion_indices[" << n << "].size() wrong";
        ...
        if (2 == dim_indices_count.at(2).at(0)) {
            EXPECT_EQ(3, dim_indices_count.at(2).at(1))
                << "dim_indices_count[2] wrong: " << dim_indices_count.at(2).at(0) << ' '
                << dim_indices_count.at(2).at(1) << " in case " << test_case;
            ...
            EXPECT_EQ(3, dim_indices_count.at(2).at(0))
                << "dim_indices_count[2] wrong: " << dim_indices_count.at(2).at(0) << ' '
                << dim_indices_count.at(2).at(1) << " in case " << test_case;
            EXPECT_EQ(2, dim_indices_count.at(2).at(1))
                << "dim_indices_count[2] wrong: " << dim_indices_count.at(2).at(0) << ' '
                << dim_indices_count.at(2).at(1) << " in case " << test_case;
            for (uintMatrixIndex i = 0; i < 2; ++i) {
                EXPECT_EQ(1, dim_indices_reduced.at(2).at(i)) << "dim_indices_reduced[2] wrong in case " << test_case;
    ...
    for (uint16_t test_case = 0; test_case < 2; ++test_case) {
        ...
        if (0 == test_case) {
            ...
            for (uintMarginId n = 3; n--;) {
                EXPECT_EQ(1, expansion_count.at(n).size()) << "expansion_count[" << n << "].size() wrong";
                EXPECT_EQ((2 == n ? 5 : 1), expansion_count.at(n).at(0)) << "expansion_count[" << n << "][0] wrong";
                EXPECT_EQ((2 == n ? 4 : 1), expansion_indices.at(n).size())
                    << "expansion_indices[" << n << "].size() wrong";
                for (uintMatrixIndex i = 0; i < expansion_indices.at(n).size(); ++i) {
                    EXPECT_EQ(0, expansion_indices.at(n).at(i)) << "expansion_count[" << n << "][" << i << "] wrong";
                }
        ...
        EXPECT_EQ(dim_indices_reduced.at(2).at(0), dim_indices_reduced.at(2).at(1))
            << "Max difference for the separation doesn't seem to work in case " << test_case;
        EXPECT_EQ(2, dim_indices_count.at(2).at(dim_indices_reduced.at(2).at(1)))
            << "Max difference for the separation doesn't seem to work in case " << test_case;

        bool count_two_left(true);
        for (auto count : dim_indices_count.at(2)) {
            EXPECT_TRUE(1 == count || (count_two_left && 2 == count))
                << "dim_indices_count[2] has wrong counts in case " << test_case;
            if (2 == count) {
                count_two_left = false;
            }
        }
        ...
        for (auto ind : dim_indices_reduced.at(2)) {
            EXPECT_TRUE(tmp_counts.at(ind)--)
                << "dim_indices_count[2] does not fit with dim_indices_reduced in case " << test_case;
        }
    }
}
...
void ProbabilityEstimatesTest::TestLogArrayCalcExpand() {
    ...
    for (uintMatrixIndex p1 = test.dim_size_.at(0); p1--;) {
        for (uintMatrixIndex p2 = test.dim_size_.at(1); p2--;) {
            for (uintMatrixIndex p3 = test.dim_size_.at(2); p3--;) {
                // Double precision cannot be expected after the logs used
                value = test.dim2_.at(2).at(p3 * 3 + p2) * test.dim2_.at(1).at(p3 * 3 + p1) *
                        test.dim2_.at(0).at(p2 * 3 + p1);
    ...
    for (uintMatrixIndex p1 = test.dim_size_.at(0); p1--;) {
        for (uintMatrixIndex p2 = test.dim_size_.at(1); p2--;) {
            for (uintMatrixIndex p3 = test.dim_size_.at(2); p3--;) {
                // Double precision cannot be expected after the logs used
                value = test.dim2_.at(2).at(p3 * 4 + p2) * test.dim2_.at(1).at(p3 * 3 + p1) *
                        test.dim2_.at(0).at(p2 * 3 + p1);
    ...
    for (uintMarginId n = 3; n--;) {
        ...
        for (auto i = dim_indices_reduced.at(dim_a).size(); i--;) {
            for (auto j = dim_indices_reduced.at(dim_b).size(); j--;) {
                EXPECT_DOUBLE_EQ(
                    marginal_sums.at(n).at(dim_indices_reduced.at(dim_a).at(i) * dim_indices_count.at(dim_b).size() +
                                           dim_indices_reduced.at(dim_b).at(j)),
                    expanded_marginal_sums.at(n).at(i * dim_indices_reduced.at(dim_b).size() + j) *
                        dim_indices_count.at(dim_a).at(dim_indices_reduced.at(dim_a).at(i)) *
                        dim_indices_count.at(dim_b).at(dim_indices_reduced.at(dim_b).at(j)))
                    << '[' << n << "][" << i << "][" << j << ']';
            }
...
void ProbabilityEstimatesTest::RandomFillingTestCounts(Vect<Vect<Vect<Vect<Vect<uintMatrixCount>>>>>& counts,
                                                       const uintMatrixIndex dim1, const uintMatrixIndex dim2,
                                                       const uintMatrixIndex dim3, const uintMatrixIndex dim4,
                                                       const uintMatrixIndex dim5, const uintSeed seed) {
    mt19937_64 rgen;
    rgen.seed(seed);
            ...
            for (auto p3 = dim3; p3--;) {
                for (auto p4 = dim4; p4--;) {
                    for (auto p5 = dim5; p5--;) {
                        ...
                        counts[p1][p2][p3][p4][p5] =
                            dist(rgen) *
                            100; // Multiply everything by 100 so we are above minimum counts and bins are not combined
                    }
                }
            }
        }
...
void ProbabilityEstimatesTest::SetOffset(Vect<Vect<Vect<Vect<Vect<uintMatrixCount>>>>>& counts,
                                         const uintMatrixIndex dim1, const uintMatrixIndex dim2,
                                         const uintMatrixIndex dim3, const uintMatrixIndex dim4,
...
void ProbabilityEstimatesTest::CheckIPFResult(uintNumFits iterations, double precision,
                                              const vector<Vect<Vect<uintMatrixCount>>>& margins,
                                              const Vect<SeqQualityStats<uintMatrixCount>>& margin_quality_position,
    ...
    for (uintMarginId i = 0; i < margins.size(); ++i) {
        if (margin_def.at(i).first) {
            for (auto pos1 = margin_quality_position.from(); pos1 < margin_quality_position.to(); ++pos1) {
                for (auto pos2 = margin_quality_position.at(pos1).from(); pos2 < margin_quality_position.at(pos1).to();
                     ++pos2) {
                    EXPECT_NEAR(
                        margin_quality_position.at(pos1).at(pos2), estimated_margins_quality_position.at(pos1).at(pos2),
                        CalculateAbsolutPrecision(margin_quality_position.at(pos1).at(pos2),
                                                  estimated_margins_quality_position.at(pos1).at(pos2), precision))
                        << "(quality_position) margins[" << i << "][" << pos1 << "][" << pos2
                        << "] does not match within precision for " << context << '\n';
                }
            }
        } else {
            for (auto pos1 = margins.at(i).from(); pos1 < margins.at(i).to(); ++pos1) {
                for (auto pos2 = margins.at(i).at(pos1).from(); pos2 < margins.at(i).at(pos1).to(); ++pos2) {
                    EXPECT_NEAR(margins.at(i).at(pos1).at(pos2), estimated_margins.at(i).at(pos1).at(pos2),
                                CalculateAbsolutPrecision(margins.at(i).at(pos1).at(pos2),
                                                          estimated_margins.at(i).at(pos1).at(pos2), precision))
                        << "margins[" << i << "][" << pos1 << "][" << pos2 << "] does not match within precision for "
                        << context << '\n';
                }
            }
        }
...
void ProbabilityEstimatesTest::SetUpDataStorageErrorRate(ProbabilityEstimatesSubClasses::DataStorage<4>& data,
                                                         array<vector<uintMatrixIndex>, 4>& dim_indices,
                                                         array<vector<uintMatrixIndex>, 4>& initial_dim_indices,
                                                         const Vect<Vect<Vect<Vect<Vect<uintMatrixCount>>>>>& counts) {
...
void ProbabilityEstimatesTest::IterativeProportionalFittingQual(
...
void ProbabilityEstimatesTest::GetIPFResultQual(const ProbabilityEstimates& estimate, uintBaseCall base,
    ...
    auto tot_counts = SumVect(margins.at(
        0)); // As the data conversion before the IPF normalizes the sum of the matrix to 1 we have to revert it here

    const std::array<std::vector<uintMatrixIndex>, 5>& dim_indices(
        estimate.quality_.at(kTemplateSegment).at(0).at(base).dim_indices_);
            ...
            for (uintMatrixIndex pq = 0; pq < dim_indices.at(2).size(); ++pq) {
                for (uintMatrixIndex pos = 0; pos < dim_indices.at(3).size(); ++pos) {
                    for (uintMatrixIndex sq = 0; sq < dim_indices.at(1).size(); ++sq) {
                        estimated_counts[dim_indices.at(0).at(q)][dim_indices.at(4).at(er)][dim_indices.at(2).at(pq)]
                                        [dim_indices.at(3).at(pos)][dim_indices.at(1).at(sq)] =
                                            estimate.quality_.at(kTemplateSegment)
                                                .at(0)
                                                .at(base)
                                                .estimates_.GetMatrixElement({q, sq, pq, pos, er}) *
                                            tot_counts;
                    }
                }
            }
...
void ProbabilityEstimatesTest::IPFStepWiseQual(uintBaseCall base, const vector<Vect<Vect<uintMatrixCount>>>& margins,
    ...
    IPFStepQual<2, 3, 0, 1, 4>(
        base, data); // (pos, prev_qual) margin: Flipped indeces for U1, U2 (Test here that it doesn't matter)
    IPFStepQual<4, 2, 0, 1, 3>(base, data); // (error_rate, prev_qual) margin
    IPFStepQual<4, 3, 0, 1, 2>(base, data); // (pos, error_rate) margin
}

void ProbabilityEstimatesTest::IterativeProportionalFittingBaseCall(
    const vector<Vect<Vect<uintMatrixCount>>>& margins,
    const Vect<SeqQualityStats<uintMatrixCount>>& margin_quality_position) {
    // Run iterative proportional fitting
    DataStats stats(nullptr);
...
void ProbabilityEstimatesTest::GetIPFResultBaseCall(const ProbabilityEstimates& estimate,
    ...
    const std::array<std::vector<uintMatrixIndex>, 5>& dim_indices(
        estimate.base_call_.at(kTemplateSegment).at(0).at(0).at(0).dim_indices_);
        ...
        for (uintMatrixIndex q = 0; q < dim_indices.at(1).size(); ++q) {
            for (uintMatrixIndex pos = 0; pos < dim_indices.at(2).size(); ++pos) {
                for (uintMatrixIndex num = 0; num < dim_indices.at(3).size(); ++num) {
                    for (uintMatrixIndex er = 0; er < dim_indices.at(4).size(); ++er) {
                        estimated_counts[dim_indices.at(0).at(bc)][dim_indices.at(1).at(q)][dim_indices.at(2).at(pos)]
                                        [dim_indices.at(3).at(num)][dim_indices.at(4).at(er)] =
                                            estimate.base_call_.at(kTemplateSegment)
                                                .at(0)
                                                .at(0)
                                                .at(0)
                                                .estimates_.GetMatrixElement({bc, q, pos, num, er}) *
                                            tot_counts;
                    }
                }
            }
        ...
        for (uintMatrixIndex dom_err = 0; dom_err < dim_indices.at(0).size(); ++dom_err) {
            for (uintMatrixIndex gc = 0; gc < dim_indices.at(2).size(); ++gc) {
                for (uintMatrixIndex start_rate = 0; start_rate < dim_indices.at(3).size(); ++start_rate) {
                    estimated_counts[dim_indices.at(0).at(dom_err)][dim_indices.at(1).at(dist)]
                                    [dim_indices.at(2).at(gc)][dim_indices.at(3).at(start_rate)][0] =
                                        estimate.dom_error_.at(0).at(0).at(0).estimates_.GetMatrixElement(
                                            {dom_err, dist, gc, start_rate}) *
                                        tot_counts;
                }
            }
        }
...
void ProbabilityEstimatesTest::GetIPFResultErrorRate(const ProbabilityEstimates& estimate,
    ...
    for (uintMatrixIndex dist = 0; dist < dim_indices.at(1).size(); ++dist) {
        for (uintMatrixIndex er = 0; er < dim_indices.at(0).size(); ++er) {
            for (uintMatrixIndex gc = 0; gc < dim_indices.at(2).size(); ++gc) {
                for (uintMatrixIndex start_rate = 0; start_rate < dim_indices.at(3).size(); ++start_rate) {
                    estimated_counts[dim_indices.at(0).at(er)][dim_indices.at(1).at(dist)][dim_indices.at(2).at(
                        gc)][dim_indices.at(3).at(start_rate)][0] =
                        estimate.error_rate_.at(0).at(0).estimates_.GetMatrixElement({er, dist, gc, start_rate}) *
                        tot_counts;
                }
            }
        }
...
TEST_F(ProbabilityEstimatesTest, DataStorageSetUp) {
    ...
    for (auto i = 0; i < 3; ++i) {
        if (1 != i) {
            for (auto j = 1; j < 3; ++j) {
                for (auto k = 0; k < 2; ++k) {
                    counts[i][j][k][0][0] =
                        ++n * data.kMinCountsPer1dBin; // Multiply everything by kMinCountsPer1dBin so we are above
                                                       // minimum counts and bins are not combined
                }
            }
        }
    }
    ...
    // Move some stuff into a higher/lower bin, so that it will be shifted back because it does not reach minimum counts
    // and values should remain the same
    for (auto i = 0; i < 3; ++i) {
        if (1 != i) {
            for (auto k = 0; k < 2; ++k) {
    ...
    // Move some stuff in between bin 0 and 2 for index i, so that it will not be combined by the end combining, but by
    // the second combining step, this slightly changes dim_indices and initial_dim_indices
    for (auto i = 0; i < 3; ++i) {
        if (1 != i) {
            for (auto k = 0; k < 2; ++k) {
...
TEST_F(ProbabilityEstimatesTest, IPFforQuality) {
    ...
    double prob_sum;
    test_.Quality(0, 0, base)
        .Draw(
            prob, prob_sum, {200, 200, 200, 200},
            0.5); // Check for a crash if random values are queried (tile_id=0 must exist, otherwise crash is expected)
}
