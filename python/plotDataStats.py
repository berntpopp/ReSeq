...
def padList(data, min_x, max_x, normalize=False, normVector=False):
    if len(data[1]) == 0:
        ...
        if min_x <= data[0] and max_x >= data[0] + len(data[1]):
            ...
            # Cut + Pad
            return (
                [0] * max(0, data[0] - min_x)
                + center_list[max(0, min_x - data[0]) : min(max_x - data[0], len(center_list))]
                + [0] * max(0, max_x - data[0] - len(data[1]))
            )
    ...
    if marker_dist and (bool(marker_max) == (marker_max >= max_x)):
        for _name, hist, col, norm, mark in zip(names, hists, colour_scheme, normVectors, markers):
            ...
            ax.plot(
                ...
                [val for x, val in zip(x_vals, y_vals) if x % marker_dist == 0],
                marker=mark,
                linewidth=0,
                color=col,
                fillstyle="none",
...
def plotBackend(
    ...
    if max_x - min_x > 1:  # Otherwise the plot has only one data point and no lines can be plotted
        ...
        if baseline:
            ax.fill_between(
                x_vals,
                padList(baseline[1], min_x * multiply_x, max_x * multiply_x),
                label=baseline[0],
        ...
        plotMinimal(
            ...
            cov_plot=cov_plot,
            ignore_zero_for_y_scale=ignore_zero_for_y_scale,
            yrange=yrange,
            rmempty=rmempty,
...
def plotBaseQualities(
    pdf, xtitle, ytitle, names, means, minima, first_quartiles, medians, third_quartiles, maxima, plot_legend, title
):
    ...
    for n in range(len(means)):
        ...
        for x_i, ymin_i, ymax_i in zip(
            x_range, padList(first_quartiles[n], min_x, max_x), padList(third_quartiles[n], min_x, max_x)
        ):
            ax.add_patch(
                patches.Rectangle(
...
def plotSequenceQualities(
    pdf, xtitle, ytitle, names, means, minima, first_quartiles, medians, third_quartiles, maxima, plot_legend
):
    plt.close()
    ...
    fig, ((ax_mean, ax_first_quartile, ax_minimum), (ax_median, ax_third_quartile, ax_maximum)) = plt.subplots(
    ...
    finalizeMultiPlot(
        fig, [ax_mean, ax_first_quartile, ax_minimum, ax_median, ax_third_quartile, ax_maximum], xtitle, ytitle
    )
...
def plot2dQuality(pdf, xtitle, ytitle, names, quals, axis_lable_dist=5):
    ...
    if sys.maxsize != min_y:  # Otherwise the whole plot is empty
        ...
        for map, name in zip(qual_maps, names):
            ...
            # Plot 2d histogram
            ax.imshow(
                map, interpolation="nearest", aspect="auto", vmin=min_z, vmax=max_z, cmap=plt.cm.magma, norm=LogNorm()
            )  #
            ...
            if num_y_subplots - 1 == subplot_y:
                ax.set_xticklabels(
                    range(((min_x - 1) // axis_lable_dist + 1) * axis_lable_dist, max_x, axis_lable_dist)
                )
                pass
    ...
    for stats, tot in zip(called_bases, total_bases):
        ...
        for call_base in range(5):
            ...
            for ref_base in range(4):
                normed_stats[ref_base][call_base] = np.array(
                    padList(stats[ref_base][call_base], min_x, max_x), dtype=float
                )
    ...
    if stats:
        with PdfPages(oFile) as pdf:
            ...
            plot(
                ...
                "# bases of reference",
                names,
                [st.Coverage() for st in stats],
                plot_legend,
                title,
                zero_padding=False,
                cov_plot=0.99,
                ignore_zero_for_y_scale=True,
            )
            plot(
                pdf,
                "Coverage",
            ...
            plot(
                pdf,
                "Coverage + strand / total Coverage [%]",
                "# bases of reference",
                names,
                [st.CoverageStrandedPercent(False) for st in stats],
                plot_legend,
                title,
                zero_padding=False,
            )
            plot(
                ...
                "# bases of reference",
                names,
                [st.CoverageStrandedPercent(True) for st in stats],
                plot_legend,
                title,
                zero_padding=False,
            )
            ...
            plot(
                pdf,
                "Fragment duplication number",
                "% fragments",
                names,
                [st.FragmentDuplicationNumber() for st in stats],
                plot_legend,
                title,
                log=True,
                normalize=True,
                rmempty=True,
                marker_dist=1 if plot_markers else 0,
                marker_max=10 if plot_markers else 0,
            )
            ...
            plot(
                ...
                "% read pairs",
                names,
                [st.InsertLengths() for st in stats],
                plot_legend,
                title,
                normalize=True,
                marker_dist=50 if plot_markers else 0,
                cov_plot=0.98,
            )

            plot(
                pdf,
                "GC read content[%] (first)",
                "% reads",
                names,
                [st.GCReadContent(0) for st in stats],
                plot_legend,
                title,
                zero_padding=False,
                normalize=True,
            ...
            plot(
                pdf,
                "GC read content[%] (second)",
                "% reads",
                names,
                [st.GCReadContent(1) for st in stats],
                plot_legend,
                title,
                zero_padding=False,
                normalize=True,
            ...
            plot(
                ...
                "% fragments",
                names,
                [st.GCFragmentContent() for st in stats],
                plot_legend,
                title,
                zero_padding=False,
                normalize=True,
            ...
            plot(
                ...
                "Preference for GC",
                names,
                [st.GCFragmentContentBias() for st in stats],
                plot_legend,
                title,
                zero_padding=False,
                normalize=True,
            ...
            nucleotidePlot(
                ...
                "% A/C/G/T at position",
                names,
                [[st.SequenceContent(0, n) for st in stats] for n in range(4)],
                plot_legend,
                zero_padding=False,
                shift_x=1,
                pos_normalize=True,
            ...
            nucleotidePlot(
                pdf,
                "Read position (second)",
                "% A/C/G/T at position",
                names,
                [[st.SequenceContent(1, n) for st in stats] for n in range(4)],
                plot_legend,
                zero_padding=False,
                shift_x=1,
                pos_normalize=True,
            ...
            nucleotidePlot(
                pdf,
                "Read Position",
                "Preference for A/C/G/T",
                names,
                [[(0, st.FragmentSurroundingBiasByBase(n)) for st in stats] for n in range(4)],
                plot_legend,
                zero_padding=False,
                shift_x=-10,
                Nlegend=3,
            ...
            nucleotidePlot(
                pdf,
                "position before(-)/after(+) fragment (forward)",
                "% A/C/G/T at position",
                names,
                [[st.OutskirtContent(0, n) for st in stats] for n in range(4)],
                plot_legend,
                zero_padding=False,
                shift_x=-20,
                pos_normalize=True,
            ...
            nucleotidePlot(
                pdf,
                "position before(-)/after(+) fragment (reverse)",
                "% A/C/G/T at position",
                names,
                [[st.OutskirtContent(1, n) for st in stats] for n in range(4)],
                plot_legend,
                zero_padding=False,
                shift_x=-20,
                pos_normalize=True,
            ...
            plot(
                ...
                "# N",
                names,
                [st.SequenceContent(0, 4) for st in stats],
                plot_legend,
                title,
                zero_padding=False,
                shift_x=1,
            ...
            plot(
                ...
                "# N",
                names,
                [st.SequenceContent(1, 4) for st in stats],
                plot_legend,
                title,
                zero_padding=False,
                shift_x=1,
            ...
            if len(names) > 2:
                ...
                plot(
                    ...
                    "Mean quality",
                    names,
                    [st.BaseQualityMean(0) for st in stats],
                    plot_legend,
                    title,
                    zero_padding=False,
                    shift_x=1,
                    legend="lower left",
                    yrange=[min_qual - 1, max_qual + 1],
                )
                plot(
                    pdf,
                    "Read position (second)",
                    "Mean quality",
                    names,
                    [st.BaseQualityMean(1) for st in stats],
                    plot_legend,
                    title,
                    zero_padding=False,
                    shift_x=1,
                ...
                plotBaseQualities(
                    ...
                    names,
                    [st.BaseQualityMean(0) for st in stats],
                    [st.BaseQualityMinimum(0) for st in stats],
                    [st.BaseQualityFirstQuartile(0) for st in stats],
                    [st.BaseQualityMedian(0) for st in stats],
                    [st.BaseQualityThirdQuartile(0) for st in stats],
                    [st.BaseQualityMaximum(0) for st in stats],
                    plot_legend,
                    title,
                )
                plotBaseQualities(
                    ...
                    names,
                    [st.BaseQualityMean(1) for st in stats],
                    [st.BaseQualityMinimum(1) for st in stats],
                    [st.BaseQualityFirstQuartile(1) for st in stats],
                    [st.BaseQualityMedian(1) for st in stats],
                    [st.BaseQualityThirdQuartile(1) for st in stats],
                    [st.BaseQualityMaximum(1) for st in stats],
                    plot_legend,
                    title,
                )
            ...
            plot(
                pdf,
                "Mean sequence quality (first)",
                "% reads",
                names,
                [st.SequenceQualityMean(0) for st in stats],
                plot_legend,
                title,
                zero_padding=False,
                normalize=True,
            ...
            plot(
                pdf,
                "Mean sequence quality (second)",
                "% reads",
                names,
                [st.SequenceQualityMean(1) for st in stats],
                plot_legend,
                title,
                zero_padding=False,
                normalize=True,
            ...
            plotSequenceQualities(
                pdf,
                "Sequence quality (first)",
                "% reads",
                names,
                [st.SequenceQualityMean(0) for st in stats],
                [st.SequenceQualityMinimum(0) for st in stats],
                [st.SequenceQualityFirstQuartile(0) for st in stats],
                [st.SequenceQualityMedian(0) for st in stats],
                [st.SequenceQualityThirdQuartile(0) for st in stats],
                [st.SequenceQualityMaximum(0) for st in stats],
                plot_legend,
            )
            plotSequenceQualities(
                pdf,
                "Sequence quality (second)",
                "% reads",
                names,
                [st.SequenceQualityMean(1) for st in stats],
                [st.SequenceQualityMinimum(1) for st in stats],
                [st.SequenceQualityFirstQuartile(1) for st in stats],
                [st.SequenceQualityMedian(1) for st in stats],
                [st.SequenceQualityThirdQuartile(1) for st in stats],
                [st.SequenceQualityMaximum(1) for st in stats],
                plot_legend,
            )
            plot(
                pdf,
                "Sequence error probability mean (first)",
                "% reads",
                names,
                [st.SequenceQualityProbabilityMean(0) for st in stats],
                plot_legend,
                title,
                zero_padding=False,
                normalize=True,
            ...
            plot(
                pdf,
                "Sequence error probability mean (second)",
                "% reads",
                names,
                [st.SequenceQualityProbabilityMean(1) for st in stats],
                plot_legend,
                title,
                zero_padding=False,
                normalize=True,
            ...
            plot2dQuality(
                ...
                [
                    (
                        st.BaseQualityForSequenceStart(0),
                        [
                            st.BaseQualityForSequence(0, sq)
                            for sq in range(st.BaseQualityForSequenceStart(0), st.BaseQualityForSequenceEnd(0))
                        ],
                    )
                    for st in stats
                ],
            ...
            plot2dQuality(
                ...
                [
                    (
                        st.BaseQualityForSequenceStart(1),
                        [
                            st.BaseQualityForSequence(1, sq)
                            for sq in range(st.BaseQualityForSequenceStart(1), st.BaseQualityForSequenceEnd(1))
                        ],
                    )
                    for st in stats
                ],
            ...
            plot2dQuality(
                pdf,
                "Preceding quality (first)",
                "Following quality",
                names,
                [
                    (
                        st.BaseQualityForPrecedingQualityStart(0),
                        [
                            st.BaseQualityForPrecedingQuality(0, sq)
                            for sq in range(
                                st.BaseQualityForPrecedingQualityStart(0), st.BaseQualityForPrecedingQualityEnd(0)
                            )
                        ],
                    )
                    for st in stats
            ...
            plot2dQuality(
                pdf,
                "Preceding quality (second)",
                "Following quality",
                names,
                [
                    (
                        st.BaseQualityForPrecedingQualityStart(1),
                        [
                            st.BaseQualityForPrecedingQuality(1, sq)
                            for sq in range(
                                st.BaseQualityForPrecedingQualityStart(1), st.BaseQualityForPrecedingQualityEnd(1)
                            )
                        ],
                    )
                    for st in stats
            ...
            plot2dQuality(
                ...
                [
                    (
                        st.SequenceQualityPairsStart(),
                        [
                            st.SequenceQualityPairs(sq)
                            for sq in range(st.SequenceQualityPairsStart(), st.SequenceQualityPairsEnd())
                        ],
                    )
                    for st in stats
                ],
            ...
            plot(
                ...
                "Mean Sequence Quality (first)",
                names,
                [st.MeanSequenceQualityMeanByFragmentLength(0) for st in stats],
                plot_legend,
                title,
                legend="lower right",
                shift_x=5,
            ...
            plot(
                ...
                "Mean Sequence Quality (second)",
                names,
                [st.MeanSequenceQualityMeanByFragmentLength(1) for st in stats],
                plot_legend,
                title,
                legend="lower right",
                shift_x=5,
            ...
            plot2dQuality(
                pdf,
                "Distance to start of error region",
                "Error rate",
                names,
                [
                    (
                        st.ErrorRatesByDistanceStart(),
                        [
                            st.ErrorRatesByDistance(dist)
                            for dist in range(st.ErrorRatesByDistanceStart(), st.ErrorRatesByDistanceEnd())
                        ],
                    )
                    for st in stats
                ],
            ...
            plot2dQuality(
                ...
                [
                    (
                        st.ErrorRatesByGCStart(),
                        [st.ErrorRatesByGC(gc) for gc in range(st.ErrorRatesByGCStart(), st.ErrorRatesByGCEnd())],
                    )
                    for st in stats
                ],
            )
            ...
            nucleotidePlot(
                pdf,
                "Nucleotide content (first) [%]",
                "Average sequence quality",
                names,
                [[st.AverageSequenceQualityForBase(0, n) for st in stats] for n in range(4)],
                plot_legend,
                Nlegend=1,
                legend="",
            )
            nucleotidePlot(
                pdf,
                "Nucleotide content (second) [%]",
                "Average sequence quality",
                names,
                [[st.AverageSequenceQualityForBase(1, n) for st in stats] for n in range(4)],
                plot_legend,
                Nlegend=1,
                legend="",
            )
            plot(
                ...
                "Average sequence quality",
                names,
                [st.AverageSequenceQualityForGC(0) for st in stats],
                plot_legend,
                title,
                zero_padding=False,
            )
            plot(
                ...
                "Average sequence quality",
                names,
                [st.AverageSequenceQualityForGC(1) for st in stats],
                plot_legend,
                title,
                zero_padding=False,
            )
            ...
            nucleotidePlot(
                ...
                "% of A/C/G/T bases",
                names,
                [[st.NucleotideQuality(0, n) for st in stats] for n in range(4)],
                plot_legend,
                normalize=True,
                zero_padding=False,
            )
            nucleotidePlot(
                ...
                "% of A/C/G/T bases",
                names,
                [[st.NucleotideQuality(1, n) for st in stats] for n in range(4)],
                plot_legend,
                normalize=True,
                zero_padding=False,
            )
            ...
            plotTileQuality(
                ...
                names,
                [
                    {tile: st.TileQualityMeanDifference(0, tile_id) for tile_id, tile in enumerate(st.TileNames())}
                    for st in stats
                ],
            )
            plotTileQuality(
                ...
                names,
                [
                    {tile: st.TileQualityMeanDifference(1, tile_id) for tile_id, tile in enumerate(st.TileNames())}
                    for st in stats
                ],
            )
            ...
            plot(
                ...
                "# bases of reference",
                names,
                [st.ErrorCoverage() for st in stats],
                plot_legend,
                title,
                zero_padding=False,
                cov_plot=0.99,
            ...
            plot(
                pdf,
                "Error Coverage / Base Coverage [%]",
                "# bases of reference",
                names,
                [st.ErrorCoveragePercent() for st in stats],
                plot_legend,
                title,
                zero_padding=False,
                log=True,
            ...
            plot2dQuality(
                pdf,
                "Error Coverage + strand",
                "Error Coverage - strand",
                names,
                [
                    (0, [(0, [st.ErrorCoveragePercentStranded(i, j) for j in range(101)]) for i in range(101)])
                    for st in stats
                ],
                axis_lable_dist=10,
            )
            ...
            plot(
                pdf,
                "Errors in read (first)",
                "% reads",
                names,
                [st.ErrorsPerRead(0) for st in stats],
                plot_legend,
                title,
                log=True,
                normalize=True,
            ...
            plot(
                pdf,
                "Errors in read (second)",
                "% reads",
                names,
                [st.ErrorsPerRead(1) for st in stats],
                plot_legend,
                title,
                log=True,
                normalize=True,
            ...
            plotCalledBases(
                ...
                "% Called",
                names,
                [[[st.CalledBasesByBaseQuality(0, i, j) for j in range(5)] for i in range(4)] for st in stats],
                [[st.NucleotideQuality(0, n) for n in range(5)] for st in stats],
                False,
            )
            ...
            plotCalledBases(
                ...
                "% Called",
                names,
                [[[st.CalledBasesByBaseQuality(1, i, j) for j in range(5)] for i in range(4)] for st in stats],
                [[st.NucleotideQuality(1, n) for n in range(5)] for st in stats],
                False,
            )
            ...
            plotCalledBases(
                ...
                "% Called",
                names,
                [[[st.CalledBasesByPosition(0, i, j) for j in range(5)] for i in range(4)] for st in stats],
                [[st.SequenceContent(0, n) for n in range(5)] for st in stats],
                False,
            )
            ...
            plotCalledBases(
                ...
                "% Called",
                names,
                [[[st.CalledBasesByPosition(1, i, j) for j in range(5)] for i in range(4)] for st in stats],
                [[st.SequenceContent(1, n) for n in range(5)] for st in stats],
                False,
            )
            ...
            plot(
                ...
                "% Coverage Blocks",
                names,
                [st.BlockErrorRate() for st in stats],
                plot_legend,
                title,
                normalize=True,
            )
            plot(
                pdf,
                "% Systematic Error Positions",
                "% Coverage Blocks",
                names,
                [st.BlockPercentSystematic() for st in stats],
                plot_legend,
                title,
                normalize=True,
            )
            plot(
                ...
                "% Valid Positions",
                names,
                [st.SystematicErrorPValues() for st in stats],
                plot_legend,
                title,
                normalize=True,
                shift_x=0.5,
            ...
            nucleotidePlot(
                ...
                "# A-/C-/G-/T-mers",
                names,
                [[st.HomopolymerDistribution(n) for st in stats] for n in range(4)],
                plot_legend,
                log=True,
            )
            ...
            plot(
                ...
                "# insertions",
                names,
                [st.InDelErrorByLength(0) for st in stats],
                plot_legend,
                title,
            )
            plot(
                ...
                "# inserted bases",
                names,
                [st.InDelErrorByPosition(0) for st in stats],
                plot_legend,
                title,
            )
            ...
            plot(
                ...
                "# deletions",
                names,
                [st.InDelErrorByLength(1) for st in stats],
                plot_legend,
                title,
            )
            plot(
                ...
                "# deleted bases",
                names,
                [st.InDelErrorByPosition(1) for st in stats],
                plot_legend,
                title,
            )
            ...
            plotTileAbundance(
                ...
                "# read pairs",
                names,
                [st.TileAbundance() for st in stats],
                [st.TileNames() for st in stats],
                plot_legend,
                title,
            )
